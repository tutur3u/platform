...
- **Mira First-Turn Handoff**: When creating a new Mira chat, do not call `sendMessage` from the chat-creation mutation callback that still closes over the pre-creation `useChat` instance. Incorrect pattern: create the chat, then immediately call `sendMessage(...)` from the same callback while the hook still points at the old runtime. Correct pattern: queue the first user turn and flush it only after the runtime remounts with the persisted chat id (for example via an effect keyed to the persisted chat id or a boolean flag checked after mount), while keeping an optimistic user message visible until the real message is sent.
...
- **Planner No-Op UX**: If Mira selects only `no_action_needed`, do not render planner-only `select_tools` rows in the transcript, and key any in-progress planner timer/status off a persisted enum or boolean (for example `PlannerStatus.SELECTING_TOOLS` or `isToolSelectionActive`) instead of transient object references so hidden retries cannot reset the visible elapsed timer.
...
See also:
- **[6.3.1 Security & Validation (Core)]** for digest, logging, and persistence integrity rules.
- **[6.3.2 Mira Tool Orchestration]** for planner/meta-tool loop rules.
- **[6.3.3 Mira Attachment Handling]** for attachment routing, restoration, and visibility rules.

#### 6.3.1 Security & Validation (Core)
...
- **Mira Post-Tool Closeout**: After successful non-visual action tools (for example task/calendar/file operations), Mira must finish with a user-facing text reply in the same turn. Do not end on tool summaries alone, and if the model emits no final text, persist a concrete outcome summary instead of a generic “background steps” fallback.
- **Mira Workspace Switching**: For explicit requests that name another workspace, do not keep re-offering or re-calling `get_workspace_context` after it has already returned. Progress the tool loop toward `list_accessible_workspaces` and `set_workspace_context`, then continue with the requested productivity tool.

#### 6.3.3 Mira Attachment Handling
