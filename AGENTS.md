...
- **Workspace Member Key Assumptions**: Do not select `workspace_members.id` in API membership checks unless the schema has been verified to include that column. Prefer existing key columns such as `user_id` or `ws_id` with `.maybeSingle()` so access checks do not fail on schemas where `workspace_members` has no surrogate `id`.
...
- **Immersive Mobile View Transitions**: When a mobile screen toggles between fullscreen/immersive and overview modes, reset scroll position deliberately and recompute bottom insets for any restored nav/FAB chrome. Do not let immersive chat scroll offsets or composer spacing bleed into overview mode, or the page can appear visually cut off at the top or bottom.
- **Flutter L10n Asset Formatting**: Do not send `.arb` files through `dart format`; format only Dart sources and regenerate `flutter gen-l10n` after ARB edits. ARB files are JSON assets, and treating them like Dart creates avoidable tooling noise during verification.
- **Compact Shell Safe Areas**: In the mobile compact shell, pages rendered inside `ShellPage` already sit under shell-managed app bar/footer chrome. Do not add unconditional inner `SafeArea` top/bottom padding in feature pages like Assistant, or the body can lose usable height and appear clipped before reaching the visible edges.
- **Flutter Compact Test Viewports**: New mobile cards and hero surfaces must be validated against the app's widget-test viewport, not just simulator screenshots. Compact horizontal carousels and featured cards should keep content within tighter test constraints or `flutter test` will catch render overflows that are easy to miss visually.
- **Global App Picker Semantics**: In the mobile shell, the global `Apps` tab should always behave as an app picker entry point, not as a proxy for the last selected mini-app. Keep module-specific navigation inside the dedicated mini-app footer or page chrome so the main bottom navigation remains predictable.
...
- **Flutter Analyze Ignore Hygiene**: In `apps/mobile`, avoid speculative `ignore_for_file` directives. `flutter analyze` reports `unnecessary_ignore` as a failing issue during `bun check:mobile`, so suppress only lints that are actually emitted in that file.
- **Shell-Owned Mobile App Chrome**: In compact mobile shell flows, when the global bottom nav already provides the route hierarchy and the `Apps` tab always returns to the picker, do not add duplicate per-page back buttons in sub-app `AppBar`s. Keep sub-app top bars focused on page title plus local actions.
- **Shared Flutter App Bars vs Test Providers**: When extracting a reusable mobile `AppBar` that injects shell widgets like avatar/profile actions, do not assume feature/widget tests mount the full auth/provider tree. Gate provider-dependent trailing actions defensively or make them injectable so shared chrome can be reused across standalone page tests without `ProviderNotFoundException`.
- **Flutter Standalone Card Surfaces**: For tinted or pastel tappable cards in Flutter, do not rely on `Ink` decoration unless there is a clear `Material` ancestor. Prefer `Material` or `Container`-backed surfaces with explicit background/border so cards remain visibly filled in both dark and light themes.
- **Chat-First Mobile Assistant Layouts**: In the mobile assistant, keep the main surface focused on the transcript and composer. Move secondary controls such as model selection, credits, rename, export, and display toggles into modal settings/history surfaces instead of persistent dashboard cards, and do not mix task/calendar insight widgets into the chat header.
- **Stable Shell Top Bars Across Tabs**: Do not inject tab-specific controls into the shared mobile shell top bar when only one tab needs them. Keep the shell navbar layout stable across Home, Assistant, and Apps, and place assistant-only actions in the assistant surface itself (for example the composer action hub) to avoid visible layout shifts during tab switches.
- **Opt-In Mobile Fullscreen Chat**: Do not auto-enter fullscreen/immersive mode merely because the user opened the assistant tab or a workspace reloaded. Keep fullscreen opt-in from an explicit assistant control so shell chrome, composer placement, and navigation state stay predictable.
- **Inline Mobile Streaming State**: In the mobile assistant, create and update the pending assistant message as soon as a stream starts, and keep streaming/thinking indicators inline with the transcript instead of mirrored in separate header cards. Otherwise streamed replies can appear stale until the user navigates away and back.
...
- **shadcn Flutter TextField API**: In `shadcn_flutter`, use supported `TextField` props like `controller`, `hintText`, and `onChanged`. Do not assume web-style props such as `placeholder`/`leading`, which can break `flutter analyze` and block `bun check:mobile`.
- **Flutter macOS Script Sandbox**: After Xcode/macOS project upgrades, keep `ENABLE_USER_SCRIPT_SANDBOXING = NO` in `apps/mobile/macos/Runner.xcodeproj/project.pbxproj` build configurations. Enabling it breaks Flutter macOS CI builds during `Flutter Assemble` with sandbox file-read/write denials inside `build/macos`.
- **FlutterFire CLI in Apple Build Scripts**: For iOS/macOS Xcode Crashlytics upload phases generated by FlutterFire, do not require a globally installed `flutterfire` binary for CI builds. Guard the script with `command -v flutterfire` or otherwise tolerate the CLI being absent so artifact builds do not fail before packaging.
