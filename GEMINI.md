...
- **Centralized Settings (CRITICAL):** Settings dialogs use a **Shell + Content** multi-app pattern. The shared shell (`@tuturuuu/ui/custom/settings-dialog-shell`) handles layout (sidebar nav, search, breadcrumbs, mobile drawer). Each app has its own `SettingsDialog` (e.g., `apps/web/src/components/settings/settings-dialog.tsx`, `apps/tudo/src/components/settings/settings-dialog.tsx`) that wraps the shell and uses `primaryGroupLabels` to highlight its domain (web → User Settings first, tudo → Tasks first, future apps follow same pattern). Portable settings components live in `@tuturuuu/ui/custom/settings/*`. **NEVER** create separate settings pages or standalone modals — add tabs within the app's `SettingsDialog`. Extract portable settings to `packages/ui` when they have no `@/` imports. Pass `workspace` prop to child components instead of raw `wsId`.
