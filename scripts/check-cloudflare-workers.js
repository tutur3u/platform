...
function validateRustBackendWorkflow(workflowContent) {
  ...
  const requiredSnippets = [
    ...
    'post-deploy-smoke:',
    'name: Smoke deployed Cloudflare Workers',
    'needs: [cloudflare-deployment-preflight, deploy-backend, deploy-tanstack-web]',
    "needs.cloudflare-deployment-preflight.outputs.deployment_mode == 'deploy'",
    "needs.cloudflare-deployment-preflight.outputs.deploy_backend == 'true'",
    "needs.cloudflare-deployment-preflight.outputs.deploy_tanstack_web == 'true'",
    "needs.deploy-backend.result == 'success'",
    "needs.deploy-tanstack-web.result == 'success'",
