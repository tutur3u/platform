...
test('inventoryNextAppRoutes ignores generated legacy API wrappers', () => {
  const { appDir, rootDir } = makeTempFixture();
  const legacyApiDir = path.join(
    rootDir,
    'apps',
    'web',
    'src',
  ...
  fs.mkdirSync(path.dirname(appWrapperPath), { recursive: true });
  fs.mkdirSync(path.dirname(legacyRoutePath), { recursive: true });
  fs.writeFileSync(
    appWrapperPath,
...
`
  ...
  fs.writeFileSync(legacyRoutePath, 'export async function GET() {}\n');
  ...
  assert.equal(
    manifest.routes.some(
      (route) =>
        route.sourceFile === 'apps/web/src/app/api/v1/users/me/profile/route.ts'
    ),
    false
  );
  assert.ok(
    manifest.routes.some(
      (route) =>
        route.id ===
        'api:/api/v1/users/me/profile:apps/web/src/legacy-api-routes/v1/users/me/profile/route.ts'
    )
  );
});
