...
begin
  ...
  if _actor_id is null
    or not public.has_workspace_permission(_ws_id, _actor_id, 'view_finance_stats')
  then
...
comment on function private.get_finance_overview_metrics(
  ...
  'Server-owned finance overview helper that returns dashboard counts, income, expense, net totals, and recent 30-day pace for one workspace after view_finance_stats permission is verified in Postgres.';
