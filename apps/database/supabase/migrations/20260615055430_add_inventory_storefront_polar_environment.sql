-- Per-storefront Polar environment: lets each storefront target Polar sandbox or
-- production for checkout, instead of guessing from the deployment env. Additive
-- with a 'production' default so existing storefronts keep their current behavior.

alter table "private"."inventory_storefronts"
  add column if not exists "polar_environment" text not null default 'production'
  check ("polar_environment" in ('sandbox', 'production'));

-- Re-create the operator storefront list RPC to surface polarEnvironment so the
-- editor pre-fills the switcher and rows can badge it. Body is identical to the
-- previous definition except for the added column in the CTE and the added
-- 'polarEnvironment' key in the storefront JSON.
