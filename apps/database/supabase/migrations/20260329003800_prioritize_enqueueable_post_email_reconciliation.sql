...
begin
  ...
  limited_posts as (
    ...
    where (
      p_max_posts is null
      or p_max_posts <= 0
      or ranked_orphaned_posts.row_num <= p_max_posts
    )
  ),
...
'Finds recent approved post recipients that have no post_email_queue row and no sent_emails coverage, prioritizes posts that still have enqueueable recipients, inserts queued rows set-wise, and returns reconciliation diagnostics for cron processing.';
