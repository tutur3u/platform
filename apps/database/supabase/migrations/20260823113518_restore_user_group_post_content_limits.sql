-- Restore the rich-text allowances lost when a later migration replaced this
-- shared limit function from an older snapshot. Group post writes also create
-- audit rows, so both tables must retain the same content limits.
...
alter table private.user_group_posts
  validate constraint content_strict_length_check;
alter table private.user_group_posts
  validate constraint content_strict_bytes_check;

alter table private.user_group_post_logs
  drop constraint if exists content_strict_length_check;
alter table private.user_group_post_logs
  drop constraint if exists content_strict_bytes_check;
alter table private.user_group_post_logs
