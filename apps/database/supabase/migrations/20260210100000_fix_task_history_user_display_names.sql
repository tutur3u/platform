-- Fix user display names in task history RPCs
-- When display_name is NULL in public.users, fall back to full_name or email
-- from user_private_details. Both RPCs are SECURITY DEFINER so the join
-- bypasses RLS on user_private_details.

-- =============================================================================
-- get_task_history: single-task variant
-- =============================================================================
...
BEGIN
    ...
    SELECT
        ...
        u.id AS user_id,
        COALESCE(u.display_name, upd.full_name, upd.email) AS user_display_name,
        u.avatar_url AS user_avatar_url, eh.total_count
    FROM enriched_history eh
    LEFT JOIN public.users u ON eh.changed_by = u.id
    LEFT JOIN public.user_private_details upd ON eh.changed_by = upd.user_id;
END;
$$;

COMMENT ON FUNCTION public.get_task_history IS 'Retrieves history for a specific task with pagination, filtering, user info (with fallback to user_private_details), and dynamic lookup for project and label names when missing.';


-- =============================================================================
-- get_workspace_task_history: workspace-wide variant
-- =============================================================================
...
BEGIN
    ...
    WITH filtered_history AS (
        ...
        WHERE th.deleted_at IS NULL
          ...
          AND (
              wb.ws_id = p_ws_id
              OR (t.id IS NULL AND (th.metadata->>'workspace_id')::UUID = p_ws_id)
          )
...
COMMENT ON FUNCTION public.get_workspace_task_history IS 'Retrieves task history for a workspace with pagination, filtering, user info (with fallback to user_private_details), project/label enrichment. Includes both soft-deleted and permanently deleted tasks. Uses workspace_id from metadata for permanently deleted tasks.';
