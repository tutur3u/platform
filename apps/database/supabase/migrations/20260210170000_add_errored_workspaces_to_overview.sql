-- Extend overview RPCs with subscription error tracking

-- Drop existing functions to change return types / parameters
DROP FUNCTION IF EXISTS get_workspace_overview_summary();
DROP FUNCTION IF EXISTS get_workspace_overview(text, text, text, text, text, text, text, int, int);

-- 1. Extend summary with errored_workspaces count
...
RETURNS TABLE (
  ...
  with_zero_subscriptions bigint,
  with_single_subscription bigint,
  with_multiple_subscriptions bigint,
  errored_workspaces bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
STABLE
...
BEGIN
  ...
  sub_counts AS (
    SELECT
      ws_sub.ws_id,
      COUNT(*) FILTER (WHERE ws_sub.status IN ('active', 'trialing')) AS active_cnt
    FROM workspace_subscriptions ws_sub
    GROUP BY ws_sub.ws_id
  ),
  subscription_errors AS (
    SELECT DISTINCT se.ws_id
    FROM workspace_subscription_errors se
  ...
  SELECT
    ...
    (COUNT(*) - COUNT(DISTINCT CASE WHEN COALESCE(scc.active_cnt, 0) > 0 THEN w.id END))::bigint AS with_zero_subscriptions,
    COUNT(DISTINCT CASE WHEN scc.active_cnt = 1 THEN w.id END)::bigint AS with_single_subscription,
    COUNT(DISTINCT CASE WHEN scc.active_cnt > 1 THEN w.id END)::bigint AS with_multiple_subscriptions,
    COUNT(DISTINCT serr.ws_id)::bigint AS errored_workspaces
  FROM workspaces w
  LEFT JOIN member_counts mc ON mc.ws_id = w.id
  LEFT JOIN sub_tiers st ON st.ws_id = w.id
  LEFT JOIN sub_counts scc ON scc.ws_id = w.id
  LEFT JOIN subscription_errors serr ON serr.ws_id = w.id;
END;
$$;

-- 2. Extend overview with has_subscription_error + errored filter
CREATE OR REPLACE FUNCTION get_workspace_overview(
  p_search text DEFAULT NULL,
  p_tier text DEFAULT NULL,
  p_status text DEFAULT NULL,
  p_workspace_type text DEFAULT NULL,
  p_sub_count text DEFAULT NULL,
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      (serr.ws_id IS NOT NULL) AS has_subscription_error
    FROM workspaces w
  ...
  filtered AS (
    ...
    WHERE
      ...
      AND (p_sub_count IS NULL OR p_sub_count = '' OR
        (p_sub_count = 'none' AND e.active_subscription_count = 0) OR
        (p_sub_count = 'single' AND e.active_subscription_count = 1) OR
        (p_sub_count = 'multiple' AND e.active_subscription_count > 1) OR
        (p_sub_count = 'errored' AND e.has_subscription_error = true))
  ),
  counted AS (
