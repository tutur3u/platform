-- Include soft-deleted tasks in task history RPC
-- This allows viewing history for deleted tasks (showing when they were deleted)

-- Drop existing function first since we're changing the return type (adding task_deleted_at column)
...
RETURNS TABLE (
    ...
    task_id UUID,
    task_name TEXT,
    task_deleted_at TIMESTAMPTZ,  -- NEW: indicates if task is soft-deleted
    board_id UUID,
    board_name TEXT,
    changed_by UUID,
...
BEGIN
    ...
    WITH workspace_tasks AS (
        SELECT
            t.id AS task_id,
            t.name AS task_name,
            t.deleted_at AS task_deleted_at,  -- Include deletion timestamp
            wb.id AS board_id,
        ...
        WHERE wb.ws_id = p_ws_id
          -- REMOVED: AND t.deleted_at IS NULL (now including soft-deleted tasks)
    ...
    SELECT
        eh.id, eh.task_id, eh.task_name, eh.task_deleted_at, eh.board_id, eh.board_name, eh.changed_by, eh.changed_at,
...
COMMENT ON FUNCTION public.get_workspace_task_history IS 'Retrieves task history for a workspace with pagination, filtering, user info, project/label enrichment, and includes soft-deleted tasks.';
