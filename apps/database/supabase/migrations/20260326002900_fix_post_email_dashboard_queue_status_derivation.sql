...
'Returns workspace-wide post email delivery counts filtered by optional queue status so dashboard summary accurately reflects active queue rows.';
  ...
  with filtered as (
    ...
    where wu.ws_id = p_ws_id
      ...
      and (
        ...
        or upc.approval_status = p_approval_status
      )
  )
  select
    filtered.row_key,
...
comment on function public.get_workspace_post_email_rows(uuid, uuid[], uuid[], uuid, text, public.approval_status, timestamptz, integer, integer) is
'Returns paginated workspace post email deliveries with queue status derived from real queue rows or sent email records.';
