-- Direct payment links are authoritative and are included by the child branch.
-- Only unlinked historical payments need the reverse transaction lookup.
...
begin
  ...
  with events as (
    ...
    where v.table_name = 'finance_invoices'
      ...
      and (p_from is null or v.ts >= p_from) and (p_to is null or v.ts <= p_to)
      and (p_invoice_id is null or coalesce(v.record, v.old_record)->>'id' = p_invoice_id::text)
      and (coalesce(p_query, '') = ''
        or coalesce(v.record, v.old_record)->>'id' ilike '%' || left(p_query, 120) || '%'
        or exists (select 1 from public.workspace_users customer
          where customer.id = (coalesce(v.record, v.old_record)->>'customer_id')::uuid and customer.ws_id = p_ws_id
            and customer.full_name ilike '%' || left(p_query, 120) || '%')
        or exists (select 1 from public.users actor
          where actor.id = coalesce(v.auth_uid, case when v.table_name = 'finance_invoices' and v.op = 'INSERT' then (v.record->>'platform_creator_id')::uuid end)
            and actor.display_name ilike '%' || left(p_query, 120) || '%'))
      and (p_action is null or (
        case when v.table_name = 'finance_invoices' and v.op = 'INSERT'
          and exists (select 1 from private.finance_invoice_recovery recovery
            where recovery.ws_id = p_ws_id and recovery.invoice_id = (coalesce(v.record, v.old_record)->>'id')::uuid
              and v.ts = any(recovery.restore_history))
        then 'RESTORE' else v.op::text end) = p_action)
      and (not p_deleted_only or (v.op = 'DELETE'
        and not exists (select 1 from public.finance_invoices current_invoice
          where current_invoice.id = (coalesce(v.record, v.old_record)->>'id')::uuid)))
    order by
      case when p_sort = 'asc' then v.ts end asc,
      case when p_sort = 'asc' then v.id end asc,
      case when p_sort <> 'asc' then v.ts end desc,
      case when p_sort <> 'asc' then v.id end desc
    limit greatest(p_offset, 0) + least(greatest(p_limit, 1), 101)
    ) source_0
    union all
    select * from (
    ...
    where not p_deleted_only
      ...
      and (p_from is null or v.ts >= p_from) and (p_to is null or v.ts <= p_to)
      and (p_invoice_id is null or context.invoice->>'id' = p_invoice_id::text)
      and (coalesce(p_query, '') = ''
        or context.invoice->>'id' ilike '%' || left(p_query, 120) || '%'
        or exists (select 1 from public.workspace_users customer
          where customer.id = (context.invoice->>'customer_id')::uuid and customer.ws_id = p_ws_id
            and customer.full_name ilike '%' || left(p_query, 120) || '%')
        or exists (select 1 from public.users actor
          where actor.id = coalesce(v.auth_uid, case when v.table_name = 'finance_invoices' and v.op = 'INSERT' then (v.record->>'platform_creator_id')::uuid end)
            and actor.display_name ilike '%' || left(p_query, 120) || '%'))
      and (p_action is null or (
        case when v.table_name = 'finance_invoices' and v.op = 'INSERT'
          and exists (select 1 from private.finance_invoice_recovery recovery
            where recovery.ws_id = p_ws_id and recovery.invoice_id = (context.invoice->>'id')::uuid
              and v.ts = any(recovery.restore_history))
        then 'RESTORE' else v.op::text end) = p_action)
      and (not p_deleted_only or (v.op = 'DELETE'
        and not exists (select 1 from public.finance_invoices current_invoice
          where current_invoice.id = (context.invoice->>'id')::uuid)))
    order by
      case when p_sort = 'asc' then v.ts end asc,
      case when p_sort = 'asc' then v.id end asc,
      case when p_sort <> 'asc' then v.ts end desc,
    ...
    where not p_deleted_only and v.table_name = 'wallet_transactions'
      ...
      and coalesce(v.record->>'invoice_id', v.old_record->>'invoice_id') is null
      and (p_from is null or v.ts >= p_from) and (p_to is null or v.ts <= p_to)
      and (p_invoice_id is null or context.invoice->>'id' = p_invoice_id::text)
      and (coalesce(p_query, '') = ''
        or context.invoice->>'id' ilike '%' || left(p_query, 120) || '%'
    ...
    select * from events
    order by
      case when p_sort = 'asc' then ts end asc,
      case when p_sort = 'asc' then id end asc,
      case when p_sort <> 'asc' then ts end desc,
