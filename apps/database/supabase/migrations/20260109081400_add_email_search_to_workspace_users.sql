-- Update get_workspace_users RPC function to include email in search
-- Step 1: Drop existing function
DROP FUNCTION IF EXISTS get_workspace_users(UUID, UUID[], UUID[], TEXT);

-- Step 2: Recreate function with updated search logic
...
BEGIN
    ...
    AND (search_query IS NULL OR (
        wu.full_name ILIKE '%' || search_query || '%' OR 
        wu.display_name ILIKE '%' || search_query || '%' OR
        wu.email ILIKE '%' || search_query || '%'
    ))
