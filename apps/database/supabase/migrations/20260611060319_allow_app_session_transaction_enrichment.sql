-- Allow server-owned Finance app-session routes to enrich transaction lists.
-- These routes authenticate at the API layer and call this RPC with an
-- explicit p_user_id through an admin-backed Supabase client, where auth.uid()
-- is null inside Postgres.
