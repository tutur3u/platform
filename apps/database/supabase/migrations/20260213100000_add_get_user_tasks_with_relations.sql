-- New consolidated RPC: returns tasks + all relations in a single database call.
-- Replaces the pattern of calling get_user_accessible_tasks + 6 parallel queries.
-- The original get_user_accessible_tasks is NOT dropped (backward compatibility).
...
RETURNS TABLE (
  -- Core task fields (same 13 columns as get_user_accessible_tasks)
  task_id UUID,
  task_name TEXT,
  ...
  -- Scheduling settings (flattened from task_user_scheduling_settings)
  ...
  -- Override fields (flattened from task_user_overrides)
...
BEGIN
  -- Resolve workspace (same logic as get_user_accessible_tasks)
  ...
  WHERE
    -- Access control (same logic as get_user_accessible_tasks)
    CASE
