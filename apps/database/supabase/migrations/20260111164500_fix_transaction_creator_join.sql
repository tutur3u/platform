-- Migration: fix_transaction_creator_join
-- Update get_wallet_transactions_with_permissions to use correct join for creator details
-- Previous migration joined workspace_users, but wallet_transactions.creator_id references auth.users
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    SELECT
      ...
      COALESCE(u.display_name, upd.full_name, upd.email) as creator_full_name,
    ...
    FROM public.wallet_transactions wt
    JOIN public.workspace_wallets ww ON wt.wallet_id = ww.id
    LEFT JOIN public.users u ON wt.creator_id = u.id
    LEFT JOIN public.user_private_details upd ON wt.creator_id = upd.user_id
    WHERE ww.ws_id = $5
