create or replace function private.cancel_pending_post_email_queue_for_recipient_email(
  p_email text,
  p_reason text default 'Recipient unsubscribed from Tuturuuu system emails.'
)
returns bigint
language plpgsql
security definer
...
begin
  ...
  update public.post_email_queue peq
  set
    status = 'cancelled',
    batch_id = null,
    claimed_at = null,
    cancelled_at = now(),
    blocked_reason = null,
    last_error = coalesce(nullif(p_reason, ''), 'Recipient unsubscribed from Tuturuuu system emails.')
  from public.workspace_users wu
  where peq.user_id = wu.id
    and lower(btrim(wu.email)) = lower(btrim(p_email))
    and peq.status in ('queued', 'processing', 'failed', 'blocked');
...
revoke all on function private.cancel_pending_post_email_queue_for_recipient_email(text, text)
  from public, anon, authenticated;
grant execute on function private.cancel_pending_post_email_queue_for_recipient_email(text, text)
  to service_role;
...
begin
  ...
  classified as (
    select
      ...
      (
        ...
        and (c.archived_until is null or c.archived_until <= now())
      ) as has_active_recipient,
      (
        c.workspace_user_id is not null
        and c.email is not null
        and btrim(c.email) <> ''
        and c.email ~* '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
        and not public.check_email_blocked(c.email)
  ...
  orphaned_post_stats as (
    select
      ...
      bool_or(
        ...
        and orphaned_rows.has_active_recipient
  ...
  rows_to_insert as (
    ...
    where selected_orphaned_rows.workspace_user_id is not null
      and selected_orphaned_rows.has_active_recipient
...
comment on function private.cancel_pending_post_email_queue_for_recipient_email(text, text) is
'Cancels not-yet-sent post email queue rows for every workspace user matching an unsubscribed recipient email.';

comment on function private.reconcile_orphaned_approved_post_email_queue(timestamptz, integer, integer, uuid) is
'Finds recent approved post recipients that have no post_email_queue row and no sent_emails coverage, excluding inactive workspace users and globally blocked emails before inserting queue rows.';
