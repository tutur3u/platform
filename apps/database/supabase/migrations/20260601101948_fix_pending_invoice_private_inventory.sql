-- Fix pending invoice RPCs after inventory stock tables moved to private.
-- Keep the public RPC contract unchanged and qualify the private inventory join.
...
as $$
declare
	v_normalized_search_query text;
	v_normalized_search_pattern text;
...
begin
	...
	v_normalized_search_query := public.normalize_workspace_user_search_text(p_query);
	v_normalized_search_pattern := '%' || replace(v_normalized_search_query, ' ', '%') || '%';
	...
	filtered_base as (
		...
		where (p_user_ids is null or bd.user_id = any(p_user_ids))
			and (
				v_normalized_search_query = '' or
				public.normalize_workspace_user_search_text(coalesce(bd.user_name, '')) like v_normalized_search_pattern or
				public.normalize_workspace_user_search_text(coalesce(bd.group_name, '')) like v_normalized_search_pattern
			)
			and (
	...
	ranked_inventory as (
		...
		left join private.inventory_products ip on ip.product_id = uglp.product_id
	),
