-- Fix workspace_id key lookup in get_workspace_task_history RPC
-- The task_created trigger stores 'workspace_id' in metadata, not 'ws_id'
-- This migration corrects the key name to match what's actually stored
...
COMMENT ON FUNCTION public.get_workspace_task_history IS 'Retrieves task history for a workspace with pagination, filtering, user info, project/label enrichment. Includes both soft-deleted and permanently deleted tasks. Uses workspace_id from metadata for permanently deleted tasks.';
