-- Create functions to get user challenge statistics

-- Function to get the total number of sessions for a user in a specific challenge
...
-- Function to get the number of sessions a user has completed today for a specific challenge
...
-- Create a stored procedure to retrieve challenge info with user stats
...
BEGIN
  ...
  total_sessions := nova_get_user_total_sessions(challenge_id, user_id);
  
  -- Get user's daily sessions for this challenge
  daily_sessions := nova_get_user_daily_sessions(challenge_id, user_id);
...
-- Create function to get all challenges with user stats
CREATE OR REPLACE FUNCTION nova_get_all_challenges_with_user_stats(
  user_id UUID
) RETURNS jsonb AS $$
DECLARE
  all_challenges jsonb;
  challenge_records RECORD;
  challenges_array jsonb := '[]'::jsonb;
BEGIN
  -- Iterate through all challenges
  FOR challenge_records IN 
    SELECT id FROM nova_challenges
  LOOP
    -- Get challenge with user stats
    all_challenges := nova_get_challenge_with_user_stats(challenge_records.id, user_id);
