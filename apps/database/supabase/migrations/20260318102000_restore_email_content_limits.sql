-- Restore large email content limits after later strict-text migrations
-- unintentionally reset sent_emails/email_audit content fields to the generic
-- 512-character ceiling.
...
alter table public.email_audit
  add constraint html_content_strict_length_check
  check (char_length(html_content) <= 1048576) not valid;
alter table public.email_audit
  add constraint html_content_strict_bytes_check
  check (octet_length(html_content) <= 4194304) not valid;
alter table public.email_audit
  add constraint text_content_strict_length_check
  check (char_length(text_content) <= 1048576) not valid;
alter table public.email_audit
  add constraint text_content_strict_bytes_check
