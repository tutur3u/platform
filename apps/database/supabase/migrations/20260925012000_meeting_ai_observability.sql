-- Preserve first-party app attribution in the central AI Hub consumption views.
...
-- Deployment readiness is published only after both billing and research schema.
create function private.parley_runtime_ready() returns boolean
language sql stable set search_path = '' as $$ select true $$;
revoke all on function private.parley_runtime_ready() from public, anon, authenticated;
grant execute on function private.parley_runtime_ready() to service_role;
