-- Restore a practical synced-calendar description limit.
-- External providers routinely exceed the generic 512-char ceiling.
...
alter table public.workspace_calendar_events
  drop constraint if exists description_strict_length_check;
alter table public.workspace_calendar_events
  drop constraint if exists description_strict_bytes_check;
alter table public.workspace_calendar_events
  drop constraint if exists title_strict_length_check;
alter table public.workspace_calendar_events
  drop constraint if exists title_strict_bytes_check;
alter table public.workspace_calendar_events
...
alter table public.workspace_calendar_events
  add constraint title_strict_length_check
  check (char_length(title) <= 255) not valid;
alter table public.workspace_calendar_events
  add constraint title_strict_bytes_check
  check (octet_length(title) <= 1020) not valid;
alter table public.workspace_calendar_events
  add constraint description_strict_length_check
  check (char_length(description) <= 10000) not valid;
alter table public.workspace_calendar_events
  add constraint description_strict_bytes_check
  check (octet_length(description) <= 40000) not valid;

alter table public.workspace_calendar_events
  validate constraint title_strict_length_check;
alter table public.workspace_calendar_events
  validate constraint title_strict_bytes_check;
alter table public.workspace_calendar_events
  validate constraint description_strict_length_check;
alter table public.workspace_calendar_events
  validate constraint description_strict_bytes_check;
