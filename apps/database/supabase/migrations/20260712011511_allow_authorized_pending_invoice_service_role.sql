-- Satellite finance routes authorize the actor before using the service-role
-- client. Keep authenticated RPC calls protected while allowing that trusted
-- server-side path to execute without an auth.uid() claim.
