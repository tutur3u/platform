-- Increase user_group_posts and user_group_post_logs content limit from 512 to 65536 chars
-- Lesson content (posts) and post change logs can be rich text and often exceed 512 characters
...
as $$
  select case
    ...
    when lower(_table_name) = 'user_feedbacks' and lower(_column_name) = 'content' then 65536
    when lower(_table_name) = 'external_user_monthly_reports' and lower(_column_name) = 'content' then 65536
    when lower(_table_name) = 'external_user_monthly_reports' and lower(_column_name) = 'feedback' then 65536
    when lower(_table_name) = 'external_user_monthly_report_logs' and lower(_column_name) = 'content' then 65536
    when lower(_table_name) = 'external_user_monthly_report_logs' and lower(_column_name) = 'feedback' then 65536
...
-- Rebuild CHECK constraints: sync_strict_public_field_constraints created them
-- with the old 512 limit. Must drop and recreate so they use the new limit.
alter table public.user_group_posts drop constraint if exists content_strict_length_check;
alter table public.user_group_posts drop constraint if exists content_strict_bytes_check;
alter table public.user_group_posts drop constraint if exists title_strict_length_check;
alter table public.user_group_posts drop constraint if exists title_strict_bytes_check;
alter table public.user_group_posts add constraint content_strict_length_check
  check (char_length(content) <= 65536) not valid;
alter table public.user_group_posts add constraint content_strict_bytes_check
  check (octet_length(content) <= 262144) not valid;
alter table public.user_group_posts add constraint title_strict_length_check
  check (char_length(title) <= 512) not valid;
alter table public.user_group_posts add constraint title_strict_bytes_check
  check (octet_length(title) <= 2048) not valid;

alter table public.user_group_post_logs drop constraint if exists content_strict_length_check;
alter table public.user_group_post_logs drop constraint if exists content_strict_bytes_check;
alter table public.user_group_post_logs add constraint content_strict_length_check
  check (char_length(content) <= 65536) not valid;
alter table public.user_group_post_logs add constraint content_strict_bytes_check
  check (octet_length(content) <= 262144) not valid;
...
alter table public.user_feedbacks drop constraint if exists content_strict_length_check;
alter table public.user_feedbacks drop constraint if exists content_strict_bytes_check;
alter table public.user_feedbacks add constraint content_strict_length_check
  check (char_length(content) <= 65536) not valid;
alter table public.user_feedbacks add constraint content_strict_bytes_check
  check (octet_length(content) <= 262144) not valid;

alter table public.external_user_monthly_reports drop constraint if exists content_strict_length_check;
alter table public.external_user_monthly_reports drop constraint if exists content_strict_bytes_check;
alter table public.external_user_monthly_reports drop constraint if exists feedback_strict_length_check;
alter table public.external_user_monthly_reports drop constraint if exists feedback_strict_bytes_check;
alter table public.external_user_monthly_reports add constraint content_strict_length_check
  check (char_length(content) <= 65536) not valid;
alter table public.external_user_monthly_reports add constraint content_strict_bytes_check
  check (octet_length(content) <= 262144) not valid;

alter table public.external_user_monthly_report_logs drop constraint if exists content_strict_length_check;
alter table public.external_user_monthly_report_logs drop constraint if exists content_strict_bytes_check;
alter table public.external_user_monthly_report_logs drop constraint if exists feedback_strict_length_check;
alter table public.external_user_monthly_report_logs drop constraint if exists feedback_strict_bytes_check;
alter table public.external_user_monthly_report_logs add constraint content_strict_length_check
  check (char_length(content) <= 65536) not valid;
alter table public.external_user_monthly_report_logs add constraint content_strict_bytes_check
  check (octet_length(content) <= 262144) not valid;
alter table public.external_user_monthly_report_logs add constraint feedback_strict_length_check
  check (char_length(feedback) <= 65536) not valid;
alter table public.external_user_monthly_report_logs add constraint feedback_strict_bytes_check
  check (octet_length(feedback) <= 262144) not valid;
alter table public.external_user_monthly_reports add constraint feedback_strict_length_check
  check (char_length(feedback) <= 65536) not valid;
alter table public.external_user_monthly_reports add constraint feedback_strict_bytes_check
  check (octet_length(feedback) <= 262144) not valid;
