-- Fix: Remove invalid FOR UPDATE clauses on set-returning function calls.
-- PostgreSQL does not allow FOR UPDATE on function RTEs (range table entries).
-- The actual row-level locking is done on the concrete table queries that follow.

-- 1. check_ai_credit_allowance: remove FOR UPDATE from
--    get_or_create_credit_balance call (this is a read-only check; the
--    actual locking is handled by deduct_ai_credits / deduct_fixed_ai_credits).
...
BEGIN
  ...
  -- No FOR UPDATE here: this is a read-only allowance check.
  -- The function call cannot be locked; actual row locks are taken
  -- by deduct_ai_credits / deduct_fixed_ai_credits when mutating.
...
-- 2. deduct_ai_credits: remove FOR UPDATE from function call at the top;
--    the real lock is the subsequent SELECT ... FROM workspace_ai_credit_balances FOR UPDATE.
...
DECLARE
  ...
  v_metadata JSONB;
BEGIN
  -- Get/create the balance row (no FOR UPDATE on function calls).
  SELECT * INTO v_balance
    FROM public.get_or_create_credit_balance(p_ws_id, p_user_id);
  ...
  -- Lock the actual table row for the duration of this transaction.
  SELECT * INTO v_balance
...
-- 3. deduct_fixed_ai_credits: same fix — remove FOR UPDATE from function call;
--    add the table-level FOR UPDATE that was missing in the original.
...
REVOKE EXECUTE ON FUNCTION public.deduct_ai_credits(
  UUID, TEXT, INTEGER, INTEGER, INTEGER, TEXT, UUID, UUID, JSONB, UUID, INTEGER, INTEGER
) FROM PUBLIC, anon, authenticated;
