-- Increase whiteboard snapshot byte limit from 16 MB to 64 MB.
-- Large boards with many elements, images, and complex drawings can exceed
-- the previous 16 MB ceiling during auto-save, silently truncating content.
...
AS $function$
  select case
    when lower(_table_name) = 'workspace_whiteboards'
      and lower(_column_name) = 'snapshot' then 67108864
