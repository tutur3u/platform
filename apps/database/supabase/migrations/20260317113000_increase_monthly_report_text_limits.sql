-- Increase monthly report text limits and keep enforcement layers aligned.
...
alter table public.external_user_monthly_reports
  drop constraint if exists content_strict_length_check;
alter table public.external_user_monthly_reports
  drop constraint if exists content_strict_bytes_check;
alter table public.external_user_monthly_reports
  drop constraint if exists feedback_strict_length_check;
alter table public.external_user_monthly_reports
  drop constraint if exists feedback_strict_bytes_check;
alter table public.external_user_monthly_reports
  add constraint content_strict_length_check
  check (char_length(content) <= 100000) not valid;
alter table public.external_user_monthly_reports
  add constraint content_strict_bytes_check
  check (octet_length(content) <= 400000) not valid;
alter table public.external_user_monthly_reports
  add constraint feedback_strict_length_check
  check (char_length(feedback) <= 100000) not valid;
alter table public.external_user_monthly_reports
  add constraint feedback_strict_bytes_check
  check (octet_length(feedback) <= 400000) not valid;

alter table public.external_user_monthly_report_logs
  drop constraint if exists content_strict_length_check;
alter table public.external_user_monthly_report_logs
  drop constraint if exists content_strict_bytes_check;
alter table public.external_user_monthly_report_logs
