...
export default function CreatePlanDialog({ plan }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (res.ok) {
      ...
      router.push(`/plans/${normalizedId}`);
      router.refresh();
    } else {
      setCreating(false);
      toast({
    ...
    >
      <DialogTrigger asChild>
        ...
        >
          <div
            className={cn(
              '-inset-px absolute rounded-lg bg-gradient-to-r from-red-500/80 via-pink-500/80 to-blue-500/80 opacity-70 blur-lg transition-all',
              missingFields ||
                creating ||
                'group-hover:-inset-1 group-hover:opacity-100 group-hover:duration-200'
            )}
          />
          <div className="relative inline-flex w-full items-center justify-center rounded-lg bg-gradient-to-r from-red-500/60 via-pink-500/60 to-blue-500/60 px-8 py-2 font-bold text-white transition-all md:text-lg">
            {t('create_plan')}
          </div>
        </button>
      </DialogTrigger>
