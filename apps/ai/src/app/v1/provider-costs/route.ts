...
export async function POST(request: Request) {
  try {
    ...
    const parsed = providerCostSchema.safeParse(body);
    if (
      !parsed.success ||
      !validAccountDay(parsed.data) ||
      Date.parse(parsed.data.observedAt) > Date.now() + 60_000
    ) {
      throw new AiStudioError('Invalid provider cost record.', {
        code: 'invalid_request_error',
        status: 400,
      });
    }
    ...
    const { error } = await db
      .schema('private')
      .rpc('record_external_provider_cost', {
        p_account_id: cost.accountId ?? '',
        p_granularity: cost.granularity,
        p_ws_id: credential.workspaceId,
        p_app_id: appId,
        p_actor_id: credential.actorId,
        p_provider: cost.provider,
        p_external_run_id: cost.externalRunId,
        p_service: cost.service,
        p_amount_usd: cost.amountUsd,
        p_occurred_at: cost.occurredAt,
        p_observed_at: cost.observedAt,
      });
    if (error) {
      console.error('Provider cost persistence failed', { code: error.code });
      throw new AiStudioError('Provider costs are temporarily unavailable.', {
        code: 'server_error',
        status: 503,
      });
    }
    return Response.json(
      { accepted: true, externalRunId: cost.externalRunId },
