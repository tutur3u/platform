...
const BOARDS_LOAD_FAILED_MESSAGE: &str = "Failed to list Mind boards";

const LIST_BOARDS_RPC: &str = "mind_list_boards";

const PATH_PREFIX: &str = "/api/v1/workspaces/";
const PATH_SUFFIX: &str = "/mind/boards";
...
pub(crate) async fn handle_workspaces_mind_boards_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let raw_ws_id = match_boards_path(request.path)?;

    // Only the GET method is migrated to Rust; every other method (POST, etc.)
    // must fall through to the still-active Next.js route, so return None for
    // them instead of 405-ing a valid mutation.
...
async fn boards_response(
    ...
    // Legacy route is configured with `allowAppSessionAuth: true`, so it accepts
    // both Supabase tokens and app session tokens.
    ...
    match list_mind_boards(&config.contact_data, outbound, &resolved_ws_id).await {
        Ok(boards) => no_store_response(json_response(200, json!({ "boards": boards }))),
        Err(()) => error_response(500, BOARDS_LOAD_FAILED_MESSAGE),
    }
}

/// Calls the `private.mind_list_boards` RPC with the service role, mirroring the
/// legacy `callMindRpc('mind_list_boards', { p_ws_id })` on the `private`
/// schema. The RPC returns a `jsonb` array (or `null` when empty), so the
/// response body is the array itself.
async fn list_mind_boards(
    contact_data: &contact::ContactDataConfig,
    outbound: &impl OutboundHttpClient,
    ws_id: &str,
) -> Result<Vec<Value>, ()> {
    let url = contact_data.rpc_url(LIST_BOARDS_RPC).ok_or(())?;
    ...
    // `listMindBoards` coalesces a null result to an empty array.
...
/// Matches `/api/v1/workspaces/<wsId>/mind/boards` exactly and returns the
/// `wsId` segment. Returns `None` for any other path (including the sub-routes
/// such as `/mind/boards/<boardId>/patches`, which contain a `/` in the
/// remaining segment).
fn match_boards_path(path: &str) -> Option<&str> {
