...
const RECURRING_PATH: &str = "/api/v1/workspaces/ws-1/finance/recurring-transactions/upcoming";
const RECURRING_URL: &str =
    "https://tuturuuu.localhost/api/v1/workspaces/ws-1/finance/recurring-transactions/upcoming";
...
fn app_session_token(target_app: &str, scopes: Vec<String>) -> String {
    ...
    let payload = contact::encode_app_session_part(
        serde_json::to_string(&contact::AppCoordinationClaims {
            ...
            jti: "finance-recurring-transactions-test".to_owned(),
...
fn successful_browser_responses(upcoming_rows: &'static str) -> Vec<OutboundResponse> {
    vec![
        outbound_response(200, r#"{"id":"user-1"}"#),
        outbound_response(200, r#"[{"id":"resolved-ws"}]"#),
        outbound_response(200, "true"),
        outbound_response(200, upcoming_rows),
    ]
}

#[tokio::test]
async fn finance_recurring_upcoming_uses_caller_rpc_and_parse_int_days_ahead() {
    let outbound = RecordingOutboundClient::with_responses(successful_browser_responses(
        r#"[{"id":"recurring-1","name":"Rent","amount":500,"frequency":"monthly","next_occurrence":"2026-07-01","wallet_name":"Cash","category_name":"Housing"}]"#,
    ));
    let response = handle_backend_request(
        &backend_config_with_contact_data(),
        request_with_bearer(
            "Bearer browser-access-token",
            "https://tuturuuu.localhost/api/v1/workspaces/ws-1/finance/recurring-transactions/upcoming?daysAhead=45days",
        ),
        &outbound,
    )
    .await;
    ...
    assert_eq!(
        response.body["upcomingTransactions"][0]["id"],
        "recurring-1"
    );
    ...
    assert_eq!(
        ...
        Some(
            r#"{"p_permission":"view_transactions","p_user_id":"user-1","p_ws_id":"resolved-ws"}"#
        )
    );
    assert!(
        calls[3]
            .url
            .ends_with("/rest/v1/rpc/get_upcoming_recurring_transactions")
    );
    assert_eq!(
        calls[3].body.as_deref(),
        Some(r#"{"_ws_id":"resolved-ws","days_ahead":45}"#)
    );
    assert_eq!(
...
#[tokio::test]
async fn finance_recurring_upcoming_accepts_app_session_and_defaults_invalid_days_ahead() {
    let bearer = finance_app_session_bearer();
    let request = BackendRequest {
        authorization: Some(Box::leak(bearer.into_boxed_str())),
        ..request_with_bearer(
            "Bearer unused",
            "https://tuturuuu.localhost/api/v1/workspaces/ws-1/finance/recurring-transactions/upcoming?daysAhead=nope",
        )
    };
    ...
    assert_eq!(response.status, 200);
    assert_eq!(response.body, json!({ "upcomingTransactions": [] }));
    ...
    assert_eq!(
        calls[2].body.as_deref(),
        Some(r#"{"_ws_id":"app-session-ws","days_ahead":30}"#)
    );
    assert_eq!(
...
#[tokio::test]
async fn finance_recurring_upcoming_rejects_missing_view_transactions_with_unauthorized_body() {
    let outbound = RecordingOutboundClient::with_responses(vec![
        outbound_response(200, r#"{"id":"user-1"}"#),
        outbound_response(200, r#"[{"id":"resolved-ws"}]"#),
    ...
    let response = handle_backend_request(
        &backend_config_with_contact_data(),
        request_with_bearer("Bearer browser-access-token", RECURRING_URL),
        &outbound,
    )
    .await;
...
#[tokio::test]
async fn finance_recurring_upcoming_preserves_rpc_failure_body() {
    let outbound = RecordingOutboundClient::with_responses(vec![
        outbound_response(200, r#"{"id":"user-1"}"#),
        outbound_response(200, r#"[{"id":"resolved-ws"}]"#),
        outbound_response(200, "true"),
    ...
    assert_eq!(
        response.body,
        json!({ "message": "Failed to fetch upcoming recurring transactions" })
    );
}
