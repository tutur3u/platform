...
const PATH_SUFFIX: &str = "/external-projects/members/roles";
...
const LOAD_ERROR_MESSAGE: &str = "Error loading CMS access levels";
...
pub(crate) async fn handle_workspaces_external_projects_members_roles_route(
    ...
    // Only GET is migrated. Every other method (e.g. POST createExternalProjectTeamRole)
...
/// Match `/api/v1/workspaces/{wsId}/external-projects/members/roles` exactly and
/// extract `wsId`. Returns None when the shape does not match (including the
/// longer `.../roles/{roleId}/members` route, which has trailing segments).
...
async fn roles_response(
    ...
    // binding, mirroring workspaces_external_projects_members_roles_roleid_members.rs.
    ...
    match fetch_roles(contact_data, outbound, &resolved_ws_id).await {
        Ok(roles) => no_store_response(json_response(200, Value::Array(roles))),
        Err(()) => message_response(500, LOAD_ERROR_MESSAGE),
    }
}

// ---------------------------------------------------------------------------
// Role listing (mirrors listExternalProjectTeamRoles)
// ---------------------------------------------------------------------------
...
async fn fetch_roles(
    ...
    // ({ id, name, permissions: [{ id, enabled }], created_at }); pass it through
    // unchanged to mirror `data ?? []`.
