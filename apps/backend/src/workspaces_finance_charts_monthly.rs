...
const MONTHLY_CHART_ERROR_MESSAGE: &str = "Internal server error while fetching monthly chart data";
const MONTHLY_CHART_INVALID_QUERY_MESSAGE: &str = "Invalid query parameters";
const MONTHLY_CHART_PATH_PREFIX: &str = "/api/workspaces/";
const MONTHLY_CHART_PATH_SUFFIX: &str = "/finance/charts/monthly";
const GET_MONTHLY_INCOME_EXPENSE_RANGE_RPC: &str = "get_monthly_income_expense_range";
...
const MAX_FINANCE_MONTHLY_DATE_RANGE_DAYS: i64 = 3660;
...
#[derive(Serialize)]
struct MonthlyChartRpcRequest<'a> {
...
pub(crate) async fn handle_workspaces_finance_charts_monthly_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let ws_id = monthly_chart_ws_id(request.path)?;

    Some(match request.method {
        "GET" => monthly_chart_response(config, request, ws_id, outbound).await,
        method => no_store_response(method_not_allowed(method, "GET")),
    })
}

async fn monthly_chart_response(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    raw_ws_id: &str,
    outbound: &impl OutboundHttpClient,
    ...
    let query = match monthly_chart_query_from_url(request.url) {
        Some(query) => query,
        None => return message_response(400, MONTHLY_CHART_INVALID_QUERY_MESSAGE),
    };
    ...
    .await
    {
        ...
        Err(FinanceAuthorizationError::Internal) => {
            return message_response(500, MONTHLY_CHART_ERROR_MESSAGE);
        }
    };

    match fetch_monthly_chart(&config.contact_data, outbound, &authorization, &query).await {
        Ok(data) => no_store_response(json_response(200, json!({ "data": data }))),
        Err(()) => message_response(500, MONTHLY_CHART_ERROR_MESSAGE),
    }
}

async fn fetch_monthly_chart(
    ...
    let rpc_url = contact_data
        .rpc_url(GET_MONTHLY_INCOME_EXPENSE_RANGE_RPC)
        .ok_or(())?;
    ...
    let body = serde_json::to_string(&MonthlyChartRpcRequest {
...
fn monthly_chart_query_from_url(request_url: Option<&str>) -> Option<MonthlyChartQuery> {
...
/// `apps/web/.../finance/date-range.ts` with the monthly default `maxDays`.
///
...
fn validate_finance_date_range(
    ...
    if range_days > MAX_FINANCE_MONTHLY_DATE_RANGE_DAYS {
...
fn monthly_chart_ws_id(path: &str) -> Option<&str> {
    let ws_id = path
        .strip_prefix(MONTHLY_CHART_PATH_PREFIX)?
        .strip_suffix(MONTHLY_CHART_PATH_SUFFIX)?;
