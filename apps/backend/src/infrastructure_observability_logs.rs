//! Port of `GET /api/v1/infrastructure/observability/logs`.
//!
//! Legacy source:
//! `apps/web/src/app/api/v1/infrastructure/observability/logs/route.ts`.
//!
...
//! 2. Calls `readObservabilityLogs(filters)` from
//!    `apps/web/src/lib/infrastructure/observability.ts`.
//! 3. Returns `NextResponse.json(result)` (no explicit cache headers).
//!
//! `readObservabilityLogs` aggregates:
//!
//! - `loadRecentLogs` — reads from a local PostgreSQL log-drain database or
//!   host filesystem (`tmp/docker-web`), both unavailable in the Worker.
//! - `groupObservabilityLogs` — groups raw events by request id.
//! - `paginate` — paginates the grouped items.
//! - `createLogFacets` — computes facets (levels, routes, sources, statuses,
//!   users) across all raw log events.
//!
//! A Cloudflare Worker has no local filesystem or log-drain SQL client, so
//! `loadRecentLogs` always returns an empty array, `groupObservabilityLogs`
//! produces `[]`, and `createLogFacets([])` produces all-empty facet arrays.
//! This handler reproduces that exact empty shape:
//!
//! ```json
//! {
//!   "hasNextPage": false,
...
//! BEHAVIOR GAPS:
//!
//! - Data gap: real log records from the log-drain Postgres database or the
//!   filesystem-based archive are never returned. The handler always produces
//!   an empty list with empty facets.
...
pub(crate) const INFRASTRUCTURE_OBSERVABILITY_LOGS_PATH: &str =
    "/api/v1/infrastructure/observability/logs";
...
pub(crate) async fn handle_infrastructure_observability_logs_route(
    config: &crate::BackendConfig,
    request: crate::BackendRequest<'_>,
    outbound: &impl crate::outbound::OutboundHttpClient,
) -> Option<crate::BackendResponse> {
    if request.path != INFRASTRUCTURE_OBSERVABILITY_LOGS_PATH {
        return None;
    }

    Some(match request.method {
        "GET" => logs_response(config, request, outbound).await,
        _ => return None,
    })
}
...
async fn logs_response(
    ...
    .await
    {
        ...
        Err(WorkspacePermissionAuthorizationError::Internal) => {
            return no_store_response(json_response(
                500,
                json!({ "message": "Failed to load observability logs" }),
            ));
        }
    }
    ...
    // The Worker has no SQL client and no host filesystem, so the log archive
    // is always empty. Mirror the empty shape from readObservabilityLogs:
    // paginate(groupObservabilityLogs([]), ...) + createLogFacets([]).
...
mod tests {
    ...
    fn path_guard_matches_exact_path() {
        assert_eq!(
            INFRASTRUCTURE_OBSERVABILITY_LOGS_PATH,
            "/api/v1/infrastructure/observability/logs"
        );
    }
    ...
    fn path_guard_does_not_match_prefix() {
        let path = "/api/v1/infrastructure/observability/logs/extra";
        assert_ne!(path, INFRASTRUCTURE_OBSERVABILITY_LOGS_PATH);
    }
    ...
    fn path_guard_does_not_match_with_trailing_slash() {
        let path = "/api/v1/infrastructure/observability/logs/";
        assert_ne!(path, INFRASTRUCTURE_OBSERVABILITY_LOGS_PATH);
    }
    ...
    fn empty_response_shape_has_expected_keys() {
        ...
        assert!(facets["levels"].as_array().unwrap().is_empty());
        assert!(facets["routes"].as_array().unwrap().is_empty());
        assert!(facets["sources"].as_array().unwrap().is_empty());
        assert!(facets["statuses"].as_array().unwrap().is_empty());
        assert!(facets["users"].as_array().unwrap().is_empty());
    }
}
