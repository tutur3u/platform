...
const PATH_SUFFIX: &str = "/users/count";
const COUNT_TABLE: &str = "workspace_users";
...
pub(crate) async fn handle_workspaces_users_count_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let raw_ws_id = users_count_ws_id(request.path)?;

    Some(match request.method {
        "GET" => users_count_response(config, request, raw_ws_id, outbound).await,
        method => no_store_response(method_not_allowed(method, "GET")),
    })
}

async fn users_count_response(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    raw_ws_id: &str,
    outbound: &impl OutboundHttpClient,
...
fn users_count_ws_id(path: &str) -> Option<&str> {
