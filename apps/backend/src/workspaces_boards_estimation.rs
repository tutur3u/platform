...
const WORKSPACE_BOARDS_ESTIMATION_PATH_PREFIX: &str = "/api/v1/workspaces/";
const WORKSPACE_BOARDS_ESTIMATION_PATH_SUFFIX: &str = "/boards/estimation";

#[derive(Serialize, Deserialize)]
struct EstimationBoardRow {
    id: Option<String>,
    name: Option<String>,
    estimation_type: Option<String>,
    extended_estimation: Option<bool>,
    allow_zero_estimates: Option<bool>,
    count_unestimated_issues: Option<bool>,
...
pub(crate) async fn handle_workspaces_boards_estimation_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let ws_id = workspaces_boards_estimation_ws_id(request.path)?;

    Some(match request.method {
        "GET" => estimation_boards_response(config, request, ws_id, outbound).await,
        method => no_store_response(method_not_allowed(method, "GET")),
    })
}

async fn estimation_boards_response(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    raw_ws_id: &str,
    outbound: &impl OutboundHttpClient,
    ...
    match fetch_estimation_boards(&config.contact_data, outbound, &resolved_ws_id).await {
        Ok(boards) => no_store_response(json_response(200, json!({ "boards": boards }))),
        Err(()) => message_response(500, FETCH_BOARDS_FAILED_MESSAGE),
    }
}

async fn fetch_estimation_boards(
    contact_data: &contact::ContactDataConfig,
    outbound: &impl OutboundHttpClient,
    ws_id: &str,
) -> Result<Vec<EstimationBoardRow>, ()> {
    ...
    response.json::<Vec<EstimationBoardRow>>().map_err(|_| ())
}
...
fn workspaces_boards_estimation_ws_id(path: &str) -> Option<&str> {
    let ws_id = path
        .strip_prefix(WORKSPACE_BOARDS_ESTIMATION_PATH_PREFIX)?
        .strip_suffix(WORKSPACE_BOARDS_ESTIMATION_PATH_SUFFIX)?;
