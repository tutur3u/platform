...
const WORKSPACES_INVENTORY_ACCESS_PATH_PREFIX: &str = "/api/v1/workspaces/";
const WORKSPACES_INVENTORY_ACCESS_PATH_SUFFIX: &str = "/inventory/access";

// Union of every permission that toggles `enabled` in the legacy route. This is
// the flattened set of all permission ids referenced by the inventory access
// permission helpers (`canViewInventoryDashboard`, `canViewInventoryCatalog`,
// `canManageInventorySetup`, `canViewInventoryStock`, `canViewInventorySales`,
// `canViewInventoryAnalytics`, `canViewInventoryAuditLogs`). `admin` is handled
// via the `has_all_permissions` shortcut, but is also listed here because the
// legacy audit-log helper checks for it explicitly.
const INVENTORY_ACCESS_PERMISSIONS: [&str; 19] = [
    // dashboard
...
#[derive(Serialize)]
struct WorkspacesInventoryAccessResponse {
    enabled: bool,
}
...
pub(crate) async fn handle_workspaces_inventory_access_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let raw_ws_id = workspaces_inventory_access_ws_id(request.path)?;

    Some(match request.method {
        "GET" => workspaces_inventory_access_response(config, request, raw_ws_id, outbound).await,
        method => no_store_response(method_not_allowed(method, "GET")),
    })
}

async fn workspaces_inventory_access_response(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    raw_ws_id: &str,
    outbound: &impl OutboundHttpClient,
    ...
    let enabled = permissions.has_all_permissions
        || permissions
            .permissions
            .iter()
            .any(|value| INVENTORY_ACCESS_PERMISSIONS.contains(&value.as_str()));

    no_store_response(json_response(
        200,
        WorkspacesInventoryAccessResponse { enabled },
    ))
}
...
fn workspaces_inventory_access_ws_id(path: &str) -> Option<&str> {
    let ws_id = path
        .strip_prefix(WORKSPACES_INVENTORY_ACCESS_PATH_PREFIX)?
        .strip_suffix(WORKSPACES_INVENTORY_ACCESS_PATH_SUFFIX)?;
