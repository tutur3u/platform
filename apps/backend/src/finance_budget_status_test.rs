...
const BUDGET_STATUS_PATH: &str = "/api/v1/workspaces/ws-1/finance/budgets/status";
const BUDGET_STATUS_URL: &str =
    "https://tuturuuu.localhost/api/v1/workspaces/ws-1/finance/budgets/status";
...
fn app_session_token(target_app: &str, scopes: Vec<String>) -> String {
    ...
    let payload = contact::encode_app_session_part(
        serde_json::to_string(&contact::AppCoordinationClaims {
            ...
            jti: "finance-budget-status-test".to_owned(),
...
fn successful_browser_responses(budget_status: &'static str) -> Vec<OutboundResponse> {
    vec![
        outbound_response(200, r#"{"id":"user-1"}"#),
        outbound_response(200, r#"[{"id":"resolved-ws"}]"#),
        outbound_response(200, "true"),
        outbound_response(200, budget_status),
    ]
}

#[tokio::test]
async fn finance_budget_status_uses_normalized_workspace_and_admin_rpc() {
    let outbound = RecordingOutboundClient::with_responses(successful_browser_responses(
        r#"[{"budget_id":"budget-1","percentage_used":95}]"#,
    ));
    ...
    assert_eq!(
        response.body,
        json!([{ "budget_id": "budget-1", "percentage_used": 95 }])
    );
    ...
    assert!(calls[3].url.ends_with("/rest/v1/rpc/get_budget_status"));
    assert_eq!(
        calls[3].body.as_deref(),
        Some(r#"{"_ws_id":"resolved-ws"}"#)
    );
}

#[tokio::test]
async fn finance_budget_status_accepts_finance_app_session_tokens() {
    ...
    assert!(calls[0].url.contains("handle=eq.ws-1"));
    assert_eq!(
        recorded_header(&calls[0], "Authorization"),
        Some("Bearer test-service-role-secret")
    );
    assert_eq!(
        ...
        Some(
            r#"{"p_permission":"manage_finance","p_user_id":"finance-user-1","p_ws_id":"personal-ws"}"#
        )
    );
}

#[tokio::test]
async fn finance_budget_status_rejects_missing_manage_finance() {
    let outbound = RecordingOutboundClient::with_responses(vec![
        outbound_response(200, r#"{"id":"user-1"}"#),
        outbound_response(200, r#"[{"id":"resolved-ws"}]"#),
...
#[tokio::test]
async fn finance_budget_status_preserves_rpc_failure_body() {
    let outbound = RecordingOutboundClient::with_responses(successful_browser_responses(
        r#"{"message":"not an array but still valid json"}"#,
    ));
    let response = handle_backend_request(
        &backend_config_with_contact_data(),
        request_with_bearer("Bearer browser-access-token"),
    ...
    let rpc_failure_outbound = RecordingOutboundClient::with_responses(vec![
        outbound_response(200, r#"{"id":"user-1"}"#),
        outbound_response(200, r#"[{"id":"resolved-ws"}]"#),
        outbound_response(200, "true"),
        outbound_response(500, r#"{"message":"failed"}"#),
    ]);
    let rpc_failure_response = handle_backend_request(
        &backend_config_with_contact_data(),
        request_with_bearer("Bearer browser-access-token"),
        &rpc_failure_outbound,
    )
    .await;

    assert_eq!(rpc_failure_response.status, 500);
    assert_eq!(
        rpc_failure_response.body,
        json!({ "message": "Error fetching budget status" })
    );
}
