//! Handler for `/api/v1/workspaces/:wsId/chat/channels/:channelId/participants`.
//!
//! Ports the **GET** method of the legacy Next.js route
//! `apps/web/src/app/api/v1/workspaces/[wsId]/chat/channels/[channelId]/participants/route.ts`.
//!
...
//!    `private.chat_get_conversation` RPC via
//!    `chat_assert_workspace_permission(..., 'view_chat')`, which raises with
//!    errcode `42501` -> mapped to 403.
//! 3. `callPrivateChatRpc('chat_get_conversation', { p_actor_user_id,
//!    p_conversation_id, p_ws_id })`. The RPC is `security definer` and lives in
...
//! 4. Success: `{ participants: conversation ? toLegacyParticipant(conversation)
//!    : [] }`. When the RPC returns `null` (conversation missing/archived) the
//!    participants list is `[]`. `toLegacyParticipant` maps each member to
//!    `{ channel_id: conversationId, last_read_at: lastReadAt, user_id: userId }`.
//! 5. RPC error: `chatRpcErrorResponse(error, 'Failed to load participants')`
...
const LOAD_PARTICIPANTS_FAILED_MESSAGE: &str = "Failed to load participants";
const NORMALIZE_FAILED_MESSAGE: &str = "Failed to load participants";
...
pub(crate) async fn handle_workspaces_chat_channels_channelid_participants_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let (raw_ws_id, channel_id) = parse_participants_path(request.path)?;

    Some(match request.method {
        "GET" => participants_response(config, request, raw_ws_id, channel_id, outbound).await,
...
async fn participants_response(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    raw_ws_id: &str,
    channel_id: &str,
    ...
    {
        Ok(ws_id) => ws_id,
        Err(()) => return message_response(500, NORMALIZE_FAILED_MESSAGE),
    };

    // Legacy: `callPrivateChatRpc('chat_get_conversation', { ... })`. The RPC
    // enforces `view_chat` and conversation access internally.
    match call_chat_get_conversation(
        &config.contact_data,
        outbound,
        &resolved_ws_id,
        channel_id,
    ...
    {
        Ok(conversation) => {
            let participants = to_legacy_participants(&conversation);
            no_store_response(json_response(200, json!({ "participants": participants })))
        }
        Err(rpc_error) => chat_rpc_error_response(rpc_error),
    }
}

/// Maps the conversation JSON's `members` array to the legacy participant shape:
/// `{ channel_id: conversationId, last_read_at: lastReadAt, user_id: userId }`.
/// A `null` conversation (missing/archived) yields an empty list.
fn to_legacy_participants(conversation: &Value) -> Vec<Value> {
    let Some(members) = conversation.get("members").and_then(Value::as_array) else {
        return Vec::new();
    };
    ...
    members
        ...
        .map(|member| {
            json!({
                "channel_id": member.get("conversationId").cloned().unwrap_or(Value::Null),
                "last_read_at": member.get("lastReadAt").cloned().unwrap_or(Value::Null),
                "user_id": member.get("userId").cloned().unwrap_or(Value::Null),
            })
        })
        .collect()
}
...
/// Mirrors `callPrivateChatRpc<ChatConversation | null>('chat_get_conversation',
...
/// `security definer`). Returns the conversation JSON (`Value::Null` when the
...
async fn call_chat_get_conversation(
    ...
    let Some(url) = contact_data.rpc_url(CHAT_GET_CONVERSATION_RPC) else {
        return Err(RpcError {
            code: None,
            message: None,
        });
    ...
    if (200..300).contains(&response.status) {
        // The RPC returns `jsonb` (the conversation object, or null).
...
/// Mirrors `chatRpcErrorResponse(error, 'Failed to load participants')`. Returns
...
fn chat_rpc_error_response(error: RpcError) -> BackendResponse {
    ...
    let message = if status >= 500 {
        LOAD_PARTICIPANTS_FAILED_MESSAGE.to_owned()
    } else {
        match error.message.as_deref() {
            Some(message) if !message.is_empty() => message.to_owned(),
            _ => LOAD_PARTICIPANTS_FAILED_MESSAGE.to_owned(),
        }
    };
...
/// Matches `/api/v1/workspaces/:wsId/chat/channels/:channelId/participants` and
/// returns `(wsId, channelId)` when the shape matches.
fn parse_participants_path(path: &str) -> Option<(&str, &str)> {
