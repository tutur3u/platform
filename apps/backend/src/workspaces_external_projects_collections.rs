//! Port of the legacy `GET /api/v1/workspaces/:wsId/external-projects/collections`
//! route (list collections only).
//!
//! Legacy source:
//! `apps/web/src/app/api/v1/workspaces/[wsId]/external-projects/collections/route.ts`
//!
//! The legacy GET handler composes:
//!   - `requireWorkspaceExternalProjectAccess({ mode: 'read', ... })` (access.ts)
//!   - `listWorkspaceExternalProjectCollections(...)` (store.ts) which selects
//!     `*` from `workspace_external_project_collections` for the resolved
//!     workspace ordered by `title` ascending.
//!
//! and returns the raw array of collection rows (`NextResponse.json(collections)`,
//! status 200). On a store failure it returns
//! `{ error: 'Failed to list workspace external project collections' }` (500).
//!
//! Only the **GET** method is migrated here. The legacy route also defines a
//! POST (create collection) method that is NOT ported yet; for every other
//! method this handler returns `None` so the Cloudflare worker falls through to
//! the still-active Next.js route (rather than 405-ing a valid mutation).
//!
//! Auth note: this port mirrors the *Supabase user session* branch only (cookie
//! or bearer), matching the sibling `workspaces_external_projects` and
//! `workspace_external_projects_summary` handlers. The legacy access helper also
//! supports app-coordination tokens and app-session (`cms`) tokens; those flows
...
//! access/normalization helpers it needs are private `fn`s in the sibling
//! `workspaces_external_projects` module and in `workspace_habits_access`, so
//! (per the one-file constraint) the relevant logic is copied here as file-local
//! `fn`s instead of editing those modules. The public crate helpers
//! (`supabase_auth::*`, `contact::ContactDataConfig::{configured, rest_url,
//! service_role_key}`, `json_response`, `method_not_allowed`, `no_store_response`)
//! are reused directly.
...
const FAILED_MESSAGE: &str = "Failed to list workspace external project collections";

const PATH_PREFIX: &str = "/api/v1/workspaces/";
const PATH_SUFFIX: &str = "/external-projects/collections";
...
pub(crate) async fn handle_workspaces_external_projects_collections_route(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    outbound: &impl OutboundHttpClient,
) -> Option<BackendResponse> {
    let raw_ws_id = collections_ws_id(request.path)?;

    Some(match request.method {
        "GET" => list_collections_response(config, request, raw_ws_id, outbound).await,
...
async fn list_collections_response(
    config: &BackendConfig,
    request: BackendRequest<'_>,
    raw_ws_id: &str,
    outbound: &impl OutboundHttpClient,
    ...
    // listWorkspaceExternalProjectCollections: select('*') filtered by ws_id,
    // ordered by title ascending. Returns the raw array verbatim.
    let collections = match list_collections(contact_data, outbound, &resolved_ws_id).await {
        Ok(rows) => rows,
        Err(()) => return error_response(500, FAILED_MESSAGE),
    };

    json_response(200, Value::Array(collections))
}
...
fn collections_ws_id(path: &str) -> Option<&str> {
    let ws_id = path.strip_prefix(PATH_PREFIX)?.strip_suffix(PATH_SUFFIX)?;
    // ws_id must be a single path segment so we only match the exact
    // `/external-projects/collections` shape (not deeper subpaths).
