    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full border-border/70 bg-background/98 p-0 sm:max-w-[min(92vw,980px)]">
        ...
        <div className="space-y-5 px-6 py-5">
            ...
            <div className="overflow-hidden rounded-[1.6rem] border border-border/70 bg-card/95">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-8">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="outline">
                          {strings.slugLabel}: {previewEntry.slug}
                        </Badge>
                      </div>
              ...
              <div className="grid gap-3 p-5 sm:grid-cols-3">
                <div className="rounded-[1.1rem] border border-border/70 bg-background/70 p-4">
                  ...
                  <div className="mt-2 font-medium">{previewEntry.title}</div>
                </div>
                <div className="rounded-[1.1rem] border border-border/70 bg-background/70 p-4">
                  <div className="text-[11px] text-muted-foreground uppercase tracking-[0.24em]">
                    {strings.slugLabel}
                  </div>
                  <div className="mt-2 font-medium">{previewEntry.slug}</div>
                </div>
                <div className="rounded-[1.1rem] border border-border/70 bg-background/70 p-4">
                  <div className="text-[11px] text-muted-foreground uppercase tracking-[0.24em]">
                  ...
                  <div className="mt-2 text-muted-foreground text-sm leading-6">
                    {getEntryDescriptionMarkdown(previewEntry.summary, '') ||
                      previewEntry.subtitle ||
                      strings.previewEmptyDescription}
                  </div>
                </div>
          ...
          {!previewPending && previewEntry ? (
            <Accordion type="single" collapsible>
              <AccordionItem
                value="developer-details"
                className="rounded-[1.2rem] border border-border/70 bg-card/70 px-4"
              >
                <AccordionTrigger>{strings.payloadLabel}</AccordionTrigger>
                <AccordionContent>
                  <pre className="max-h-[48svh] overflow-auto rounded-xl border border-border/70 bg-background/80 p-4 text-xs leading-6">
