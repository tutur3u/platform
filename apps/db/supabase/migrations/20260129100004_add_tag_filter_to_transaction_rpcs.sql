-- Migration: add_tag_filter_to_transaction_rpcs
-- Adds p_tag_ids parameter to get_transaction_stats and get_wallet_transactions_with_permissions

-- ============================================
-- Update get_transaction_stats
-- ============================================
DROP FUNCTION IF EXISTS public.get_transaction_stats(uuid, uuid, uuid[], uuid[], uuid[], text, timestamp with time zone, timestamp with time zone);
...
BEGIN
  ...
  WITH filtered_data AS (
    ...
    WHERE ww.ws_id = p_ws_id
      ...
      -- Tag filter: if p_tag_ids is provided, only include transactions that have at least one of the specified tags
...
GRANT EXECUTE ON FUNCTION public.get_transaction_stats(uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamp with time zone, timestamp with time zone) TO authenticated;

-- ============================================
-- Update get_wallet_transactions_with_permissions
-- ============================================
...
BEGIN
  ...
  IF p_include_count THEN
    ...
    USING
      ...
      p_end_date,                   -- $12
      p_search_query,               -- $13
      p_cursor_taken_at,            -- $14
      p_cursor_created_at,          -- $15
      p_limit,                      -- $16
      p_offset,                     -- $17
      has_manage_finance,           -- $18
      v_wallet_windows,             -- $19
      can_view_expenses,            -- $20
      can_view_transactions,        -- $21
      has_granular_permissions,     -- $22
      can_view_incomes;             -- $23
  END IF;
