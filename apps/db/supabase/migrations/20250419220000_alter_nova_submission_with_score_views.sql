...
SELECT 
  ...
  tc.total_tests,
  tc.passed_tests,
  (COALESCE(tc.passed_tests, 0)::float / NULLIF(tc.total_tests, 0)::float) * 10 AS test_case_score,

  cr.total_criteria,
  cr.sum_criterion_score,
  (COALESCE(cr.sum_criterion_score, 0)::float / (10 * NULLIF(cr.total_criteria, 0)::float)) * 10 AS criteria_score,

  ((COALESCE(tc.passed_tests, 0)::float / NULLIF(tc.total_tests, 0)::float) * 10 * 0.5) + 
  ((COALESCE(cr.sum_criterion_score, 0)::float / (10 * NULLIF(cr.total_criteria, 0)::float)) * 10 * 0.5) AS total_score
