-- Migration: Fix invoice analytics user grouping - use workspace_user_linked_users table
-- The previous migration incorrectly referenced wu.user_id which doesn't exist on workspace_users.
-- The correct approach is to join workspace_user_linked_users to resolve workspace user -> platform user.
...
BEGIN
  ...
  IF group_by_creator THEN
    ...
    relevant_creators AS (
      ...
      -- Join through workspace_user_linked_users to get the platform user for workspace users
      SELECT DISTINCT 
