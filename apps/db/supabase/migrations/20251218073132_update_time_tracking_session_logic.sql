-- Migration: Fix threshold bypass when pausing/stopping sessions
-- This migration updates the time tracking session update trigger to validate
-- when a session is being completed (is_running changing from true to false)
-- against the workspace threshold settings.
...
3. Validates when is_running changes from true to false (completing/pausing session) against threshold
4. Otherwise enforces missed_entry_date_threshold from workspace_settings for time field edits';
