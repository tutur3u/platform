-- Migration: Add _wallet_ids filter to get_category_breakdown
-- Allows filtering category breakdown by specific wallet(s),
-- so wallet detail pages show only that wallet's categories.
...
CREATE OR REPLACE FUNCTION public.get_category_breakdown(
  ...
  _wallet_ids UUID[] DEFAULT NULL  -- Optional wallet filter: NULL = all wallets
)
RETURNS TABLE (
  period DATE,
  category_id UUID,
...
-- Drop old 8-parameter signature and grant on new 9-parameter signature
DROP FUNCTION IF EXISTS public.get_category_breakdown(UUID, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN, TEXT, TEXT, BOOLEAN, TEXT);

GRANT EXECUTE ON FUNCTION public.get_category_breakdown(UUID, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN, TEXT, TEXT, BOOLEAN, TEXT, UUID[]) TO authenticated;

COMMENT ON FUNCTION public.get_category_breakdown(UUID, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN, TEXT, TEXT, BOOLEAN, TEXT, UUID[]) IS
  'Returns category breakdown by specified interval with timezone support and optional wallet filtering. transaction_type can be "expense", "income", or "all". interval can be "daily", "weekly", "monthly", or "yearly". When anchor_to_latest is true, uses most recent transaction as the end date anchor. timezone parameter specifies the IANA timezone for date grouping (default: UTC). wallet_ids filters to specific wallets (default: NULL = all wallets).';
