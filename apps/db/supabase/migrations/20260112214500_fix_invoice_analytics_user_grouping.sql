-- Migration: Fix invoice analytics user grouping
-- Updates logic to merge workspace users with their linked platform users
-- to prevent duplicate entries in analytics charts (e.g. same person showing twice)
...
BEGIN
  ...
  IF group_by_creator THEN
    ...
    relevant_creators AS (
      ...
      SELECT DISTINCT 
        COALESCE(fi.platform_creator_id, wu.user_id, fi.creator_id) AS creator_id
      FROM finance_invoices fi
      LEFT JOIN workspace_users wu ON fi.creator_id = wu.id
      WHERE fi.ws_id = _ws_id
        AND (user_ids IS NULL OR array_length(user_ids, 1) IS NULL 
             OR COALESCE(fi.platform_creator_id, wu.user_id, fi.creator_id) = ANY(user_ids))
    ),
    creator_info AS (
      -- Get creator details. IDs in relevant_creators are now mostly Platform User IDs (from users table)
      -- but could be Workspace User IDs (from workspace_users table) if unlinked.
      SELECT 
        rc.creator_id,
        COALESCE(
          u.display_name,
  ...
  ELSE
    -- Group by wallet (original behavior with updated user filtering)
    RETURN QUERY
    WITH date_series AS (
      SELECT 
        CASE _interval_type
