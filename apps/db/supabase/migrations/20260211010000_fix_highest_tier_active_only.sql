-- Fix highest_tier in get_workspace_overview to only consider active/trialing subscriptions.
-- Previously, MAX(tier_rank) had no status filter, so canceled subscriptions
-- (e.g. a past Pro sub) would still show as the workspace's tier.
    ...
    SELECT
      ...
      MAX(
        CASE p.tier
          ...
          ELSE 0
        END
      ) FILTER (WHERE ws_sub.status IN ('active', 'trialing')) AS max_tier_rank
