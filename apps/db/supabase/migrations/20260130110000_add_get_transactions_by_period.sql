-- Migration: add_get_transactions_by_period
-- Adds RPC function for period-based transaction grouping (day/week/month/year)
...
BEGIN
  ...
  WITH filtered_transactions AS (
    SELECT
      ...
      date_trunc(v_trunc_interval, wt.taken_at) as period_bucket
  ...
  period_aggregates AS (
    SELECT
      ft.period_bucket,
      ft.period_bucket + (v_interval_text)::interval as period_end_calc,
      COUNT(*) as tx_count,
  ...
  paginated_periods AS (
    ...
    WHERE p_cursor_period_start IS NULL OR pa.period_bucket < p_cursor_period_start
    ORDER BY pa.period_bucket DESC
    LIMIT p_limit + 1
  )
  SELECT
    pp.period_bucket as period_start,
...
COMMENT ON FUNCTION public.get_transactions_by_period IS
'Returns transactions grouped by period (day/week/month/year) with aggregated statistics.
Supports all filters from get_wallet_transactions_with_permissions.
