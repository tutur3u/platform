-- Update get_workspace_users RPC function to add include_archived parameter
-- This allows filtering out archived users by default while still providing option to include them
-- 
-- IMPORTANT FIX: Uses COALESCE to handle NULL full_name and display_name values properly.
-- Without COALESCE, users with NULL names would be excluded when search_query is empty string,
-- because NULL ILIKE '%%' returns NULL (not TRUE), filtering out those users.

-- Drop existing function
DROP FUNCTION IF EXISTS get_workspace_users(UUID, UUID[], UUID[], TEXT);

-- Recreate function with include_archived parameter
