...
export async function authorizeRequest(
  ...
  const { user, authError } = await resolveAuthenticatedUser(request, supabase);
...
export async function authorize(
  ...
  const { user, authError: userError } = await resolveAuthenticatedUser(
    null,
    supabase
  );
...
async function resolveAuthenticatedUser(
  ...
  const resolution = request
    ? await resolveAuthenticatedSessionUser(request, supabase)
...
export function withSessionAuth<T = unknown>(
  ...
  return async (
    ...
    const supabase = (await createClient(request)) as TypedSupabaseClient;
    const { user, authError } = await resolveAuthenticatedUser(
      request,
      supabase
    );
