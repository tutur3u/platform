...
const TASK_HABITS_APP_SESSION_AUTH = {
  targetApp: [CLI_APP_TARGET_APP, 'tasks'],
} as const;

export const GET = withSessionAuth<RouteParams>(
  async (request: NextRequest, { supabase, user }, { wsId }) => {
    try {
  ...
  { allowAppSessionAuth: TASK_HABITS_APP_SESSION_AUTH }
);
    ...
    try {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'Min instances per day cannot exceed ideal instances per day',
          },
          { status: 400 }
        );
      }
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'Ideal instances per day cannot exceed max instances per day',
          },
          { status: 400 }
        );
      }
