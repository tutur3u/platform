...
const TASK_SCHEDULE_APP_SESSION_AUTH = {
  targetApp: ['calendar', 'tasks'],
} as const;

export const POST = withSessionAuth<ScheduleParams>(
  async (_request: NextRequest, { supabase, user }, { wsId, taskId }) => {
    try {
      // Validate UUIDs
  ...
  { allowAppSessionAuth: TASK_SCHEDULE_APP_SESSION_AUTH }
);

export const GET = withSessionAuth<ScheduleParams>(
  async (_request: NextRequest, { supabase, user }, { wsId, taskId }) => {
    try {
      // Validate UUIDs
      if (!validate(wsId) || !validate(taskId)) {
