...
export const GET = withSessionAuth<Params>(
  async (_request, auth, { draftId, wsId: rawWsId }) => {
    try {
      const wsId = await normalizeWorkspaceId(rawWsId, auth.supabase);
      const result = await verifyAccess(wsId, draftId, auth);
...
export const PUT = withSessionAuth<Params>(
  async (request: NextRequest, auth, { draftId, wsId: rawWsId }) => {
    try {
      const wsId = await normalizeWorkspaceId(rawWsId, auth.supabase);
      const result = await verifyAccess(wsId, draftId, auth);
      ...
      if (parsed.label_ids !== undefined)
        updateData.label_ids = parsed.label_ids;
