  ...
  group('TimeTrackerCubit history behavior', () {
    ...
    test('changes view mode and loads initial history', () async {
      ...
      when(
        ...
        (_) async =>
            const TimeTrackingPeriodStats(totalDuration: 1200, sessionCount: 2),
      );
    ...
    test('toggleHistoryStatsAccordion persists preference', () async {
      ...
      expect(prefs.getBool('time_tracker_history_stats_open'), isTrue);
    });
    ...
    test(
      ...
      () async {
        ...
        when(
          () => repository.getSessions(any(), limit: any(named: 'limit')),
    ...
    test('loadData clears cached history while a new history context refresh '
        'is in flight', () async {
      when(
        () => repository.getRunningSession(any()),
      ).thenAnswer((_) async => null);
      ...
      when(
        () =>
            repository.getStats(any(), any(), timezone: any(named: 'timezone')),
      ).thenAnswer((_) async => const TimeTrackerStats());
      ...
      expect(cubit.state.historySessions.map((session) => session.id), [
