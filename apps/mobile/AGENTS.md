...
- **Adaptive Drawer Test Fallback**: In compact-mode widget tests, `shad.openDrawer` can assert when `DrawerOverlay` is absent from the harness. Keep `showAdaptiveDrawer` resilient by checking for `DrawerOverlay` first and falling back to `showModalBottomSheet` in debug/test environments.
