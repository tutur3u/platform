...
it('binds the encrypted event to the enabled primary calendar before inserting UTC times', async () => {
  const calendar = {
  ...
  const save = await prepareCalendarFollowup(db as never, 'workspace', payload);
  expect(db.schema).toHaveBeenCalledWith('private');
  expect(calendar.eq).toHaveBeenCalledWith('ws_id', 'workspace');
  expect(calendar.eq).toHaveBeenCalledWith('calendar_type', 'primary');
