...
export async function GET(req: Request) {
  ...
  const result = await batchFetch({
    table: 'workspace_user_group_session_files',
    wsId,
    offset,
    limit,
    schema: 'private',
  });
  return createFetchResponse(result, 'workspace-user-group-session-files');
}
...
export async function PUT(req: Request) {
  ...
  return createMigrationResponse(result, 'workspace-user-group-session-files');
}
