...
export function WorkspacePolicyEditor({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <div className="flex items-start justify-between gap-4 rounded-lg border bg-background p-3 md:col-span-2 xl:col-span-4">
        <div>
          ...
          {policy.apiKeyCreationDecidedAt ? (
            <p className="mt-1 text-muted-foreground text-xs">
              {t('workspaces.api_key_decided_at', {
                date: new Date(policy.apiKeyCreationDecidedAt).toLocaleString(),
              })}
            </p>
          ) : null}
        </div>
      ...
      <div className="space-y-1.5">
        ...
        <Select
          onValueChange={(value) =>
            onChange({
              captureEnabled: value === 'inherit' ? null : value === 'enabled',
            })
          }
          value={
      ...
      <NumberPolicyField
        ...
        max={365}
        min={1}
        onChange={(contentRetentionDays) => onChange({ contentRetentionDays })}
        placeholder={t('workspaces.inherit')}
