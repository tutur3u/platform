...
- **Document product decisions** – Record customer-facing rules in the relevant platform feature page with scope, default, affected apps, and exceptions; check implementation and documentation across live and migration paths before delivery.
- **Authorized completion** – Continue the requested implementation, relevant checks, fixes, and delivery. Ask only for a missing decision, permission, or access.
...
| **Dependencies** | `cd <app-or-package> && bun add <pkg>` / remove deps | Prefer `workspace:*` for internal packages | Add duplicate versions already satisfied |

## Mandatory Guardrails

1. Finite local setup, tests, and builds needed to validate authorized work may run without repeated confirmation. Start long-lived servers only when requested or needed for explicitly requested runtime verification. Deployment authorization remains separate.
...
Ask when a concrete action exceeds the requested scope, needs missing access, or
requires an unresolved product decision. File counts and SQL line counts alone do
not require approval. Prepare the reviewable change before requesting approval for
an external action. Preserve existing authorization for routine retries and fixes;
never infer permission for broader production scope. Fetch before production
sync and inspect the promotion range: if main advanced with unrelated commits,
use the supported pinned-SHA path or obtain authorization for the expanded range.

Read only the focused skill and reference sections relevant to the task. Keep
AGENTS.md constraints concise; preserve detailed app lessons in skill references.
Define completion from the request and carry authorized work through the required
checks and fixes. A read-only CLI operation needs its diagnostic procedure, not
the implementation test suite; a local UI change needs its owning surface, not
a full application audit. See [focused agent guidance](/build/development-tools/codex-plugin#maintaining-focused-agent-guidance).

## Canonical Workflows (Cheat Sheet)

- **Add/Update dependency** – scope → `cd <app-or-package> && bun add <pkg>` → ensure types/docs → run targeted build.
...
- **Supabase migration** – run `bun sb:new`, author additive SQL, apply locally and regenerate types via `bun sb:typegen`; production pushes remain user-only, commit regenerated files together.

See `plugins/tuturuuu/skills/tuturuuu-platform/references/repository-workflows.md`
for task-specific commands and route ownership.
...
- For behavior spanning apps, assign one owner to the product decision and its feature page. Each implementation lane reports the surfaces it checked and any intentional difference; the integrator compares those reports with the live app and migration path before merging.

### Open Pull Request Worktrees

Every open Tuturuuu pull request must be reviewed, fixed, validated, and prepared
for merge in an isolated checkout under `.worktrees/`. Do not switch the shared
...
- Do not invent new cache stores; rely on Turborepo, Bun, BuildKit, and Cargo
  caches plus the documented bounded cleanup wrappers.
- Python services (`apps/discord/`) maintain their own pinned dependencies and README instructions.

## Testing & Quality Expectations

- Test meaningful affected behavior and required repository gates. Once checks pass, repeat or broaden only for new changes, failures, or unresolved risks. Pure documentation edits do not need application test suites.
- For DB changes, apply migrations locally when feasible, then generate and verify types. Production pushes remain user-only.
...
## Share local validation capacity

Before heavy local validation, check `ttr resources status --json` and use the
`tuturuuu-cli-resources` skill. Run broad tests, builds, type checks, Supabase
setup and browser suites through `ttr resources run -- <command>`. The per-user
queue spans registered projects and linked worktrees, so concurrent agents can
keep editing while heavy stages run sequentially. Prefer a verified remote
runner for broad validation. Do not bypass live queues or stop another session's
services to free capacity. See the SDK reference for setup, diagnosis, and
rollback. If the installed CLI lacks the command, report its version and keep
heavy local work serial with explicit worker limits until upgraded.
