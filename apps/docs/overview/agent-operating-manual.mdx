...
7. Bump `TUTURUUU_PLATFORM_VERSION` once for every authored platform commit and update its test expectation. This is the version shown by the account-gated web version badge.
...
- `$tuturuuu-web-release` for version badge metadata, `PLATFORM_BUILD_*`, and `TUTURUUU_PLATFORM_VERSION` bumps.
