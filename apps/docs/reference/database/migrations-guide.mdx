...
`--include-all`. Staging CI intentionally runs
`supabase db push --include-all` so the staging project can converge after
`main` migration history changes, but ordinary forward deploys should still keep
timestamps monotonic. After a rebase, merge, or hotfix, re-list
`apps/database/supabase/migrations` and rename any still-unapplied local
migration forward instead of asking production to replay out-of-order history.
