...
sorts before the last remote migration unless the operator passes `--include-all`,
and CI should not need that recovery flag for ordinary forward deploys. After a
rebase, merge, or hotfix, re-list `apps/database/supabase/migrations` and rename
any still-unapplied local migration forward instead of asking staging or
production to replay out-of-order history.
