...
server-side Postgres access from `apps/web` through `getPlatformSql()` when an
app route or server component needs private table or helper access. This keeps
browser and mobile consumers on centralized `apps/web` API routes while avoiding
new Supabase REST endpoints for private data.
