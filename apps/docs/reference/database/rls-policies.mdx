...
Supabase Data API exposed schemas or extra search path. If an app route needs
to operate on a private table, expose a narrow `public` RPC that is executable
only by `service_role`, then call it from the centralized `apps/web` API route
with `createAdminClient()`.
...
- Revoke direct `EXECUTE` on bridging RPCs from `public`, `anon`, and
  `authenticated`; grant only to `service_role`.
- Cover private placement, table privileges, schema usage, and RPC grants with
  pgTAP tests.
...
create or replace function public.server_owned_table_action(...)
returns ...
language sql
security definer
...
revoke execute on function public.server_owned_table_action(...)
from public, anon, authenticated;
grant execute on function public.server_owned_table_action(...)
to service_role;
```
