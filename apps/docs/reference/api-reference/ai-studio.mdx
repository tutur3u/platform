...
Workspace Studio dashboards aggregate settled `ai_studio_runs` and
`ai_studio_usage` data. They show billed credits and settled provider cost
separately; registered external-app runs therefore report provider cost while
showing zero billed workspace credits.
