...
and exactly one of `--end` or `--duration-minutes`; `events update
--duration-minutes` also requires `--start`. Source flags map to provider
payloads: use `--source-provider tuturuuu --calendar <workspace-calendar-id>`
for workspace calendars, or `--source-provider google|microsoft --connection
<connection-id>` for connected provider calendars. `calendar calendars reset`
...
Operators can use the authenticated CLI instead of app-local token scripts:
...
### Register and configure an external site from the CLI

Root operators can manage registered apps, approve scopes, create templates,
and bind workspaces without opening Infrastructure or CMS. Existing root and
workspace permission checks remain mandatory; the CLI does not grant access.
...
ttr external binding get --workspace <workspace-id> --json
```

App JSON requires `id`, `displayName`, `enabled`, `origins`, `allowedScopes`,
and `allowedWorkspaceIds` (explicit UUID allowlist). Do not include secrets or
audit fields. Save updates these fields without rotating credentials. Scope
changes replace the whole approved list; `--clear-scopes` revokes all scopes.
Custom template JSON can be
`{"id":"rennu","display_name":"Ren Everheart","adapter":"custom"}`;
the template ID must match the registered app ID. Run `projects setup`, `diff`,
and confirmed `apply` after binding to configure and populate the workspace.

```bash
ttr external apps rotate-secret rennu --secret-out /private/secure/ren-app-secret --confirm ROTATE_EXTERNAL_APP_SECRET
```

This explicitly issues or rotates the secret and immediately invalidates the
previous value. The destination must not exist; its parent must exist. The CLI
writes an owner-only (`0600`) file, never the secret to stdout. Transfer it to
the app's server environment and securely remove the local file; never commit
it. A failed request may leave an empty file, which is not overwritten on retry.
The SDK equivalent lives at `TuturuuuUserClient.external.admin`.
...
Use `ttr resources` to keep multiple coding sessions responsive while serializing
heavy validation on the same machine. This opt-in feature supports macOS and
Linux, works without login, and does not start a background daemon.
...
Setup includes linked Git worktrees automatically. A shared per-user queue admits
one finite validation command at a time; nested commands reuse its slot. Turbo
runs one task and Vitest/Cargo workers adapt to RAM and CPU capacity. Use
`--workers 2` to override workers and `--dry-run` to inspect planned changes.
New admission waits for elevated memory pressure to clear. Existing jobs keep
running; there is no automatic process killing or hard memory cap.

Shell setup backs up existing files and adds a marked block to Zsh or Bash
startup files. Open a new shell to activate it. Use `--shell none` for explicit
`resources run` usage without shell changes. Absolute executable paths, custom
PATH replacement and already-running processes can bypass the shims. Wrap
unrecognized scripts explicitly. Custom scripts can still run parallel child
jobs within a slot; sequence lint, tests and browser QA when memory is limited.

Use `ttr resources disable` or `enable` to toggle future automatic admission.
`TTR_RESOURCES_BYPASS=1` deliberately bypasses one command. Once the queue is idle,
`ttr resources uninstall` removes owned hooks and shims while preserving other
shell content, configuration and backups. Keep the default `TTR_RESOURCES_HOME`
across projects so they share one queue.

The monitor emits bounded-duration JSONL samples with memory pressure, T3 process
tree resident memory, and the largest processes. It omits command arguments and
environment values. RSS totals are not the same as Activity Monitor's memory
footprint and can count shared pages more than once.

Offload full validation with `ttr box run --runner <remote-id> -- <command>` only
after verifying that the runner is on another machine with the intended checkout.
A same-machine CI runner or Docker container still uses local RAM. Resource setup
does not configure runners, transmit code or credentials, or restart Docker.
