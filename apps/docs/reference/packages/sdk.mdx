...
by excluding rows with `completed_at` or `closed_at`; use `--all`, `--done`,
`--closed`, `--include-done`, or `--include-closed` to adjust that filter. Add
`--compact` to task lists when an agent only needs the task title, task list
name, and per-task workspace name. Task lists are ordered by priority and due
date, with prettier due dates and configured task-list colors in table output. Human-readable
tables use smooth box borders, respect the terminal width, and wrap long cell
content so task titles stay readable instead of overflowing narrow panes. Add
`--json` when another agent or script needs machine-readable output. `tasks
