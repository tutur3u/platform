...
`ttr login` opens the browser and creates a fresh Supabase session specifically
designated for the Tuturuuu CLI. The terminal and browser confirmation page
show the signed-in account email when the web session exposes it. The CLI stores
that session and selected workspace, board, list, task, label, and project in
the OS app config directory. `personal` is the default workspace after login and
when no workspace has been selected:
...
`.github/workflows/release-sdk-package.yaml` after a merged version-bump PR
updates `packages/sdk/package.json` and includes `chore(tuturuuu)` in the PR
title. The workflow runs the SDK tests, skips publication if the exact package
version already exists on npm, and otherwise publishes `packages/sdk` to npm
through [npm trusted publishing](https://docs.npmjs.com/trusted-publishers): the
`publish-npm` job requests `id-token: write` so GitHub Actions mints a short-lived
OIDC token, upgrades to the latest `npm` CLI (trusted publishing requires
npm 11.5.1+), and runs `npm publish` without an `NPM_TOKEN`. Because the
`tuturuuu` package is public and `tutur3u/platform` is a public repository, npm
also generates and attaches build provenance attestations automatically.
