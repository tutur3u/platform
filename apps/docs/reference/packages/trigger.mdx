  ...
  run: async (payload: { wsId: string; userId: string }) => {
    ...
    try {
      ...
      for (let i = 0; i < events.length; i += 100) {
        ...
        await supabase.from("workspace_calendar_events").upsert(
          batch.map((event) => ({
            ws_id: payload.wsId,
      ...
      // Log error
      await supabase.from("workspace_calendar_sync_log").update({
        completed_at: new Date().toISOString(),
...
Recurring Calendar provider sync runs from `apps/web` cron, not Trigger.dev:
