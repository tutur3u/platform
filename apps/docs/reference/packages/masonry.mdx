...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.6 introduces hybrid algorithm (Min-Max + post-optimization) for virtually perfect visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, hybrid optimization, and memoized calculations.
...
**v0.3.6 Improvement**: Hybrid algorithm combining Min-Max placement with post-optimization refinement. Achieves virtually perfect balance through two-phase approach: global Min-Max followed by local swap optimization.
...
5. **Hybrid Optimization** (v0.3.6 new):
   - Phase 1: Min-Max placement (evaluates all placements)
   - Phase 2: Post-optimization (swap-based refinement)
   - Phase 3: Build layout from optimized assignments
   - Achieves virtually perfect balance
   - Best of both global and local optimization
...
- **Balanced Strategy (v0.3.6)**: Modern ResizeObserver API with hybrid algorithm (Min-Max + post-optimization) and aggressive stability controls. Event-driven with two-phase optimization for virtually perfect visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with columns ending at virtually identical heights.
...
7. **Hybrid Optimization** (v0.3.6 new):
   - Phase 1: Min-Max placement minimizes height range
   - Phase 2: Up to 2 swap optimization passes
   - Only applies swaps improving balance ≥2px
   - Combines global and local optimization
   - Achieves virtually perfect column equality
...
- ✅ **Hybrid Algorithm** (v0.3.6) - Two-phase approach: Min-Max + swap refinement
- ✅ **Post-optimization** (v0.3.6) - Refines placement after initial Min-Max distribution
...
### 0.3.6 (Current - Perfect Balance Release)
