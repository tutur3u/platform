...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid layout component for React. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm that progressively optimizes layout as images load.
...
- 🖼️ **Progressive Image Loading**: Visible-first rendering with background optimization for image galleries
...
- ⚡ **Zero Layout Shift**: Items render immediately, no hidden measurement phase
...
- ✅ Smooth progressive improvement - no layout jumps
