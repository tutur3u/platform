...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.7 introduces best-first optimization for excellent visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, best-first optimization, and memoized calculations.
...
**v0.3.7 Improvement**: Best-first optimization - each pass finds and applies the BEST swap among all possibilities. Up to 5 passes with 0.5px threshold ensures excellent balance by systematically converging to local optimum.
...
5. **Best-First Optimization** (v0.3.7 enhanced):
   - Phase 1: Min-Max placement (evaluates all placements)
   - Phase 2: Best-first refinement (finds BEST swap in each of 5 passes)
   - Phase 3: Build layout from optimized assignments
   - Accepts improvements >0.5px for fine-grained optimization
   - Systematically converges to excellent balance
...
- **Balanced Strategy (v0.3.7)**: Modern ResizeObserver API with best-first optimization and aggressive stability controls. Event-driven with systematic best-first search (finds BEST swap in each of 5 passes) for excellent visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with excellent column balance.
...
7. **Best-First Optimization** (v0.3.7 enhanced):
   - Phase 1: Min-Max placement minimizes height range
   - Phase 2: Up to 5 best-first optimization passes
   - Each pass finds BEST swap among all pairs
   - Accepts any improvement >0.5px
   - Systematically achieves excellent column equality
...
- ✅ **Best-First Optimization** (v0.3.7) - Finds BEST swap in each pass, not just first improvement
- ✅ **Exhaustive search** (v0.3.7) - 5 passes evaluating ALL swaps ensures excellent balance
...
### 0.3.7 (Current - Perfect Balance Release)
