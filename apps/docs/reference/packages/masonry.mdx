...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.1 brings smooth UX improvements with debounced updates. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement and memoized calculations.
...
**v0.3.1 Improvement**: Debounced updates (200ms) prevent excessive redistributions and jerky movement for smoother UX.
</Note>
...
5. **Smart Redistribution** (v0.3.1 improved):
...
- **Balanced Strategy (v0.3.0)**: Modern ResizeObserver API with memoization and debouncing. Event-driven updates with 200ms debounce to prevent excessive redistributions. 50-70% faster than v0.2.x interval-based measurement.
...
6. **Smart Change Detection**: Only redistributes when heights change by >3px (avoids minor fluctuations from font rendering, subpixel changes)
7. **Intelligent Distribution**: Threshold-based greedy algorithm with coefficient of variation tracking
8. **Debounced Updates** (v0.3.1): 200ms debounce after last change + `requestAnimationFrame` prevents excessive redistribution
9. **Memoized Calculations**: Distribution only recalculates when dependencies actually change
10. **Auto-Cleanup**: Observer automatically disconnects on unmount or strategy change
...
- ✅ **Debounced updates** (v0.3.1) - 200ms wait prevents jerky movement
- ✅ **Smart thresholds** (v0.3.1) - 3px minimum change avoids micro-adjustments
...
- Balanced strategy (v0.3.0):
  - Event-driven measurement (no polling)
  - Change detection (>3px) ignores minor fluctuations
  - 200ms debounce prevents excessive redistributions
...
### 0.3.1 (Current - UX Polish Release)
