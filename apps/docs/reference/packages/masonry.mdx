...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.9 introduces global exhaustive optimization for near-optimal visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, global exhaustive optimization, and memoized calculations.
...
**v0.3.9 Improvement**: Global exhaustive optimization - tries moving EVERY item to EVERY column in each pass. Up to 20 passes exploring all N×M possibilities with tighter tolerance (1% or 10px) ensures near-optimal balance.
...
5. **Global Optimization** (v0.3.9 new):
   ...
   - Up to 20 passes exploring all N×M possibilities
   - Stops when columns within 1% or 10px of each other
   - Guaranteed to find improving move if one exists
...
- **Balanced Strategy (v0.3.9)**: Modern ResizeObserver API with global exhaustive optimization and aggressive stability controls. Event-driven with exhaustive search (tries all item×column moves in up to 20 passes) for near-optimal visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with optimal column balance within 1% or 10px.
...
### 0.3.9 (Current - Optimal Balance Release)
