...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.8 introduces direct item migration for excellent visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, iterative item migration, and memoized calculations.
...
**v0.3.8 Improvement**: Direct item migration - iteratively moves items from tallest to shortest column. Up to 10 passes with smart stopping (within 5% or 20px) ensures excellent balance through direct balancing.
...
- **Balanced Strategy (v0.3.8)**: Modern ResizeObserver API with iterative item migration and aggressive stability controls. Event-driven with direct balancing (moves items from tallest to shortest in up to 10 passes) for excellent visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with excellent column balance.
...
### 0.3.8 (Current - Perfect Balance Release)
