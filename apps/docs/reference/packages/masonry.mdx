...
**v0.3.10 Improvement**: Ultra-aggressive optimization - 50 passes (vs 20) with ultra-tight 0.5%/5px tolerance (vs 1%/10px) ensures near-perfect balance. Column heights differ by ≤5px for professional appearance.
...
For the canonical, machine-generated changelog see [`packages/masonry/CHANGELOG.md`](https://github.com/tutur3u/platform/blob/main/packages/masonry/CHANGELOG.md).

### 0.4.1 – 0.4.3 (Current - CI/Release Fixes)

These releases contain **no public API changes** — the `Masonry` component, props, and
distribution algorithms are identical to 0.3.10. They are release-pipeline and build
recovery fixes only:

- **0.4.3**: `fix(ci)` auto-recover package release dependencies
- **0.4.2**: `fix(ci)` recover releases and package `tsgo` builds
- **0.4.1**: Release-pipeline recovery (no source changes)

No migration or code changes are required when upgrading from 0.3.10 to 0.4.x.

### 0.3.10 (Near-Perfect Balance Release)
