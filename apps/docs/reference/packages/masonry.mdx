...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.5 introduces Min-Max Balanced Greedy algorithm for near-perfect visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, advanced min-max optimization, and memoized calculations.
...
**v0.3.5 Improvement**: Min-Max Balanced Greedy algorithm - evaluates all possible placements and chooses the one that minimizes height range across columns. Produces near-perfect balance by considering global distribution, not just local shortest column.
...
5. **Min-Max Balanced Greedy** (v0.3.5 new):
   - Sorts items by height in descending order
   - Evaluates all possible column placements
   - Minimizes height range (max - min)
   - Considers global balance, not just local shortest
   - Superior balance to simple greedy algorithms
...
- **Balanced Strategy (v0.3.5)**: Modern ResizeObserver API with Min-Max Balanced Greedy algorithm and aggressive stability controls. Event-driven with advanced look-ahead optimization for near-perfect visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with columns ending at nearly identical heights.
...
7. **Min-Max Balanced Greedy** (v0.3.5 new):
   - Sorts items by height (largest first)
   - For each item, evaluates all k possible placements
   - Calculates height range for each option
   - Chooses placement that minimizes range
   - Produces near-perfect balance across columns
...
- ✅ **Min-Max Balanced Greedy** (v0.3.5) - Advanced algorithm that minimizes column height differences
- ✅ **Look-ahead optimization** (v0.3.5) - Evaluates all placements to choose most balanced
...
### 0.3.5 (Current - Perfect Balance Release)
