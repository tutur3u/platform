...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.4 introduces Largest First Decreasing (LFD) algorithm for near-optimal visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, proven bin-packing optimization, and memoized calculations.
...
**v0.3.4 Improvement**: Largest First Decreasing (LFD) algorithm - a proven bin-packing approach that sorts items by size and places largest first. Produces near-optimal balance (within 11/9 of perfect) and handles varied aspect ratios excellently.
...
5. **Largest First Decreasing (LFD)** (v0.3.4 new):
   - Sorts items by height in descending order
   - Places largest items first (more constrained)
   - Greedy placement in shortest column
   - Threshold-based tie-breaking
   - Proven near-optimal bin-packing algorithm
...
- **Balanced Strategy (v0.3.4)**: Modern ResizeObserver API with Largest First Decreasing (LFD) algorithm and aggressive stability controls. Event-driven with proven bin-packing approach for near-optimal visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with mathematically near-optimal column balance.
...
7. **Largest First Decreasing (LFD)** (v0.3.4 new):
   - Sorts items by height (largest first)
   - Greedy placement: each item → shortest column
   - Threshold-based tie-breaking for similar heights
   - Proven algorithm: results within 11/9 of optimal
   - Handles varied aspect ratios exceptionally well
...
- ✅ **Largest First Decreasing** (v0.3.4) - Proven bin-packing algorithm for near-optimal balance
- ✅ **Smart sorting** (v0.3.4) - Places large items first, small items fill gaps naturally
...
### 0.3.4 (Current - Optimal Distribution Release)
...
- ⚖️ **Near-optimal balance**: Results within 11/9 of mathematically perfect distribution
