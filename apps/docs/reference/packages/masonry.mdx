...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with robust measurement validation and progressive optimization as content loads.
...
- 🖼️ **Progressive Image Loading**: Visible-first rendering with intelligent measurement
...
- 🔄 **Auto-cleanup**: Smart stopping conditions for measurement intervals
...
2. **Accurate Measurement**: Dual measurement system (`getBoundingClientRect()` + `offsetHeight`)
3. **Validation & Fallbacks**: Validates all measurements; uses average for missing/invalid values
4. **Smart Change Detection**: Only redistributes when heights change by >1px (avoids float precision)
5. **Progressive Optimization**: Every 100ms, layout redistributes based on current measurements
6. **Image Load Tracking**: Monitors image loading with proper event listeners and `naturalHeight` validation
7. **Immediate Updates**: Triggers redistribution as soon as each image loads
8. **Auto-Cleanup**: Stops measuring when stable or after 50 cycles (5 second safety limit)
9. **Final Balance**: Achieves optimal height-balanced distribution using greedy algorithm

This approach provides:
- ✅ Fast initial render - no waiting for measurements
- ✅ Robust validation - handles edge cases gracefully
- ✅ Smart updates - only when necessary
- ✅ Smooth progressive improvement - no jarring layout jumps
- ✅ Optimal final balance - equal column heights
- ✅ No hidden content phase - everything visible immediately
- ✅ Performance limits - prevents runaway measurements
...
- Balanced strategy: 
  - Dual measurement approach for accuracy
  - Change detection (>1px) prevents unnecessary redistributions
  - Only measures during image loading phase
  - Safety limit prevents infinite loops (max 50 cycles / 5 seconds)
...
<AccordionGroup>
  ...
  <Accordion title="Component stuck in single column with balanced strategy">
    This was fixed in v0.2.1. If you're still experiencing this:
    - Update to the latest version (`bun add @tuturuuu/masonry@latest`)
    - The component now requires only 50% of items to have valid measurements before switching to multi-column
