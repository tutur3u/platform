...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.3 introduces multi-pass optimization for dramatically better visual balance. Version 0.3.2 brought rock-solid stability with aggressive debouncing and auto-stop mechanisms. Version 0.3.0 brought major performance improvements (50-70% CPU reduction), fixed the columns prop bug, and added advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement, multi-pass optimization, and memoized calculations.
...
**v0.3.3 Improvement**: Multi-pass optimization with swap-based rebalancing dramatically improves visual balance. Columns end at similar heights for professional gallery appearance.
...
5. **Multi-Pass Optimization** (v0.3.3 new):
...
- **Balanced Strategy (v0.3.3)**: Modern ResizeObserver API with multi-pass optimization and aggressive stability controls. Event-driven with swap-based rebalancing for optimal visual balance, 500ms debounce, 10px threshold, max 10 redistributions, and auto-stop after images load. Rock-solid layouts that settle and stay settled with dramatically better column balance.
...
7. **Multi-Pass Optimization** (v0.3.3 new):
   - Phase 1: Initial greedy placement (shortest column first)
   - Phase 2: Up to 3 optimization passes trying item swaps
   - Calculates variance before/after each swap
   - Only accepts swaps that improve balance by ≥5%
...
- ✅ **Multi-pass optimization** (v0.3.3) - Swap-based rebalancing for dramatically better visual balance
- ✅ **Variance reduction** (v0.3.3) - Columns end at similar heights for professional appearance
...
### 0.3.3 (Current - Optimal Distribution Release)
