...
The `@tuturuuu/masonry` package provides a Pinterest-style masonry grid component for React with modern ResizeObserver-based measurement. Version 0.3.0 brings major performance improvements (50-70% CPU reduction), fixes the columns prop bug, and adds advanced configuration options. It offers two distribution strategies: a fast count-based approach for uniform content, and an intelligent height-based balancing algorithm with event-driven measurement and memoized calculations.
...
- **Balanced Strategy (v0.3.0)**: Modern ResizeObserver API with memoization. Event-driven updates, no polling overhead. 50-70% faster than v0.2.x interval-based measurement.
...
6. **Smart Change Detection**: Only redistributes when heights change by >1px (avoids float precision issues)
7. **Intelligent Distribution**: Threshold-based greedy algorithm with coefficient of variation tracking
8. **Debounced Updates**: Uses `requestAnimationFrame` to batch multiple changes
...
### 0.3.0 (Current - Major Performance & Bug Fix Release)
