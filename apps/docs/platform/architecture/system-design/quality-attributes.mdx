...
**Monitoring:** Tuturuuu does not ship a custom per-service metrics SDK. Runtime/server logs in `apps/web` API, cron, and infrastructure code flow through the internal **log-drain logger** (never raw `console.*`), and Vercel provides per-project deployment and runtime insight. Treat the `monitorErrorRate(...)` snippet style as conceptual.
