...
**Browser framing boundary.** Every Next.js app that uses
`createTuturuuuNextConfig` receives `Content-Security-Policy: frame-ancestors
'none'` and `X-Frame-Options: DENY` on its responses. This prevents an
...
## Authenticator and meeting coordination

Authenticator registry mutations use the `apps/coordination` Cloudflare Worker
and SQLite Durable Objects. One object per hashed user serializes changes to
server-owned Supabase app metadata. Meeting delivery uses a separate namespace
with payload fingerprints and completion records, so retries preserve their
original identity. Login approval challenges remain in Supabase; this service
never stores authenticator secrets, session tokens, email addresses, or meeting
contents. Upstash is not required for these operations.

Provision `COORDINATION_TOKEN` as a Worker secret. Store its value in the
Infrastructure root-workspace secret `cloudflare_coordination:v1` as an
envelope (`version`, `encryptedDataKey`, `encryptedSecret`) using the existing
mobile deployment encryption helpers, then provision the same value as the server-only
`CLOUDFLARE_COORDINATION_TOKEN` in the web, Calendar, and Infrastructure deployment environments.
Set `CLOUDFLARE_COORDINATION_URL` to the deployed Worker's HTTPS origin. Never
expose these credentials in Flutter defines, public environment variables, URLs,
or logs. Missing credentials and network failures fail closed. Deploy the Worker
and verify its authenticated endpoint before enabling the server callers.

Registry leases expire after five minutes. Meeting leases expire after ten
minutes, while request fingerprints and completion records remain for at least two hours
and never expire before an active lease.
An expired owner cannot complete or release a newer owner's operation. The
calling service must still enforce request age and provider idempotency: a lease
alone cannot prevent an external provider request from finishing after a timeout.
Alarms clean up expired state. Arbitrary payloads and unbounded TTLs are rejected.

Validate changes with:

```sh
bun x --no-install wrangler types --env-interface CoordinationEnv apps/coordination/worker-configuration.d.ts -c apps/coordination/wrangler.jsonc
bun x --no-install tsc --project apps/coordination/tsconfig.json
node --test apps/coordination/src/worker.test.mjs
bun x --no-install wrangler deploy --dry-run -c apps/coordination/wrangler.jsonc
```

The Coordination Cloudflare workflow runs these checks and deploys only the
production branch. Pre-provision `COORDINATION_TOKEN` on the Worker and configure
`COORDINATION_CLOUDFLARE_API_TOKEN` in the GitHub `coordination-production`
environment (the existing account deployment credential is a bootstrap fallback).
Inspect secret names only during deployment checks. A successful `/health`
response proves routing; also verify an authenticated acquire/check/release cycle
and rejection of unauthenticated requests before declaring coordination ready.


## Internal account recovery

Infrastructure's internal account directory and mobile Settings → Internal
accounts share the same authorized API. Native requests use their Supabase Bearer
session; an explicit invalid credential never falls back to another browser
session. Operators need a currently confirmed `@tuturuuu.com` account, enabled
access, and `manage_internal_accounts` in the root workspace.

Profile, access, password, and authenticator actions apply only to exact internal
email accounts. Sensitive actions require typing the target email and cannot
reset the operator's own access or credentials. The mobile directory remains in
memory only; password input is never cached or logged.

Authenticator reset clears server-owned trusted-device proofs before deleting
Supabase MFA factors. It shares the Cloudflare authenticator lease with device
registration and fails closed if coordination is unavailable. A partial provider
failure returns an error; retry the reset to finish removing remaining factors.
The action preserves an existing account ban. Factor enrollment still requires
the account holder to prove possession; administrators cannot silently enroll an
authenticator for someone else. Resetting factors is a recovery operation, not a
policy that requires MFA for subsequent sign-ins.

Before releasing these controls, verify Infrastructure has the same server-only
coordination credentials as web and test both authorized and denied native
requests. Never distribute those credentials in mobile configuration.
