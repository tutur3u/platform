...
Security-relevant actions should be traceable, and heavier security workflows (anomaly scans, notifications, periodic reviews) should run as durable background jobs. Tuturuuu uses **Trigger.dev v4 `task()`** for background work and the internal **log-drain logger** for server-side observability.

<Warning>
**Aspirational, not implemented as written.** A comprehensive, cryptographically signed, immutable, event-sourced audit log is a **target pattern**, not a shipped subsystem. Tuturuuu does not have an event store or event-sourcing layer, and there is no global `audit_events` table that captures every state change. Treat the audit-log code below as an illustration of the goal. What exists today: Supabase keeps row history where tables are designed for it, the log-drain logger records server-side events, and Trigger.dev runs durable background tasks. Do not add raw `console.*` calls in `apps/web` API/cron/infra runtime code — route server logging through the internal log-drain logger.
</Warning>
...
// mechanism and emit server-side logs through the log-drain logger.
...
Trigger it from server code with `securityScanTask.trigger({ userId })` (or `batchTrigger` for many). See the [Event-Driven Architecture](/platform/architecture/system-design/event-driven-architecture) page for how Tuturuuu actually uses Trigger.dev v4 and the log-drain logger.
...
**An append-only audit trail would support accountability and forensics.** This is the target; today, rely on the log-drain logger and table-level history for traceability.

**Compliance benefits from consistent, centralized logging.** Routing server-side events through the log-drain logger (never raw `console.*` in `apps/web` runtime code) keeps audit signals consistent.
