...
Infrastructure exposes **Platform Admin → Internal Accounts** only when the
operator has the root-only `manage_internal_accounts` permission. The server
rechecks that permission against the root workspace on every request and also
requires the actor and target to use an exact `@tuturuuu.com` email address.

Use this surface to disable or restore sign-in access, or to force-set a new
temporary password. Each mutation requires the operator to type the exact
target email, and self-service mutations are rejected to reduce accidental
lockouts. Passwords must never be copied into logs, tickets, or coordination
notes; share a temporary password only through an approved secure channel.
