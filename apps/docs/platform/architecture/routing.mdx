...
description: 'Comprehensive documentation of the workspace ID routing system, middleware handling, and URL patterns in NCT Hub'
updated: '2025-01-10'
---

The workspace ID routing system provides a flexible and consistent way to handle workspace-scoped routes across the NCT Hub platform. This system supports multiple workspace identifier formats, automatic resolution, and intelligent middleware-based routing for club management operations.
...
```typescript
import { resolveWorkspaceId, toWorkspaceSlug } from '@ncthub/utils/constants';
...
```typescript
import { isPersonalWorkspace } from '@ncthub/utils/workspace-helper';
...
```typescript
import { getUserDefaultWorkspace } from '@ncthub/utils/user-helper';
