...
| `/:locale/:wsId/users/topic-announcements/*` | The parent redirect is migrated and the tab wrappers are tiny, but the shared shell still depends on workspace secret gates, send permissions, calendar timezone/E2EE setup, contacts/templates/announcement APIs, scheduling, and import mutations. | Port the route family only after the calendar setup dialog and topic-announcement API lane use typed internal-api or backend facades end to end. |
