...
source of truth. The latest local snapshot on 2026-08-03 is:
...
| `/:locale/:wsId/ai/spark`, `/:locale/:wsId/mira`, and the canonical `/:locale/:wsId` Mira dashboard | Page files are compact, but the behavior is coupled to AI object APIs, Mira API routes, voice/live API helpers, or assistant providers. The standalone `/:locale/:wsId/assistant` route has been removed in favor of the unified workspace-root chat and rich voice experience. | Treat these as dedicated product lanes with typed AI/Mira/assistant facades and focused browser evidence. |
