...
| `/:locale/:wsId/users/topic-announcements/{announcements,import}` | The parent redirect plus templates, contacts, and delivery leaves are migrated, but the remaining shared shell still depends on send permissions, calendar timezone/E2EE setup, announcement APIs, scheduling, and import mutations. | Continue with `announcements` next so create/send/schedule/preview contracts can be reused by the import lane; port import only after those mutation lanes use typed internal-api or backend facades end to end. |
