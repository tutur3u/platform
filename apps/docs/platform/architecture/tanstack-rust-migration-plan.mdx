...
| `/:locale/:wsId/users/topic-announcements/import` | The parent redirect plus announcements, templates, contacts, and delivery leaves are migrated. Import remains separate because it owns bulk CSV/XLSX parsing, contact creation by email, batch/source metadata, row-level errors, `/import`, and create-and-send bulk behavior. | Port import after its parser/grid/result UI and import backend/API parity are scoped, reusing the stable typed topic-announcement facade for shared create/send/contact shapes. |
