...
source of truth. The latest local snapshot on 2026-06-25 is:
...
| `/:locale/:wsId/users/topic-announcements/{announcements,contacts,delivery,import}` | The parent redirect and templates leaf are migrated, but the remaining shared shell still depends on send permissions, calendar timezone/E2EE setup, contacts/announcement APIs, scheduling, and import mutations. | Continue with `contacts` next using the typed contact facades, gate verification behind `send_user_group_post_emails`, and port the rest of the route family only after the calendar setup dialog and announcement/import lanes use typed internal-api or backend facades end to end. |
