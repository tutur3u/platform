...
| `/:locale/:wsId/users/topic-announcements/{announcements,delivery,import}` | The parent redirect plus templates and contacts leaves are migrated, but the remaining shared shell still depends on send permissions, calendar timezone/E2EE setup, announcement APIs, scheduling, delivery history, and import mutations. | Continue with the read-only `delivery` tab next using typed announcement facades, then port announcements/import only after the calendar setup dialog and mutation lanes use typed internal-api or backend facades end to end. |
