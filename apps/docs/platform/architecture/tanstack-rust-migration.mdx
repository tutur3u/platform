...
- `306` pages
- `67` layouts
- `1,154` route handler artifacts
- `19` cron handlers
- `1,532` total tracked route artifacts
- `316` migrated artifacts, `399` terminal artifacts, and `1,133` remaining
...
| `GET /api/v1/infrastructure/{product-units,warehouses}` | `migrated` | Rust `handle_backend_request` accepts inventory app-session credentials or normal Supabase browser/Bearer credentials, requires inventory catalog/setup read permissions, and reads private inventory setup rows through the service-role Supabase REST adapter with exact-count pagination. |
...
| `GET /api/v1/infrastructure/{product-units,warehouses}`                                                                                                        | Legacy-compatible inventory setup exports for private `inventory_units` and `inventory_warehouses`, preserving inventory app-session auth, catalog/setup permission checks, exact-count pagination, and error bodies.     |
