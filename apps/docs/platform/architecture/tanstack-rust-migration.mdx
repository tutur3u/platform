...
- `218` migrated artifacts, `301` terminal artifacts, and `1,217` remaining
...
| `GET` / `POST` `/api/v1/internal/holidays`                                                                                                                     | Legacy-compatible Vietnamese holiday list/create routes. GET uses the Rust server-owned Supabase REST adapter with optional parseInt-style year filtering. POST revalidates Supabase auth, requires root workspace `MEMBER` access, and writes with the caller token so holiday RLS stays active. Item and bulk admin holiday mutations remain legacy-owned. |
