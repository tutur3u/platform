...
- `306` pages
- `67` layouts
- `1,165` route handler artifacts
- `19` cron handlers
- `1,543` total tracked route artifacts
- `369` migrated artifacts, `453` terminal artifacts, and `1,090` remaining
...
| `GET` / `POST` `/api/v1/infrastructure/ai/whitelist/{domains,emails}` and `PUT` / `DELETE` `/api/v1/infrastructure/ai/whitelist/:email` / `/domain/:domain`              | `apps/backend` Rust route dispatcher | `route-overrides.json` marks the list/create collection methods plus detail `PUT` and `DELETE` method artifacts `migrated`; Rust revalidates the normal Supabase browser session or non-app-session Bearer token, requires a `@tuturuuu.com` operator email, reads, inserts, patches, and deletes private whitelist rows through the service-role private-schema REST adapter, preserves list pagination and create validation/defaults, applies legacy `Boolean(enabled)` truthiness for detail updates, and keeps the legacy success, validation, 403, JSON failure, and plain-text failure bodies. |
...
| `GET /api/workspaces/:wsId/finance/charts/balance`                                                                                                                       | `apps/backend` Rust route dispatcher | `route-overrides.json` marks the route `migrated`; Rust accepts finance/platform app-session tokens and CLI app-session tokens before normal Supabase fallback, normalizes workspace identifiers, requires `view_finance_stats`, preserves the legacy required `date` query plus `includeConfidential` parsing, calls `get_wallet_balance_at_date`, and returns the legacy `{ balance, date }`, Unauthorized, Forbidden, invalid-query, and balance lookup error bodies.                                                                                                                                                       |
...
| `GET /api/v1/workspaces/:wsId/settings/permissions` | `migrated` | Rust `handle_backend_request` mirrors the legacy `getPermissions({ wsId, request })` result for settings permission flags after Supabase auth revalidation and workspace membership/default/role permission resolution. |
...
| `/:locale/:wsId/education/courses` | The TanStack route exists and now preserves the legacy `pageSize`/view URL flow, archived-course visibility, module-count display, and row action workspace ids, but it reads through the course API facade rather than the legacy page's broader admin Supabase query. The API also enforces course/user-group permissions and non-guest course semantics. | Decide whether the narrower course API contract is accepted migration behavior, or add a Rust-owned/internal-api reader that exactly preserves the legacy visible row set before adding the page override. |
| `/:locale/:wsId/education/library/quizzes` | The TanStack route exists and now preserves the legacy page-size default plus row action workspace ids, but it reads through the quiz API facade. That facade currently requires mutation-style education access and browser-side internal-api create/edit/delete/AI explanation flows need deployment proxy validation before terminal ownership. | Move or validate the quiz API contract under the Rust/internal-api boundary with read-access parity, then add the page override once list and mutation flows are proven under TanStack deployment. |
| `/:locale/:wsId/education/library/quiz-sets` | The TanStack route and CRUD components exist, but the legacy library table renders linked course/module chips from a joined `linked_modules` shape. Current internal-api list and linked-module facades expose only counts or module/group ids, not course names. | Extend the quiz-set list or linked-modules API facade to return the legacy joined `course_id`, `module_id`, `course_name`, and `module_name` shape, then mark the page artifact migrated once the table column has full parity. |
...
| `GET` / `POST` `/api/v1/infrastructure/ai/whitelist/{domains,emails}`                                                                                          | Legacy-compatible protected AI whitelist collection routes backed by service-role private-schema reads/inserts after Supabase auth and `@tuturuuu.com` operator email validation.                                      |
| `PUT` / `DELETE` `/api/v1/infrastructure/ai/whitelist/{email,domain/:domain}`                                                                                  | Legacy-compatible protected AI whitelist detail update/delete backed by service-role private-schema writes after Supabase auth and `@tuturuuu.com` operator email validation.                                      |
