...
<Warning>
  ...
  combined `rust-backend.yml` CI/Cloudflare workflow, route-manifest CI, and
  automatic TanStack Vercel production builds are disabled in `tuturuuu.ci.ts`.
  Direct TanStack preview/production dispatches also skip their build jobs.
  Shared type-check, unit-test, coverage, and Biome workflows exclude both
  migration source trees; Docker CI skips the TanStack/Rust image and dual-stack
  steps; and maintained Next.js E2E disables the Rust support service while the
  migration matrix remains skipped. `apps/backend` is not deployed and receives
  no production traffic; `apps/web` remains the live API runtime. A route marked
  `migrated` below records implemented source work, not a production cutover or
  a currently maintained migration target.
</Warning>

The pause is enforced by
`node --test scripts/ci/paused-migration-workflows.test.js`. Resume migration CI
only as an intentional policy change: re-enable the switchboard entries and
direct deploy jobs, restore the shared-workflow package/source coverage and
Docker/E2E runtime services together, then update this warning and its guard
...
| ----------------------- | ---------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust backend            | `apps/backend/wrangler.jsonc`      | `bun wrangler deploy --config apps/backend/wrangler.jsonc` | Source- and preview-ready, but not currently deployed. The command is reserved for an explicitly approved future preview.                                                                    |
...
- `152` pages
- `67` layouts
- `594` route handler artifacts
- `8` cron handlers
- `817` total tracked route artifacts
- `215` migrated artifacts, `229` terminal artifacts, and `588` remaining
...
| `/:locale/:wsId/ai/spark`, `/:locale/:wsId/mira`, and the canonical `/:locale/:wsId` Mira dashboard | These look compact at the page file level, but their clients call AI object generation, Mira focus/task/calendar/pet APIs, live voice/audio helpers, or assistant multimodal providers that are still Next/API coupled. The standalone `/:locale/:wsId/assistant` route was removed when text and rich live voice were consolidated into the workspace root. | Move each AI surface as a dedicated product lane with typed AI/Mira/assistant facades and focused browser evidence; avoid shell-only migrations that would expose incomplete AI behavior. |
...
| `GET` / `POST` `/api/v1/internal/holidays`, `POST` `/api/v1/internal/holidays/bulk`, and `PUT` / `DELETE` `/api/v1/internal/holidays/:holidayId`                 | Rust counterparts for the Infrastructure-owned Vietnamese holiday list/create/bulk-import/update/delete routes. GET uses the Rust server-owned Supabase REST adapter with optional parseInt-style year filtering. Mutations revalidate Supabase auth, require root workspace `MEMBER` access, and write with the caller token so holiday RLS stays active. Bulk import preserves the replace-existing year delete plus date-conflict upsert behavior. |
...
```bash
bun benchmark:web-setups -- --setup compare --profile smoke
```
...
```bash
bun benchmark:web-setups -- --setup compare --profile full --require-all
```
