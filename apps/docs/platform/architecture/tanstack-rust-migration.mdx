...
- `117` migrated artifacts, `118` terminal artifacts, and `1,371` remaining
  legacy-owned artifacts after the current polls page batch
...
| `/:locale/contact`             | A TanStack preview route now preserves the visible contact shell and routes profile/inquiry calls through `@tuturuuu/internal-api`, but the backing `/api/v1/users/me/profile` and `/api/v1/inquiries` endpoints are still Next-owned.                                                                        | Move current-user hydration and inquiry creation behind Rust/backend or TanStack server-function facades with CSRF/session handling and shared internal-api coverage before adding contact manifest overrides.                                                    |
