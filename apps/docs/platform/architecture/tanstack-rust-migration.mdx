...
Set `BACKEND_INTERNAL_TOKEN` in the shell before running the smoke command. The
smoke command probes `/healthz`, `/readyz`, authenticated
`/api/migration/status`, and the TanStack root shell. It fails on `4xx` and
`5xx` responses and redacts the bearer token from output. For deployed E2E
smoke runs, set `TANSTACK_EXPECT_BACKEND_REACHABLE=1` and
...
- `90` migrated artifacts, `91` terminal artifacts, and `1,398` remaining
...
reuse the same assertions for migrated product, legal, redirect, offline,
branding, blog, careers, demo, security, partners, UI docs, visualization, and
static solution routes.
