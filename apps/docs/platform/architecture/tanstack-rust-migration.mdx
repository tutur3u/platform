...
- `301` migrated artifacts, `384` terminal artifacts, and `1,145` remaining
...
| `GET /api/v1/infrastructure/{bill-coupons,bill-packages,class-packages,class-scores,package-stock-changes,student-feedbacks}`                                  | Legacy-compatible related-filter export lists backed by caller-token Supabase REST reads with embedded workspace relationships and exact-count pagination.                                                               |
