...
- `260` migrated artifacts, `343` terminal artifacts, and `1,181` remaining
...
| `/:locale/:wsId/tasks`, `/:locale/:wsId/tasks/{cycles,estimates,initiatives,labels,logs,notes,projects,templates,templates/marketplace}`                                                 | `apps/tanstack-web` TanStack Start route loaders | `route-overrides.json` marks these page artifacts `migrated`; the Start routes preserve authenticated workspace resolution, route metadata, and shared task UI clients. Project/cycle/initiative/log/template/estimate/label routes keep the legacy `manage_projects` permission gate through the forwarded-auth internal permission check. Parent dashboard/session and task data APIs remain separately owned. |
