...
- `320` migrated artifacts, `403` terminal artifacts, and `1,131` remaining
...
| `GET /api/v1/infrastructure/email-blacklist` and `GET /api/v1/infrastructure/email-blacklist/:entryId`                                                                     | `apps/backend` Rust route dispatcher | `route-overrides.json` marks only the `GET` method artifacts `migrated`; Rust revalidates the normal Supabase browser session or non-app-session Bearer token, preserves the root workspace membership gate through caller-token Supabase REST reads, returns the legacy ordered collection/detail row payloads, maps `PGRST116` detail misses to the legacy `404` body, and keeps POST/PUT/DELETE legacy-owned until blacklist writes move to Rust.                                                                                 |
...
| `GET /api/v1/infrastructure/email-blacklist[/:entryId]` | `migrated` | Rust `handle_backend_request` validates the Supabase browser session or Bearer token, checks root workspace membership with the caller token, returns the legacy ordered collection/detail rows, and leaves blacklist write methods legacy-owned. |
...
| `GET /api/v1/infrastructure/email-blacklist[/:entryId]`                                                                                                        | Legacy-compatible root-workspace email blacklist reads backed by caller-token Supabase REST reads; POST/PUT/DELETE remain legacy-owned write methods.                                                                     |
