...
- `307` pages
- `67` layouts
- `1,139` route handler artifacts
- `19` cron handlers
- `1,518` total tracked route artifacts
- `219` migrated artifacts, `302` terminal artifacts, and `1,216` remaining
...
| `/:locale/changelog/:slug`     | The TanStack detail route exists, but terminal ownership still needs deterministic no-auth E2E evidence for at least one published slug or an accepted fixture strategy for an empty database.                                                                                                                 | Add stable public changelog detail fixture coverage, then mark the dynamic page artifact migrated with the existing Rust-owned slug lookup endpoint.                                                                                                               |
...
| `GET` / `POST` `/api/v1/internal/holidays` and `PUT` / `DELETE` `/api/v1/internal/holidays/:holidayId`                                                          | Legacy-compatible Vietnamese holiday list/create/update/delete routes. GET uses the Rust server-owned Supabase REST adapter with optional parseInt-style year filtering. Mutations revalidate Supabase auth, require root workspace `MEMBER` access, and write with the caller token so holiday RLS stays active. Bulk holiday import remains legacy-owned. |
