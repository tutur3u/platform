...
- `305` pages
- `67` layouts
- `1,134` route handler artifacts
- `19` cron handlers
- `1,511` total tracked route artifacts
- `170` migrated artifacts, `253` terminal artifacts, and `1,258` remaining
...
| `GET /api/v1/aurora/ml-metrics`, `GET /api/v1/aurora/statistical-metrics`                                                                                                | `apps/backend` Rust route dispatcher | `route-overrides.json` marks only the read-only `GET` method artifacts `migrated`; Rust reads the public Aurora metric tables through the server-owned Supabase REST adapter and returns the raw legacy row arrays while the external ingest `POST` methods remain `legacy-next`.                                                                                                                                                                                                                                                                           |
...
| `GET /api/v1/aurora/{ml-metrics,statistical-metrics}`                                                                                                         | Legacy-compatible public Aurora metric readers backed by the Rust server-owned Supabase REST adapter. External metric ingest `POST` methods remain legacy-owned.                                                              |
