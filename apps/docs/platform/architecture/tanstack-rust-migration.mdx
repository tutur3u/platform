...
app. `AURORA_EXTERNAL_URL` is required before the Rust-owned Aurora health probe
can call the Aurora service. Wrangler prompts for values; do not place those
...
- `306` pages
- `67` layouts
- `1,146` route handler artifacts
- `19` cron handlers
- `1,524` total tracked route artifacts
- `268` migrated artifacts, `351` terminal artifacts, and `1,173` remaining
...
| `GET /api/v1/aurora/forecast`, `GET /api/v1/aurora/ml-metrics`, `GET /api/v1/aurora/statistical-metrics`                                                                 | `apps/backend` Rust route dispatcher | `route-overrides.json` marks only the read-only `GET` method artifacts `migrated`; Rust reads the public Aurora forecast and metric tables through the server-owned Supabase REST adapter. Forecast reads preserve the legacy combined `{ statistical_forecast, ml_forecast }` object, `date.asc` table ordering, and `YYYY-MM-DD` date shape, while metric reads return the raw legacy row arrays. The external ingest `POST` methods remain `legacy-next`.                                                                                              |
...
| `/:locale/:wsId/inventory/{suppliers,warehouses}`                                                                                                                                      | `apps/tanstack-web` TanStack Start dynamic routes | `route-overrides.json` marks both inventory resource page artifacts `migrated`; the Start routes preserve fail-closed current-user auth, forwarded-auth workspace resolution, inventory view/create/update/delete permission booleans, localized access-denied states, URL-backed `q`/`page`/`pageSize` table state, and TanStack Query table loading plus create/update/delete actions through the internal-api inventory facades. The matching workspace inventory and product compatibility APIs remain separately tracked backend work. |
...
| `GET /api/v1/aurora/{forecast,ml-metrics,statistical-metrics}`                                                                                                | Legacy-compatible public Aurora forecast and metric readers backed by the Rust server-owned Supabase REST adapter. External Aurora ingest `POST` methods remain legacy-owned.                                                |
