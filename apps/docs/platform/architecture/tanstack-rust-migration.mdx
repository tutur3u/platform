...
| `/:locale/contact` page        | A TanStack preview route now preserves the visible contact shell and owns the static metadata layout, but profile/inquiry calls still route through `@tuturuuu/internal-api` to Next-owned `/api/v1/users/me/profile` and `/api/v1/inquiries` endpoints.                                                        | Move current-user hydration and inquiry creation behind Rust/backend or TanStack server-function facades with CSRF/session handling and shared internal-api coverage before adding the page manifest override.                                                     |
