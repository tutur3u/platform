...
- `305` pages
- `67` layouts
- `1,131` route handler artifacts
- `19` cron handlers
- `1,508` total tracked route artifacts
- `168` migrated artifacts, `251` terminal artifacts, and `1,257` remaining
...
| `GET /api/v1/infrastructure/changelog/slug/:slug`                                                                                          | `apps/backend` Rust route dispatcher | `route-overrides.json` marks the route `migrated`; Rust reads `changelog_entries` through the server-owned Supabase REST adapter, preserves the public published-only slug lookup, requires `published_at` to be non-null, requests the same singular PostgREST object shape as the legacy Supabase `.single()` call, and maps `PGRST116` to the legacy `404` body. Full changelog list/admin routes remain legacy-owned.                                      |
