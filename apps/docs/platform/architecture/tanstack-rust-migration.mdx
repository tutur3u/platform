...
both frontends. Each frontend result includes `passRate` and `wallMs` so
`bun migration:tanstack:gates` can evaluate E2E regressions from the same
evidence file. Set `E2E_COMPARE_REPORT_PATH` when a run needs to write the file
elsewhere under `tmp/`.
