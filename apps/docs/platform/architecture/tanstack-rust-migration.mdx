...
re-running smoke checks deterministic. If a preview deploy regresses, redeploy
the last known-good Worker version through Wrangler or remove the preview route,
then run `bun smoke:cloudflare` against the remaining preview origins before
resuming canary traffic.
...
- Bind `BACKEND_INTERNAL_TOKEN`, `BACKEND_PUBLIC_ORIGIN`, and
  `BACKEND_INTERNAL_URL` with `wrangler secret put` or Cloudflare dashboard
  secret bindings. Do not commit literal origins that are account-specific
  private targets, tokens, API keys, cookies, or session material.
