...
<Warning>
  `apps/backend` is not currently deployed and receives no production traffic.
  `apps/web` remains the live API runtime. A route marked `migrated` in this
  document means its future Rust source contract is implemented and tested; it
  does not mean that request routing has cut over.
</Warning>
...
- `151` pages
- `58` layouts
- `590` route handler artifacts
- `8` cron handlers
- `803` total tracked route artifacts
- `215` migrated artifacts, `228` terminal artifacts, and `575` remaining
