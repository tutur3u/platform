...
- `231` migrated artifacts, `314` terminal artifacts, and `1,210` remaining
...
| `GET /api/v1/devboxes/cache`                                                                                                                                             | `apps/backend` Rust route dispatcher | `route-overrides.json` marks the read-only cache list route `migrated`; Rust accepts CLI app-session Bearer tokens for the platform target with `cli:access` or browser/non-app-session Supabase credentials, requires root workspace `MEMBER` access, and preserves the legacy `{ caches: [] }` body. `/api/v1/devboxes/cache/prune` remains legacy-owned.                                                                                                                                                                                                  |
...
| `GET /api/v1/devboxes/cache`                                                                                                                                  | Legacy-compatible devbox cache list route accepting platform CLI app-session access tokens or browser Supabase sessions, requiring root workspace `MEMBER` access, and returning the current empty cache list body. Prune remains legacy-owned. |
