...
- `116` migrated artifacts, `117` terminal artifacts, and `1,372` remaining
  legacy-owned artifacts after the current chatbot form batch
...
| `/:locale/contact`              | The legacy contact form depends on current-user hydration and a protected support-inquiry mutation path.                                                               | Move inquiry creation behind a Rust/backend or TanStack server-function facade with CSRF/session handling and shared internal-api coverage before route ownership changes.                                                                                 |
