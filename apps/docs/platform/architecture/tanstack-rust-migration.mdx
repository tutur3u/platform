...
`DELETE`, `HEAD`, and `OPTIONS` handlers.
...
- `119` migrated artifacts, `120` terminal artifacts, and `1,369` remaining
  legacy-owned artifacts after the current women-in-tech metadata batch
...
- a Rust route dispatcher test covering the success case and unsupported method
  behavior
