...
| ----------------------- | ---------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust backend            | `apps/backend/wrangler.jsonc`      | `bun wrangler deploy --config apps/backend/wrangler.jsonc` | Preview-ready for migrated pure dispatcher routes, health/readiness, protected migration inventory, static/cookie endpoints, and the native/Worker outbound boundary. |
...
| ---------------------------------- | ------------------------------------------------------------------------- |
| `apps/backend/wrangler.jsonc`      | `BACKEND_INTERNAL_TOKEN`, `TUTURUUU_APP_COORDINATION_SECRET`              |
...
Cloudflare environments. Wrangler prompts for secret values; do not place those
values in `wrangler.jsonc`, `vars`, docs, or shell history.

`SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` can be configured as backend
Worker secrets now to exercise the redacted contact data readiness signal on
`GET /api/migration/status`. They are not listed as required bootstrap secrets
yet because the contact data adapter still returns
`CONTACT_DATA_LAYER_NOT_READY`. The async native/Worker HTTP boundary exists,
but profile hydration and support-inquiry persistence still need route wiring,
Supabase auth revalidation, and persistence tests before those endpoints can be
terminally migrated.
...
the static shell rendered. Use `--output` to persist the smoke report under
...
- Outbound HTTP proxy parity for the Discord cron routes is pending route
  wiring and tests that use the async outbound abstraction in both native Axum
...
- `145` migrated artifacts, `147` terminal artifacts, and `1,358` remaining
...
comparison; smoke reports alone are useful for previews but are not sufficient
cutover evidence. The Cloudflare smoke report must come from
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /api/cron/discord/daily-report`     | Exact parity proxies a POST to `DISCORD_APP_DEPLOYMENT_URL/daily-report`; the shared native/Worker outbound boundary exists, but the route-specific cron proxy behavior is not wired yet.             | Use the outbound abstraction, add cron bearer config for `CRON_SECRET` / `VERCEL_CRON_SECRET`, upstream JSON/status passthrough, invalid-JSON fallback parity, OpenAPI coverage, and route tests before marking this route migrated.              |
...
| `/:locale/contact` page        | A TanStack preview route now preserves the visible contact shell, owns the static metadata layout, hydrates the current-user contact profile through a TanStack Query loader/server function, and submits inquiries through a TanStack server function backed by `@tuturuuu/internal-api` Rust backend facades. The Rust backend can verify Tuturuuu app-session JWTs and expose app-session-only profile/inquiry foundation routes, but Supabase session revalidation, private profile hydration, and `support_inquiries` persistence are not Rust-owned yet; valid writes still return `CONTACT_DATA_LAYER_NOT_READY`. | Add the Rust Supabase auth/data adapter, persist `support_inquiries` server-side, preserve same-origin mutation checks, add contact E2E/benchmark evidence, and keep `page:/:locale/contact`, `/api/v1/users/me/profile`, and `/api/v1/inquiries` `legacy-next` until those checks pass. |
