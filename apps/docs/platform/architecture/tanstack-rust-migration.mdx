...
- `247` migrated artifacts, `330` terminal artifacts, and `1,194` remaining
...
| `/:locale/:wsId/tasks/{estimates,labels,notes}`                                                                                                                                         | `apps/tanstack-web` TanStack Start route loaders | `route-overrides.json` marks these page artifacts `migrated`; the Start routes preserve authenticated workspace resolution, route metadata, and shared UI clients. Estimates and labels keep the legacy `manage_projects` permission gate through the forwarded-auth internal permission check. Parent dashboard/session and task data APIs remain separately owned.                         |
