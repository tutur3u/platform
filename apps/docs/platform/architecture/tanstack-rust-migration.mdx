...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `apps/backend/wrangler.jsonc`      | `BACKEND_INTERNAL_TOKEN`, `TUTURUUU_APP_COORDINATION_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `CRON_SECRET`, `DISCORD_APP_DEPLOYMENT_URL` |
...
- `155` migrated artifacts, `234` terminal artifacts, and `1,271` remaining
...
| `GET /api/share/course/:courseId`, `GET /api/sync-logs`, `POST /api/payment/migrations/subscriptions/cross-check`, `GET /api/users/search` | `apps/backend` Rust route dispatcher | `route-overrides.json` marks these retired legacy API aliases and broad maintenance/search routes `accepted-removal`; Rust returns terminal `410 ENDPOINT_REMOVED` with legacy `Allow` headers while maintained replacements, workspace-scoped readers, or phase-specific routes remain separate migration work.                               |
...
| `/:locale/contact` page        | A TanStack preview route now preserves the visible contact shell, owns the static metadata layout, hydrates the current-user contact profile through a TanStack Query loader/server function, and submits inquiries through a TanStack server function backed by `@tuturuuu/internal-api` Rust backend facades. The Rust backend owns `/api/v1/users/me/profile` and `/api/v1/inquiries` for the app-session contract, including server-side Supabase profile hydration, profile updates, support-inquiry inserts, same-origin mutation checks, OpenAPI coverage, and mocked native/Worker outbound tests. | Add browser E2E evidence for the TanStack contact route against the Rust backend, then mark `page:/:locale/contact` migrated when route-level profile hydration and inquiry submission pass in the compare flow.                                                  |
...
outbound adapter, and have mocked native/Worker persistence tests. Broader
Supabase session revalidation remains part of the dedicated auth migration
milestone, so route notes must stay explicit about the app-session boundary.
...
- Bind `BACKEND_INTERNAL_TOKEN`, `BACKEND_PUBLIC_ORIGIN`,
  `BACKEND_INTERNAL_URL`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`,
  `CRON_SECRET`, and `DISCORD_APP_DEPLOYMENT_URL` with `wrangler secret put`,
