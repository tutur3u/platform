...
- `305` pages
- `67` layouts
- `1,117` route handler artifacts
- `19` cron handlers
- `1,494` total tracked route artifacts
- `124` migrated artifacts, `125` terminal artifacts, and `1,369` remaining
...
| `OPTIONS /api/v1/auth/otp/settings`                                             | `apps/backend` Rust route dispatcher | `route-overrides.json` marks only the `OPTIONS` method artifact `migrated`; Rust preserves the shared legacy wildcard CORS `204` response while the `GET` OTP settings lookup remains `legacy-next`.                                                                                                                                           |
| `OPTIONS /api/v1/auth/mfa/mobile/approvals`                                     | `apps/backend` Rust route dispatcher | `route-overrides.json` marks only the `OPTIONS` method artifact `migrated`; Rust preserves the legacy bare empty `204` response while the `GET` approval listing remains `legacy-next`.                                                                                                                                                        |
