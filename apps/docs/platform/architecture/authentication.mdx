...
1. The browser first completes Cloudflare Turnstile. The QR card must not create
...
2. The browser renders a `tuturuuu://auth/qr-login` payload that contains the
...
5. The browser polls `GET /api/v1/auth/qr-login/challenges/:id?secret=...`.
...
- Add `apps/<app>/src/app/api/auth/verify-app-token/route.ts` with `createPOST('<app>')` so `/verify-token` can exchange the cross-app token for an app-local session.
