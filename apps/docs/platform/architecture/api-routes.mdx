...
### Trusted accounts and locations (read uplift)

Proxy read limits are keyed per-IP and treat every caller as anonymous, because
auth headers and cookies are forgeable at the edge — their presence alone must
never raise a budget. To give legitimate teams (often many people behind one
office NAT/VPN IP) higher read throughput, the proxy consults a server-written
**trust cache** in Redis, keyed by the same subject keys as the abuse-reputation
system (`session:<hash>`, `cidr:<network>`, `ip:<addr>`):

- A genuinely trusted **session** (its key is a hash of the real auth cookie, so
  it cannot be forged) gets its own per-session read bucket, so trusted
  teammates behind one shared IP no longer collide.
...
caps. The cache only ever raises read limits (never lowers them — restrictive
decisions stay server-side) and fails open to neutral when Redis is unavailable.
It is populated by the session write-through in `recordResponseAbuseSignal` and
reconciled every 10 minutes by the
`/api/cron/infrastructure/sync-trust-cache` cron (which also propagates admin
trusted-location overrides). Set `API_PROXY_EDGE_TRUST_ENABLED=0` to disable the
uplift and fall back to per-IP read limiting; tune the cache lifetime with
