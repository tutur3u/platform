...
3. Run **Test** as a readiness check for required credentials and channel
   status. This does not replace a live external-platform round trip.
...
5. Select a mirrored `ai-agent-thread-<uuid>` conversation and use **Sync
   external thread**. A zero-message result means the adapter found no newer
   messages.
6. Draft and send a manual response from the thread panel, then confirm the
