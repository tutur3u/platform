...
and injects the internal API key, bundled Postgres password, Better Auth secret,
database URL, internal base URL, fail-open setting, and timeout. Use
...
`DOCKER_SUPERMEMORY_ENABLED=false` only when an operator needs to override that
generated runtime. An explicit false value also removes the Supermemory sidecar
from Docker blue/green support builds and health gates, so local-only test runs
do not need access to the private enterprise image.
...
services. The watcher and cron recovery path use the same generated Docker env,
so recreating `web-blue-green-watcher` does not require manually exporting
Supermemory secrets.
