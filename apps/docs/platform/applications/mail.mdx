...
batches preserve each message's mailbox/thread target. The native payload contains
...
minutes. Every attempt repeats the same access and preference checks. As with
other push providers, a process crash after Firebase accepts a notification but
before its database acknowledgement can cause a repeated device delivery.
