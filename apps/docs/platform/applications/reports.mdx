...
Manual delivery requests lock the report before the queue and update both states in the same database transaction. Approval transitions use the same lock order; worker queue claims do not lock reports. Stale queue requests are rejected after approval is revoked. Provider-accepted deliveries retain their acceptance timestamp even if later tracking fails, preventing re-approval from automatically resending them.
