...
Queue lifecycle changes synchronize the visible report status in the same database transaction. Stale queue requests are rejected after approval is revoked. Provider-accepted deliveries retain their acceptance timestamp even if later tracking fails, preventing re-approval from automatically resending them.
