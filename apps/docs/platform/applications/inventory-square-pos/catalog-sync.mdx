...
| Price and currency | Square Money amount and currency | Exact whole-major-unit prices import directly; fractional major-unit prices are preserved for review instead of rounded |
...
<Warning>
  Square can report fractional prices or physical counts, while Tuturuuu's base
  Inventory rows currently store whole major-currency units and whole stock
  units. Import still creates the product and variation link, but it preserves
  an existing local value or initializes a new value at zero and marks that
  link **Sync error** for review. Tuturuuu does not round the Square value or
  write anything back to Square automatically.
</Warning>
