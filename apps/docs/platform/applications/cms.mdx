...
- CMS media uploads show per-file progress for cover images, gallery media, and WebGL ZIP packages. WebGL ZIP package uploads must resolve the large `PUT` request to the self-hosted web API origin, not the Vercel-hosted CMS origin, because Vercel function payload limits reject large game archives before the web stack can store them.
