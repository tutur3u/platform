...
- CMS does not register the shared offline service worker or emit the shared web app manifest. It is a Vercel-hosted satellite that proxies protected APIs to `apps/web`, so PWA routes owned by `apps/web` must not be advertised from `cms.tuturuuu.com`.
