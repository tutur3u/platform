...
HttpOnly `tuturuuu_app_session` cookie, and then redirects to the requested
Hive path. It must not create or refresh a Hive-domain Supabase Auth session.
Hive local logout is intentionally app-local: `/api/auth/logout` clears
...
- Pending Hive requests appear in the same platform roles panel. Approving a
  ...
  server gate. Do not update only one store, or users can authenticate through
  Tuturuuu and then loop back to the restricted page.
...
- `apps/web` remains the login authority and verifies platform roles through
  ...
  cookie. Hive product data belongs in the dedicated Docker-managed Hive
  Postgres database and is accessed through `HIVE_DATABASE_URL`.
...
only, while `apps/web` internal APIs use the forwarded app-session cookie to
resolve the caller and platform-admin status.
...
and deducts credits only after successful model usage. If model access is not
configured, the route can still persist a deterministic NPC decision for
inspectable simulation history.
...
`hive_npc_runs` stores grouped interaction metadata for both manual and
autonomous runs: `interaction_id`, source/target NPCs, trigger, status, model,
provider, token counts, credit source/workspace, deducted credits, and errors.
Timeline surfaces should group or filter from this persisted run history plus
world events rather than recomputing transcripts from transient client state.
...
Manual editor changes persist through the web gateway world-event API. That API
stores the authoritative `world_data` and the matching CRDT state in the same
Hive Postgres transaction, then the Hive client only broadcasts the already
...
users, realtime status, revision, and the last sync notice. The NPC Lab edits
only the selected NPC. When no NPC is selected, it should show a selection
prompt instead of silently mutating the first NPC in the server.

The editor also has a `World / Workflows` mode switch. Workflow mode is a 2D
React Flow graph builder for manual Hive automations. The workflow command bar
owns server selection, always-visible server info, workflow selection, editable
workflow title/description, latest run status, Save/Run actions, and collapse
toggles for the palette, node inspector, and run trace. Keep those controls in
the top bar instead of restoring separate title blocks, hover-only server info,
or large tutorial chrome inside workflow mode. Admins can drag nodes from the
palette, use starter templates, edit JSON config in the inspector, save shared
workflows, and run them against the selected server. Members see the same graph
read-only and can manually run saved workflows. Workflow runs apply live effects
through the same gateway-backed Hive actions used elsewhere: world events,
simulation ticks, NPC decision records, farming, warehouses, and trades. Effects
are audited through the workflow run trace plus the existing Hive event/economy
tables; v1 has no schedule, webhook trigger, retry queue, credential vault, or
rollback approval layer.
...
The editor mode switch also includes Timeline. Timeline mode reads
`/api/v1/hive/servers/:serverId/timeline`, merges world events with NPC run
history, and displays model/cost/trigger metadata for each persisted run. Keep
manual and autonomous interaction rows grouped by `interaction_id` whenever a
deeper transcript view is added.
