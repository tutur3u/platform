...
Learn owns its email/password and email-code auth shell locally. Login, callback, logout, and session routes are implemented inside `apps/learn`. Auth metadata still uses the `tulearn` client value because the central API, OTP settings, and database tables keep the existing product namespace. Web-style Supabase cookies are kept on the Learn app.

Social auth providers still run on `apps/web` because OAuth provider callbacks are registered for `tuturuuu.com`. Learn social buttons redirect to the central web login with `returnUrl=<Learn /verify-token URL>` and `provider=<provider>`. After the web callback succeeds, `apps/web` generates a cross-app token for the existing `tulearn` target app, redirects back to `/verify-token`, and Learn creates its own domain-scoped Supabase session through `POST /api/auth/verify-app-token`.

Protected product data still belongs to the central platform. Learn calls `apps/web` APIs through `@tuturuuu/internal-api`, and the satellite app rewrites `/api/v1/*` to the central web app when no local Learn route exists.
...
Language switching is handled inside Learn with `next-intl` localized navigation links. Keep English and Vietnamese strings in `apps/learn/messages/en.json` and `apps/learn/messages/vi.json`, then run `bun i18n:sort`.
...
Both workflows are registered in `tuturuuu.ts` and use the shared `ci-check.yml` switchboard. They require the standard Vercel secrets plus `VERCEL_LEARN_PROJECT_ID`; production also uses the shared production Supabase secrets.
