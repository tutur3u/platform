...
`apps/tasks` is registered internally as the `tudo` app. All app-session
checks, cross-app token verification, and Web API allowlists must use
`targetApp: 'tudo'`; `tasks` is only the hostname label used by
`tasks.tuturuuu.com` and `tasks.tuturuuu.localhost`.
