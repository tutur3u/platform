...
uses the server-only `private.list_task_source_filter_ids` RPC through a direct
server database connection, not client-facing PostgREST schema exposure. The RPC
rechecks target workspace membership and source workspace access before
returning paginated task ids plus `total_count` for API-side hydration. Client UI
must not call the private RPC directly or trust client-provided source ids as
authorization.
