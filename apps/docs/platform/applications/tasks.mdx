...
when the user wants to focus on planned lists.

By default the external lane shows only open source tasks from normal task
lists. Source tasks in `documents`, `done`, or `closed` lists are filtered out
unless the lane-level compact filters explicitly include them. Lane sorting is
...
returns to the external lane. Routes that read personal board tasks should keep
filtering inaccessible source tasks out instead of surfacing stale placements to
users who no longer have source-workspace access.

Done and closed actions are the exception to the source-list rule. Marking a
placed external task as done or closed moves the personal reference to the first
matching personal list and also moves the source task to the first matching
`done` or `closed` list on its original board. Regular personal-list moves still
update only the user's placement.
