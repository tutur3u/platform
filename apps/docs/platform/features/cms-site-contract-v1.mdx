...
Use `cms_site` for new CMS-backed websites. It is the global version 1 site
contract, not a project-specific implementation. Existing dedicated adapters
remain supported, but new site differences should normally be represented by
template metadata and collection schema/profile metadata.
...
`taxonomies`, `landing-sections`, and `redirects`. Apps may add fields and
...
Template metadata is persisted in the canonical project's delivery profile and
metadata. It is returned in sync snapshots and public delivery, but it does not
create content diff operations or destructive changes.
