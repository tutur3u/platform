...
the meeting, personal capture pauses before sending more audio. Use headphones:
...
Private workspace tools in text chat retain their separate requester review
flow; the voice session does not automatically inherit workspace credentials.

## Explicit approvals

A personal assistant may propose remembering a fact or sharing an exact reply
with the room. The owner sees the proposed text and can approve or deny it.
...
and billing receipts remain separate. Audio is not archived, and audio never received by
...
provider events by modality. Reservations renew before exhaustion; failed
settlements remain durable and retry after the session ends.
