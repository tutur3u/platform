...
verified session. A changed account invalidates an open review. Tasks use the existing workspace executor. Calendar writes resolve the native primary calendar and use the existing encryption helper before inserting. Meet performs these authorized writes with its own verified session.
...
write, check the destination before creating another copy. Confirmed pre-write failures clear the local attempt marker so the user can correct the destination and retry. Receipt persistence can
