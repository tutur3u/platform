...
- External app admin dashboards should call `POST /api/v1/workspaces/{wsId}/external-projects/setup` with their sync manifest before diff/apply. The route uses the existing app-coordination token, requires manage access, creates the canonical `{adapter}-main` project if needed, and binds the configured workspace without requiring production Supabase keys in the external app.
