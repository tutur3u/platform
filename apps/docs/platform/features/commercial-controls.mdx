...
Plan changes open Polar checkout for the final quantity, discount, tax and payable
amount. Seat calculation preserves purchased capacity, covers current members and
pending workspace/email invitations, and honors target minimum/maximum bounds.
An invitation alone does not increase the purchased quantity; a plan-transition
checkout explicitly quotes any additional reserved capacity for confirmation. Direct plan mutation rejects changes that
would alter the purchased seat count; those require checkout confirmation.
