...
disables resubmission. A retry reconciles an already-applied plan without invoicing
the same change again. Pending state survives confirmation-dialog dismissal until
the billing page observes the target plan. Subscription projections insert without
overwriting existing rows, then update only when the stored provider timestamp is
not newer. Older webhook deliveries return no subscription data, preventing stale
