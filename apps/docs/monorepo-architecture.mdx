...
description: 'Understanding NCT Hub monorepo architecture and the benefits of using Turborepo.'
---
...
## Why NCT Hub Uses a Monorepo

NCT Hub's platform is built as a monorepo (`@rmit-nct/hub`) for several key reasons:
...
NCT Hub uses [Turborepo](https://turborepo.org/) as its build system for several reasons:
