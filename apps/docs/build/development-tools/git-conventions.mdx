...
Use the root command below when `staging` and `production` should point at the
same commit as `main`:
...
The command refuses to run unless the worktree is clean, then fetches `origin`,
pulls `main`, `staging`, and `production` with `--ff-only`, pushes `main`, and
fast-forwards `staging` and `production` to the current `main` commit before
pushing those branches. It finishes by fetching again and verifying that local
and remote `main`, `staging`, and `production` all resolve to the same commit.
After verification, it checks out the branch you started from.

`bun git-sync` does not create commits for you. Commit all intended changes on
`main` first; if `staging` or `production` contain commits that are not already
in `main`, reconcile those branches manually before rerunning the command.

Use `--only-branch` to update one branch while leaving the other release branch
untouched:

```bash
bun git-sync --only-branch staging
bun git-sync --only-branch production
```

`main` is still refreshed locally as the source commit, but only the selected
branch is fast-forwarded and pushed. Use `--only-branch main` when you only want
to pull and push `main`.

Use `--no-push` for local-only synchronization:

```bash
bun git-sync --no-push
bun git-sync --only-branch staging --no-push
```
