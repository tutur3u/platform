...
`main`, and fast-forwards `staging` and `production` to the current `main`
commit from that temporary worktree before pushing those branches. It finishes
by fetching again and verifying that local and remote `main`, `staging`, and
`production` all resolve to the same commit. The checkout you started from is
left untouched, so uncommitted work on another branch can keep running while the
release refs are synchronized.
