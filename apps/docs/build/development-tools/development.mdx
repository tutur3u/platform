...
First-party package families that publish coordinated releases together may be
explicitly excluded from the age gate in `bunfig.toml`; keep each trusted
family's exact-name list aligned with the packages resolved in `bun.lock` and
cover the list with a policy test.
...
unless you are deliberately comparing language server behavior. Every app
configuration overrides TypeScript 7's `types` compatibility wildcard with its
explicit runtime globals, so editors with an older bundled language service do
not report `Cannot find type definition file for '*'` across the app tree.
...
its protected runner and executes this exact command. Each store upload derives
its app version from the major and minor values in `apps/mobile/pubspec.yaml`
and the highest existing TestFlight prerelease version for that series. For
example, `0.11.0` advances to `0.11.1`, then `0.11.2`. Android and iOS receive
the same version, while the unique CI build number remains separate. This does
not commit a version bump; Release Please still owns the source version.
Development builds remain available through `bun build:android:dev`.

TestFlight distribution defaults to all available beta groups. Configure
`TESTFLIGHT_BETA_ENABLED=false` to disable automatic distribution, or set
`TESTFLIGHT_BETA_GROUPS` to a comma-separated list of group names or IDs to
limit it. If Apple is still reviewing an earlier external build, the new build
remains available to internal testers and its external review is deferred.
The Mobile TestFlight Review Queue workflow checks every five minutes and can be
run manually; after the earlier review ends, it submits the newest processed
internal build to the selected external groups. It never withdraws the earlier
submission. The retry workflow accesses App Store Connect only after its code
has been promoted to `production`.

Validate the store workflow and review queue locally with
`node --test scripts/ci/mobile-store-workflow.test.js scripts/mobile-deployment/verify-store.test.mjs`.
