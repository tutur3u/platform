...
- `Live web dev process RSS` is the memory held by the running `next dev`
...
clear the generated dev cache:

```bash
rm -rf apps/web/.next/dev/cache/turbopack
```

If the next dev session is still stuck on stale generated state, remove the
whole local dev output:
