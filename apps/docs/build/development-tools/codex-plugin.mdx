...
- `$tuturuuu-cli`: native `ttr` CLI install, SDK-backed command work, browser
  ...
  open-task defaults, finance CRUD commands, compact task tables, and release
  verification guidance. It also covers scoped help such as `ttr finance
  --help`, `ttr finance transactions --help`, `ttr tasks --help`, `ttr tasks
  create --help`, quick status shortcuts such as `ttr tasks done` and `ttr
  tasks close`, `ttr upgrade`, and `ttr help tasks create`. It documents
  ttr-first task capture for requests to add, create, track, or split Tuturuuu
  tasks, including label discovery and closing superseded combined tasks after
  replacements are created. It documents autonomous Bun and CLI installation
  with `curl -fsSL
