...
- `$tuturuuu-e2e-auth-debugging`: local E2E authentication guidance for
  Playwright, `dev-session`, guest access, onboarding redirects, app-session
  verification, rate-limit tests, and Docker-backed local auth flows.
...
- `$tuturuuu-web-release`: `apps/web` release badge metadata, blue/green
  ...
  `TUTURUUU_PLATFORM_VERSION` bump guidance for every authored platform commit.
...
- dedicated prompt coverage for local E2E auth and dev-session debugging
