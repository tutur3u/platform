...
the journal shows a missing runner token on an existing installation, inspect
the wrapper in the Tuturuuu CLI config directory and make sure the token file is
sourced with automatic export enabled:

```sh
set -a
. "$HOME/.config/tuturuuu/devbox-runner.env"
set +a
```
...
After upgrading to a fixed CLI, `ttr box setup --agent --service` will generate
that wrapper shape for new or rotated registrations. Rerunning it registers a
new runner token, so revoke the old runner if you use setup as the repair path:
