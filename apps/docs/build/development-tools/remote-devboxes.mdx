...
`ttr box setup` is the default bootstrap for a runner host. It clones or reuses
`https://github.com/tutur3u/platform.git` at `~/Documents/tuturuuu`, installs
dependencies with `bun install --frozen-lockfile`, starts local Supabase with
`bun sb:start`, verifies `supabase status -o json`, and writes local Supabase
connection values into ignored `apps/*/.env.local` files. Secret values are
redacted from human and JSON output. Pass `--dir <path>` for a different
checkout and `--yes` only when the host should install detected missing
