...
<Info>
  **Prerequisite**: You should have read the [Monorepo
  Architecture](/build/development-tools/monorepo-architecture) page to understand the basic structure
  of the codebase.
</Info>
...
env:
  ...
  NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: ${{ secrets.PRODUCTION_SUPABASE_ANON_KEY }}
...
We automatically check for broken links in our documentation to ensure all references remain valid:
...
1. Triggers on documentation file changes (PRs and pushes to main)
2. Runs weekly to catch external link rot
3. Uses [lychee](https://github.com/lycheeverse/lychee) for fast, concurrent link checking
4. Checks all Markdown files and the Mintlify configuration
5. Comments on PRs when broken links are found
6. Can be triggered manually via workflow_dispatch

**Configuration**: Link checking behavior can be customized via `.lychee.toml` in the repository root. Common exclusions include:

- Local development URLs (`localhost`, `127.0.0.1`)
- Email and JavaScript links
- Example/placeholder domains
- Rate-limited external sites

#### Database Migration Validation

We validate database migrations to prevent common issues:
...
1. Migration files have proper names (not ending in `_new_migration.sql`)
2. New migrations are chronologically ordered after existing ones
3. Prevents common migration mistakes before they reach production
...
- `check-docs-links.yml`: Documentation link validation
