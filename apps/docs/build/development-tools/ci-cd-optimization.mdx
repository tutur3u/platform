...
CodeQL scans canonical `main` pushes, pull requests targeting `main` or
`production`, and the daily schedule. Production is fast-forwarded from `main`
by `bun git-sync`, so scanning the mirrored production push would analyze the
same SHA twice. CodeQL starts its analysis job directly and uses same-ref
concurrency; it does not allocate a switchboard preflight runner for an
unconditionally enabled security scan.
...
E2E consumers. A nightly run on `main` retains broad regression coverage, and
manual dispatch remains available for deliberate validation. E2E uses native
push paths and launches its image producer directly, without an extra static
switchboard job.
...
The result: a one-app change selects only the necessary production deploys,
