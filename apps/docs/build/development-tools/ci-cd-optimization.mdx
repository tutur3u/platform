...
layers. Docker setup and E2E jobs use shared GitHub BuildKit scopes per service;
all shards restore, while only the first default-branch shard exports. Expensive
web, TanStack, and backend images use `mode=max`; small leaf images use
`mode=min` or restore-only caching. Turbo values enter image builds only as
BuildKit secrets. Rust verify and deploy jobs share one pinned,
...
```text
48 Vercel deploy workflows
- 2 that actually deploy the touched app (preview + production)
= 46 deploy workflows skipped at the cheap check-ci gate
```
