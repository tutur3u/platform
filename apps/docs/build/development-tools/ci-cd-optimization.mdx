...
Environment and workflow-ref concurrency group. They accept `workflow_call` for
commit-driven planner jobs and retain `workflow_dispatch` for deliberate
operator reruns. A missing or untrusted marker fails open for that app without
forcing every other app's gate open.
