...
CodeQL is owned by GitHub's managed `dynamic/github-code-scanning/codeql`
workflow. Do not add a checked-in `.github/workflows/codeql.yml`: it would run
alongside the managed JavaScript/TypeScript and Python analysis for the same
SHA. `bun git-sync` mirrors the already-scanned `main` SHA to `production`, so
the repository keeps a single managed source of CodeQL runs.
