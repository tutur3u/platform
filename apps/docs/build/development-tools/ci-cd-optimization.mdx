...
CodeQL is owned by the checked-in `.github/workflows/codeql.yml`. It scans
JavaScript/TypeScript and Python on canonical `main` commits and pull requests,
with a manual fallback and no cron trigger. GitHub's legacy dynamic CodeQL
workflow must remain disabled; enabling both owners analyzes the same SHA
twice. `bun git-sync` mirrors the already-scanned `main` SHA to `production`,
so the custom workflow does not listen for production pushes.
