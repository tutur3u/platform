...
Web E2E runs must use the local Dockerized Supabase stack. The repo root and
`apps/web` both route `bun test:e2e` through
`scripts/run-web-e2e-docker.js`, which writes a temporary `tmp/e2e/web.env`,
starts and resets local Supabase, runs the production-style Docker web stack,
