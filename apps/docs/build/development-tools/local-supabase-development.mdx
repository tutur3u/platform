...
This creates a new empty migration file in `apps/database/supabase/migrations` that you can edit to include your desired schema changes.
...
Web E2E runs must use the local Dockerized Supabase stack. `apps/web` routes
`bun test:e2e` through `scripts/run-web-e2e-docker.js`, which writes a temporary
`tmp/e2e/web.env`, starts and resets local Supabase, runs the production-style
Docker web stack, and then runs Playwright.
...
1. Create a SQL file in `apps/database/supabase/tests` with your test cases
