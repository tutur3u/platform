...
3. Shared Satellite and Nova React Query clients use a 30-second stale window
   ...
   Versioned external-project asset reads bypass the global API proxy guard and
   rely on the asset route's publication/access checks, avoiding one middleware
   invocation per cache-busted media redirect. Mutations, unversioned reads, and
   WebGL delivery remain behind the global guard.
