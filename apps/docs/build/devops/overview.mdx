...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| Platform web app (`apps/web`)                                                                                                                                                         | `.github/workflows/vercel-preview-platform.yaml`, `.github/workflows/vercel-production-platform.yaml`   | Vercel build validation for on-premise deployment       |
| Satellite web apps (`apps/apps`, `apps/nova`, `apps/rewise`, `apps/calendar`, `apps/finance`, `apps/inventory`, `apps/meet`, `apps/tasks`, `apps/track`, `apps/shortener`, `apps/qr`) | `.github/workflows/vercel-*.yaml`                                                                       | Vercel preview and production deployments               |
...
- Vercel handles hosted satellite web deployments and platform build validation. Supabase migrations run as separate workflows; `main` drives staging schema promotion and `production` drives production schema promotion.
...
flowchart LR
    ...
    Production["Platform production build"]
...
- Production Redis in Docker now requires a token, but `scripts/docker-web.js`
  ...
  projects use the same integrated Docker Redis runtime by default unless the
  project has `redis_enabled=false`.
