...
| Migration web runtime (`apps/tanstack-web`)                                                                                                                                           | `apps/tanstack-web/Dockerfile`, `apps/tanstack-web/wrangler.jsonc`, `.github/workflows/rust-backend.yml` | Docker sidecar + Cloudflare Worker (`tuturuuu-tanstack-web`) — see [Web Docker Deployment](/build/devops/web-docker-deployment) |
| Rust backend (`apps/backend`)                                                                                                                                                         | `apps/backend/Dockerfile`, `apps/backend/wrangler.jsonc`, `.github/workflows/rust-backend.yml`          | Docker sidecar on port `7820` + Cloudflare Worker (`tuturuuu-backend`) — see [Web Docker Deployment](/build/devops/web-docker-deployment) |
...
- Vercel handles hosted satellite web deployments, platform preview build validation, and the hosted platform production deploy. Supabase migrations run as separate workflows; `main` drives staging schema promotion and `production` drives production schema promotion.
- Self-hosted web deployment is Docker-based, and blue/green rollout is the supported rebuild-before-restart path.
- The TanStack/Rust migration runs in parallel with `apps/web`: `apps/tanstack-web` (TanStack Start) and `apps/backend` (Rust, port `7820`) ship as Docker sidecars and Cloudflare Workers, validated by `.github/workflows/rust-backend.yml` and `bun check:cloudflare`. See [Web Docker Deployment](/build/devops/web-docker-deployment) and the [TanStack/Rust migration](/platform/architecture/tanstack-rust-migration) plan.
...
- The TanStack/Rust migration runtimes are now first-class deployment surfaces: `apps/backend` (Rust) builds a native binary, a Docker image, and a Cloudflare Worker bundle via `.github/workflows/rust-backend.yml`, and `apps/tanstack-web` deploys as a Cloudflare Worker (`tuturuuu-tanstack-web`) bound to the backend Worker (`tuturuuu-backend`).
