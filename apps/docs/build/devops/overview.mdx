...
| Migration web runtime (`apps/tanstack-web`)                                                                                                                                           | `apps/tanstack-web/Dockerfile`, `apps/tanstack-web/wrangler.jsonc`, `.github/workflows/rust-backend.yml` | Docker sidecar + manually dispatched Cloudflare Worker (`tuturuuu-tanstack-web`) with preflighted secrets — see [GitHub Actions Runbook](/build/devops/github-actions-runbook) |
...
- The TanStack/Rust migration runtimes are now first-class deployment surfaces: `apps/backend` (Rust) builds a native binary, a Docker image, and a Cloudflare Worker bundle via `.github/workflows/rust-backend.yml`, and `apps/tanstack-web` validates type-checks/tests plus deploys as a Cloudflare Worker (`tuturuuu-tanstack-web`) bound to the backend Worker (`tuturuuu-backend`). Cloudflare uploads are manual-dispatch only and preflight `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and Worker runtime secret names before deploying.
