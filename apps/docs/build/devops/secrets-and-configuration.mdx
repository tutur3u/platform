...
runs only from `production` pushes. Keep this environment branch-restricted to
`production`.
...
No Firebase, signing, store API, or `.env.github` contents should be stored in
GitHub Environment secrets or variables. Those resources live in the apps/web
mobile deployment vault at `/internal/infrastructure/mobile-deployment`, which
is root-workspace-only and requires `manage_workspace_secrets`.
...
- `.env.github` keys are managed as first-class rows in the vault. Expected
  production keys are `NEXT_PUBLIC_SUPABASE_URL`,
  `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `API_BASE_URL`,
  `TURNSTILE_SITE_KEY`, `TURNSTILE_BASE_URL`, `GOOGLE_WEB_CLIENT_ID`,
  `GOOGLE_IOS_CLIENT_ID`, `MOBILE_TASK_DESCRIPTION_EDITING_ENABLED`, and
  `MOBILE_CALENDAR_INTEGRATIONS_ENABLED`. Custom keys are allowed when they use
  uppercase `A-Z0-9_` names and single-line UTF-8 values.
- CI file payload keys such as `MOBILE_ANDROID_GOOGLE_SERVICES_JSON_B64` and
  `MOBILE_IOS_GOOGLE_SERVICE_INFO_PLIST_B64` are rejected from `.env.github`;
  upload those payloads through the file resources below.
- Android files: production `google-services.json`, upload keystore, and Google
  Play service-account JSON.
- Android scalar values: `ANDROID_KEYSTORE_ALIAS`,
...
- iOS scalar values: `APPLE_BUNDLE_ID=com.tuturuuu.app.mobile`,
...
- Watcher-managed Infrastructure projects use that integrated Docker Redis
  runtime by default too. Set `redis_enabled=false` on the project only when it
  should run without Redis; use `DOCKER_UPSTASH_REDIS_REST_URL` and
  `DOCKER_UPSTASH_REDIS_REST_TOKEN` for intentional Docker-host overrides.
