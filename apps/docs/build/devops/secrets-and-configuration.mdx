...
- Square Terminal (Inventory + Storefront pay-at-terminal checkout) uses
  `SQUARE_APPLICATION_ID`, `SQUARE_APPLICATION_SECRET`,
  `SQUARE_SANDBOX_APPLICATION_ID`, `SQUARE_SANDBOX_APPLICATION_SECRET`,
  `SQUARE_OAUTH_REDIRECT_URL`, `SQUARE_WEBHOOK_NOTIFICATION_URL`, and
  `SQUARE_WEBHOOK_SIGNATURE_KEY` for OAuth and webhook validation. Workspace
  access tokens, webhook keys, locations, and terminal ids are stored encrypted
  from Inventory settings. See the Square Terminal integration runbook for the
  complete setup and smoke checklist.
...
in GitHub Environment secrets or variables. Those resources live in the apps/web
mobile deployment vault at `/internal/infrastructure/mobile-deployment`, which
is root-workspace-only and requires `manage_mobile_deployment_vault`.
