...
- `.env.github` contents as key-value lines. Keys must be uppercase
  `A-Z0-9_`, values must be single-line UTF-8, and CI file payload keys such as
  `MOBILE_ANDROID_GOOGLE_SERVICES_JSON_B64` are rejected.
