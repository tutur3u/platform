...
- Docker web builds use `bun run build:web:docker`, which keeps the normal web
  ...
  the smaller of `DOCKER_WEB_BUILD_MEMORY` and Docker's reported memory limit.
  The helper reads Docker's `MemTotal` and forwards it as
  `DOCKER_WEB_DOCKER_MEMORY_LIMIT`; auto buckets never use more than half of
  that effective Docker memory budget once the machine is eligible for a larger
  bucket. Docker production builds default to webpack to avoid Turbopack memory
  spikes on smaller watcher hosts. The Docker `builder` stage is based on
  ...
  4096 MB are rejected. Set `DOCKER_WEB_NEXT_BUILD_ENGINE=turbopack` only on
  hosts with enough memory headroom for Turbopack. This avoids Bun runtime
  crashes while loading native Next SWC modules and avoids host OOM kills on
  smaller Docker Desktop allocations.
