...
| `bun test:e2e:web:docker` | Start local Supabase, reset it, run the production blue/green Docker web stack, then run Playwright |
...
`bun test:e2e` in `apps/web` now runs through
