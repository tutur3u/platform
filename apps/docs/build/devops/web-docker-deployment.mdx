...
profile and persists that profile for later runs. The retry also recreates the
Compose-owned BuildKit service even if the cleanup prune command itself fails
with the same transport signature, then recreates the remote Buildx builder if
