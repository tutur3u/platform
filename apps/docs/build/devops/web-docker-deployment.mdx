...
`ResourceExhausted`, or `cannot allocate memory`, the deploy retries once with
the next lower profile and persists that profile for later runs. The downgrade
ladder is:
...
- If the default blue/green deploy keeps failing with BuildKit transport EOFs
  or graceful-stop messages, leave the root-script defaults in place and let the
  adaptive profile step down automatically. Use explicit caps only when you want
  to bypass the remembered local profile for a one-off run.
