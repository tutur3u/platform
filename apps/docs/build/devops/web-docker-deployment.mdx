...
| `--build-builder-name platform` | Override the throttled Buildx builder name |
...
- `DOCKER_WEB_BUILD_BUILDER_NAME=platform`
...
- The helper starts the Compose-owned `buildkit` service and then creates or
  ...
  The container is named `${COMPOSE_PROJECT_NAME:-platform}-buildkit-1`, so it
  stays visually grouped under the `platform` Docker Desktop stack.
...
- During capped-build setup, the helper removes known legacy Buildx builders
  ...
  Compose-owned remote `platform` builder. If `docker buildx ls` still shows
...
- The watcher image must also include both the Docker Compose and Buildx CLI
  ...
  builds create/use the remote `platform` Buildx builder from inside the watcher
