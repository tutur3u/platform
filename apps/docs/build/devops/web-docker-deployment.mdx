...
- The **`apps/web` Dockerfile `deps` stage** retries `bun install
  ...
  `@biomejs/cli-linux-x64`), prune BuildKit state (`docker builder prune`) and
  retry; the blue/green helper may also restart the Compose-owned `buildkit`
  service on certain BuildKit cache errors during a deploy attempt.
...
- Docker web builds use `bun run build:web:docker`, which keeps the normal web
  build dependency graph, caps the web app's V8 heap at 4 GB instead of the
  local 8 GB build default, and still uses the Turbopack production build.
...
- If Bun fails during an image install with a tarball extraction error such as
  ...
  service, and retries the same build once. A second failure is recorded as a
  real deployment failure with the original command context preserved in logs.
