...
7. Immediately verifies the proxy can serve `/api/health` through the newly
   promoted color before retiring the old one.
8. Polls an internal drain-status endpoint on the old color and waits until it
   has no in-flight HTTP work left before retiring it. This keeps long-running
   server actions, route handlers, and other open requests from being cut off
   mid-flight.
...
10. Stops the old color after traffic has moved.
...
Cookie Too Large` before the active web container sees the request. It also
uses Docker DNS re-resolution plus a shorter keepalive timeout so promotions are
less likely to produce transient `502 Host Error` responses for existing
Cloudflare Tunnel connections, while the old color remains alive until its
in-flight work count reaches zero. The proxy now keeps both blue and green in
the nginx upstream group during steady state too, with the active color as the
primary upstream and the inactive color as a backup. That lets stale keepalive
workers fail over to the newly promoted color instead of returning `502` when
some clients or Cloudflare Tunnel connections are still pinned to older nginx
workers after a reload. The runtime DNS resolver is defined at the nginx
include/http scope, not just inside `server`, so Docker service-name resolution
continues to work for the blue/green upstream block at reload time.
...
11. The replacement watcher refreshes the live `web-proxy` nginx config and
   ...
   routing through `/api/health`, and only then starts the new blue/green
   build/promotion.
...
- Persists and renders the last 3 auto-deployments as stacked terminal cards
...
- Internal proxy health checks for `/api/health` are excluded from the request
  totals so the numbers reflect real served traffic more closely.
- When a new color takes over, the retired blue/green app container is stopped
  and removed instead of being left behind in an exited state.
