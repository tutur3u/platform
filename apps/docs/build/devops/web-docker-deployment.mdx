...
The managed workspace cron worker runs at
`/api/cron/workspaces/managed-jobs` through the `managed-workspace-cron-jobs`
entry. It claims due `workspace_cron_jobs` rows with endpoint URLs, but only for
workspaces that have the `MANAGED_CRON_ENABLED` workspace secret set to `true`.
That secret is intentionally special: create, update, and delete operations are
allowed only from an exact `@tuturuuu.com` employee session.

Endpoint egress is separately constrained by the private
`private.managed_cron_whitelisted_domains` allowlist. Manage that allowlist from
Infrastructure -> Managed Cron Domains; it is not shared with the AI whitelist.
Job endpoint URLs must use HTTPS, cannot include credentials, and must resolve
to an exact or subdomain match of an enabled allowlist domain. Per-job request
headers can be static values or references to `workspace_secrets` names; secret
values are read server-side at execution time and are not exposed in the job UI.
...
`[internal] waiting for connection`, the deploy retries once with the next lower
profile and persists that profile for later runs. The retry also restarts the
Compose-owned BuildKit service and recreates the remote Buildx builder if
`docker buildx inspect tuturuuu` reports `Status: inactive`. The downgrade
ladder is:
...
If the retry fails with the same BuildKit infrastructure signature, the helper
persists one more lower profile for the next run before surfacing the failure.
Delete `tmp/docker-web/buildkit/resource-profile.json` to reset back to the
default profile. Explicit build cap flags or `DOCKER_WEB_BUILD_MEMORY`,
`DOCKER_WEB_BUILD_CPUS`, or `DOCKER_WEB_BUILD_MAX_PARALLELISM` opt out of the
adaptive profile for that run.
...
- If the default blue/green deploy keeps failing with BuildKit transport EOFs,
  ...
  adaptive profile step down automatically. Use `docker buildx ls` or
...
- After a watcher-owned child deploy command fails, the watcher prunes
  ...
  to preserve failed build layers for debugging.
