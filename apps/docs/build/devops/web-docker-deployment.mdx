...
```bash
bun serve:web:docker:bg -- --build-memory 8g --build-cpus 2 --build-max-parallelism 1
```

Current root-script defaults:

- `bun serve:web:docker` defaults to `--build-memory 8g --build-cpus 2 --build-max-parallelism 1`
- `bun serve:web:docker:bg` defaults to `--build-memory 8g --build-cpus 2 --build-max-parallelism 1`

The Compose-owned BuildKit service also defaults `mem_limit` to **8g** when
`DOCKER_WEB_BUILD_MEMORY` is unset, which reduces swap thrash on shared hosts
that still run the web stack, Redis, log drain, and other sidecars alongside
builds. Raise the cap on dedicated builders if you have spare RAM.
