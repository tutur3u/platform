...
- The production and TanStack-dual Compose files accept restore entries through
  `DOCKER_WEB_CACHE_<SERVICE>_FROM` and optional exporter entries through the
...
through the private `ghcr.io/tutur3u/platform-e2e` package. The producer and
test jobs start after the same CI switchboard check, so shards can restore
Supabase, Bun, and Playwright while the image bundle is being prepared. The
producer publishes immutable tags under
`<run-id>-<run-attempt>-<commit-sha>-<service>` and pushes the `-ready` tag only
after every planned blue/green and TanStack image exists and the package has
been verified private. Consumers wait up to
`E2E_IMAGE_BUNDLE_WAIT_SECONDS` (six minutes by default), pull the complete
bundle, retag it for their isolated Compose project, and skip web/support image
builds. Missing, late, or partial bundles emit a notice and retain the existing
local cache-backed build path.
...
production release metadata remains unchanged. An `always()` cleanup job
deletes the exact run prefix after all shards and migration modes finish, and
the next producer removes abandoned bundle versions older than 24 hours to
cover canceled workflows. Bundle publication is best-effort, but cleanup is a
required storage-safety gate. Investigate package visibility, `packages` token
permissions, or GHCR deletion errors when cleanup fails; do not make the package
public or replace these images with long-retention Actions artifacts.
...
container: <id>` when a dependency was recreated between Docker's dependency
