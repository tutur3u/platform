...
6. Refreshes support services (`markitdown`, `storage-unzip-proxy`, and
   `web-cron-runner`) after web/Hive target work. Their build step is scoped:
...
Infrastructure Monitoring → Deployments reads `target-state.json` and the
watcher deployment history together, so operators can see a partial promotion
such as web serving a new commit while Hive remains blocked on migration.
Rows recorded before staged deployment tracking do not invent queued stages;
their stage chips are shown as not applicable. Active watcher deployments that
only have pending build/deploy status are surfaced with a synthetic current
stage so operators can see the build is in progress before full stage history is
written.
After each Hive migration pass, the deploy helper removes the completed
`hive-db-migrate` Compose container. This keeps the one-shot migration service
from accumulating as an exited container after `depends_on` starts it while Hive
services come up.
...
- Blue/green watcher handoffs preserve the Compose-owned BuildKit cache by
  default (`DOCKER_WEB_BUILDKIT_PRUNE_AFTER_BUILD=0` and
  `DOCKER_WEB_BUILDKIT_STOP_AFTER_BUILD=0`) so the next staged retry can reuse
  layers instead of rebuilding already successful targets. Set either variable
  to `1` only when an operator intentionally wants a post-handoff cleanup.
