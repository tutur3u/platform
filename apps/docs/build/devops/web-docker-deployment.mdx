...
15. When the locked branch is `production`, the watcher also evaluates
    ...
    fast-forward `main` candidate on the standby lane before CI is green when
    GitHub checks are inspectable, then updates `production` to `main` only
    after the `main` commit is at least 10 minutes old and every latest GitHub
    check run/status is non-failing with at least one CI signal. The promotion
    writes `tmp/docker-web/watch/blue-green-production-promotion.state.json`
    with the main/production SHAs, age gate, CI summary, prebuild state, next
    evaluation time, and latest decision. Missing `GITHUB_TOKEN`, missing
    GitHub repository metadata, dirty worktrees, deployment locks, divergent
...
16. Infrastructure operators can queue
    ...
    a non-forced GitHub `production` ref update, requires a fast-forward
    ancestry relationship, and leaves normal post-switch blue/green health
    checks in the deploy path.
