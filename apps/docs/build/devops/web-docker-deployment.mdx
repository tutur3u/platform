...
- On **Linux**, the helper compares `/proc/<pid>/cmdline` to the recorded lock
  so a reused PID after a crash cannot masquerade as an in-flight deploy. When
  no `web-proxy` / `web-blue` / `web-green` containers exist, the auto-deploy
...
- The **`apps/web` Dockerfile `deps` stage** retries `bun install
  ...
  If tarball extraction still fails (for example `@biomejs/cli-linux-x64`),
  prune BuildKit state (`docker builder prune`) and retry the deploy; the
  blue/green helper may also restart the Compose-owned `buildkit` service on
  certain BuildKit cache errors during a deploy attempt.
