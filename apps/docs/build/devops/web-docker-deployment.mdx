...
4. Starts the shared `markitdown`, `storage-unzip-proxy`, and
   `web-cron-runner` helper containers alongside the target color. The build
   step is scoped separately: ordinary web commits build only `web-blue` or
   `web-green`, while Hive and helper images are rebuilt only when their source,
   Dockerfile, compose wiring, or shared dependency inputs changed. Image-only
   services such as `redis`, `serverless-redis-http`, `web-proxy`, and
   `cloudflared` are never passed to `docker compose build`.
