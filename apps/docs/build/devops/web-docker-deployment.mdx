...
11. Runs `bun serve:web:docker:bg` automatically after a successful fast-forward
   pull.
12. If `scripts/watch-blue-green-deploy.js` itself changed in the pulled
...
14. Retries recoverable Git command failures instead of exiting. The first retry
    waits 1 minute, then the watcher backs off exponentially on consecutive Git
    failures up to a 15 minute ceiling.
15. Stops immediately if the checked-out branch changes while the watcher is
    running.
16. If another watcher already owns the lock, a new invocation can fail with
