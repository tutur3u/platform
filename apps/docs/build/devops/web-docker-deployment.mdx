...
builds. Consumer jobs use `always()` at the job boundary, so a failed
best-effort producer still starts the existing local cache-backed build path.
...
production release metadata remains unchanged. Published images also carry the
official `org.opencontainers.image.source` label so GHCR links the package to
this repository and grants its workflow token package administration. The
...
The producer also maintains one mutable `cache-<service>` tag per planned image.
...
by the 24-hour stale sweep. Cache images are pushed with bounded concurrency,
then each immutable run tag is published from the same local image; the ready
marker remains last, so consumers never observe a partial bundle. These
registry-layer anchors complement Turbo and BuildKit caches without adding
Actions cache entries or artifact storage.
