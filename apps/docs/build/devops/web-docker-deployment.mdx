...
- Dockerized production commands generate `BACKEND_INTERNAL_TOKEN` when one is
  ...
  fallback token.
- The production stack runs Supermemory as an internal support service at
  `http://supermemory:8787`. Set `SUPERMEMORY_IMAGE` to the official enterprise
  self-host image or an internal image built from the enterprise deployment
  package. The wrapper Dockerfile only retags that image for the local
  blue/green bake flow; it does not vendor the public Supermemory repository.
- Dockerized production commands auto-configure Supermemory unless explicitly
  ...
  `BETTER_AUTH_SECRET`, injects `SUPERMEMORY_DATABASE_URL`, and defaults
  `SUPERMEMORY_ENABLED=true`, `SUPERMEMORY_FAIL_OPEN=true`, and
...
- Operators can override generated values with `DOCKER_SUPERMEMORY_API_KEY`,
  ...
  `DOCKER_SUPERMEMORY_BETTER_AUTH_SECRET`, or
  `DOCKER_SUPERMEMORY_ENABLED`; standard `SUPERMEMORY_*` and
  `BETTER_AUTH_SECRET` env still work. Provider keys such as `OPENAI_API_KEY`
  or `GEMINI_API_KEY` and the approved `SUPERMEMORY_IMAGE` remain supplied by
  the deployment package/operator.
- Blue/green promotion health-gates `supermemory` with the rest of the support
  services. Changing `apps/supermemory/Dockerfile`, the production Compose
  fragments, or the Docker bake file refreshes the support service set.
  ...
  for local-only runs that do not have the approved enterprise image.
...
`SUPERMEMORY_ENABLED=false`; the memory integration is not under test there, and
GitHub-hosted runners do not have access to the private enterprise Supermemory
image.
...
`tmp/docker-web/supermemory-postgres-password`, and
`tmp/docker-web/supermemory-better-auth-secret`. Override them with
