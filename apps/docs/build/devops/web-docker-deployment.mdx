...
4. registers the Portless `https://tuturuuu.localhost` route only after the
   direct Docker proxy is healthy,
5. waits for `https://tuturuuu.localhost/login`, then runs Playwright against
   that shared-cookie origin, and
...
Hive, proxy, and support services, the Portless route list, a
`https://tuturuuu.localhost/login` probe, and `bun sb:status`. The workflow also
has a failure-only diagnostic step after `Run Playwright shard` as a backstop,
so the job output should show the failing service or stage even when Playwright
report artifacts are incomplete. The workflow uploads diagnostics, Playwright
reports, and `apps/web/test-results` for every non-cancelled shard so traces and
