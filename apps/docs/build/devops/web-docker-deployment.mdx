...
through a same-origin WebGL package proxy route before the server writes to
Supabase Storage or R2, so CMS editors do not depend on object-storage CORS
settings for ZIP uploads.
