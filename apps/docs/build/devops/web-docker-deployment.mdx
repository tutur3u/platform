...
- After each blue/green deployment handoff finishes, the helper runs
  ...
  removed before the command exits. The cleanup then stops the Compose-owned
  `buildkit` service by default so it does not keep CPU or memory reserved
  while no deployment is building. Set
