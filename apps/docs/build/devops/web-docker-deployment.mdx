...
- The watcher also treats a build lock older than 30 minutes as a timed-out
  build. It sends `SIGTERM` to the recorded deployment PID, clears the lock,
  records a failed deployment history row with the timeout reason, and waits
  until the next polling cycle before retrying. Override the window with
  `DOCKER_WEB_WATCHER_BUILD_TIMEOUT_MS`; set it to `0` only when an operator
  explicitly wants to disable watcher-side build termination.
...
12. Treats a worktree that is dirty only because of `bun.lock` as watchable, so
...
- If `git pull --ff-only` fails only because `.git/index.lock` already exists,
  the watcher now inspects the lock age and removes it automatically only when
  it is stale (older than 2 minutes). Fresh lock files are left in place so
  active Git operations are not interrupted.
