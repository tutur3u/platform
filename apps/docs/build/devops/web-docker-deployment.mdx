...
the same command until the build succeeds or the floor profile fails. Each
downgrade persists the selected profile for later runs, recreates the
...
signature, the helper leaves the machine-local profile at `floor` and surfaces
the failure. Delete `tmp/docker-web/buildkit/resource-profile.json` to reset
back to the default profile. Explicit build cap flags or
...
- If BuildKit fails with `ResourceExhausted`, `cannot allocate memory`, or exit
  ...
  is the relevant build budget. Prefer the default `--build-memory auto` path so
  the cap follows Docker's configured allocation and the adaptive profile can
  lower pressure automatically. Raise `DOCKER_WEB_BUILD_MEMORY`/`--build-memory`
  only when `docker info` shows enough Docker memory and host swap is not
  saturated.
