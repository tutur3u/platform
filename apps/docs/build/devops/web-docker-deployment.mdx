...
14. If compose or helper-image wiring changed, including
    `docker-compose.web.prod.yml`, `apps/discord/Dockerfile.markitdown`, or
    `apps/storage-unzip-proxy/Dockerfile`, the containerized watcher recreates
    its own compose service before the pending deploy handoff. That lets a
    running watcher pull this update and then launch `markitdown` plus
    `storage-unzip-proxy` through the refreshed blue/green workflow.
