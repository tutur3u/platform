...
- The dependency stages in `apps/web/Dockerfile`, `apps/hive/Dockerfile`,
  ...
  parity. `apps/backend` is an independent Go module, and `apps/meet-realtime`
...
Watcher-managed Infrastructure projects use the same integrated Docker Redis
runtime by default. The watcher generates or reuses `tmp/docker-web/redis-token`
and injects `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, and
`SRH_TOKEN` for managed project compose runs unless the project explicitly sets
`redis_enabled=false`. Generic host `UPSTASH_REDIS_REST_*` values are ignored
there too; use the Docker-specific override variables when a managed project
...
15. When the locked branch is `production`, the watcher also evaluates
    `main` for production promotion every 5 seconds. It prebuilds the newest
    fast-forward `main` candidate from a disposable detached worktree under
    `tmp/docker-web/worktrees/production-prebuild/<mainShortHash>`, copies the
    effective production `.env.local` layout into that clone, and keeps watcher
    runtime/control/history files in `tmp/docker-web/watch`. The production
    checkout stays on `production`; the disposable prebuild worktree is removed
    after success or failure, and watcher-owned stale prebuild worktrees older
    than the build timeout plus one hour are swept on startup and before new
    prebuilds. The watcher then updates `production` to `main` only after the
    `main` commit is at least 10 minutes old and every latest GitHub check
    run/status is non-failing with at least one CI signal. The promotion writes
    `tmp/docker-web/watch/blue-green-production-promotion.state.json` with the
    main/production SHAs, age gate, CI summary, prebuild state, prebuild
    duration/failure details, next evaluation time, and latest decision.
    Missing CI inspection, dirty worktrees, deployment locks, divergent
    `production`, or an active rollback pin block automatic promotion.
    Build-lock conflicts during the prebuild or standby-refresh phase are
    recorded as a `deployment-active` deferral instead of a failed deployment
    attempt, so active Docker work does not poison retry limits.
16. Infrastructure operators can queue
    `tmp/docker-web/watch/control/blue-green-production-promote.request.json`
    from the deployments dashboard to promote `main` immediately. Manual
    promotion bypasses only the CI-green and 10 minute age gates; it still uses
    a non-forced update, requires a fast-forward ancestry relationship, and
    leaves normal post-switch blue/green health checks in the deploy path. Once
    those Git safety checks pass, the request is an operator-authorized override
    that cancels any active blue/green build before updating `production`; it
    also clears an active rollback pin as operator intent to resume production.
    When `GITHUB_TOKEN` is configured, the watcher updates the GitHub ref
    through the API; otherwise it uses the host's configured Git credentials with
    `git push <remote> <mainHash>:refs/heads/production`.
17. Infrastructure operators can queue
    ...
    cached revert verifies the cached target first, supersedes any queued
    manual production promotion request, cancels any active blue/green build,
    ...
    `instant-revert`, and writes a deployment pin so automatic promotion does
    not immediately overwrite the rollback. Older retained deployments remain
