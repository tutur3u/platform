...
`https://<app>.tuturuuu.localhost:1355` routes through the shared Portless proxy
...
- `apps/web` and `apps/meet-realtime` must share the same
  ...
  both to `wss://meet-realtime.tuturuuu.com/realtime` for browser join-token payloads.
- Browsers connect to `wss://meet-realtime.tuturuuu.com/realtime?token=...`.
...
Calendar jobs run every 15 minutes (provider sync) and six hours (smart
scheduling) from `apps/web/cron.config.json`. The Docker scheduler sends these
two fixed paths to the infrastructure app's `/api/cron/calendar/:job` gateway;
the old web-origin paths no longer have handlers. Other jobs continue using
`INTERNAL_WEB_API_ORIGIN`.

The infrastructure gateway authenticates `CRON_SECRET` (or `VERCEL_CRON_SECRET`)
before loading the existing encrypted `mobile_calendar_gateway:v1` credential.
It forwards only provider sync and smart scheduling to the fixed Calendar origin,
attaching the private hosting header and server cron authentication. Deploy the
infrastructure gateway before promoting the scheduler change. The scheduler,
infrastructure, and Calendar must share cron authentication configuration.
No new hosting credential is copied into scheduler environments or mobile builds.
Missing credentials fail closed; redirects are never followed with credentials.
Verify actual successful job executions after rollout, rather than relying on
the next-run estimate. The gateway has a four-minute upstream deadline and a
bounded JSON response; failures remain visible to the scheduler.

The Calendar cron handler invokes its local workspace sync handler directly.
That handler selects active Google and Microsoft connections, refreshes provider
credentials on the server, and persists events independently of any open client.
Background privileges are derived from verified cron authorization, never the
JSON `source` field. Calendar should not also be scheduled through Trigger.dev.
Microsoft OAuth connections now persist the account-bound refresh credential
before their request-local MSAL cache is discarded. Existing Microsoft rows with
an empty `refresh_token` need one reconnect: the old callback stored no durable
credential, so a background job cannot recover it. Token refresh preserves the
calendar write scopes needed for invitations. Never log the serialized MSAL
cache, access tokens, refresh credentials, or provider sync tokens.

Mobile renders its cached snapshot immediately and revalidates on opening or
resuming Calendar; this fetch does not initiate provider synchronization.
