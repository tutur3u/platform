...
| `--build-cpus 8` | Run builds through a capped Buildx builder with an approximate CPU limit |
...
```bash
bun serve:web:docker:bg -- --build-memory 12g --build-cpus 8 --build-max-parallelism 1
```

Current root-script defaults:

- `bun serve:web:docker` defaults to `--build-memory 12g --build-cpus 8 --build-max-parallelism 1`
- `bun serve:web:docker:bg` defaults to `--build-memory 12g --build-cpus 8 --build-max-parallelism 1`

The Compose-owned BuildKit service also defaults `mem_limit` to **12g** and
`cpus` to **8** when `DOCKER_WEB_BUILD_MEMORY` and `DOCKER_WEB_BUILD_CPUS` are
unset, which balances large monorepo builds with shared hosts that still run the
web stack, Redis, log drain, and other sidecars alongside builds. Raise or lower
the caps with env vars or helper flags when your machine is tighter or has spare
capacity.
...
```bash
bun serve:web:docker:bg -- --build-memory 16g --build-cpus 8 --build-max-parallelism 2
```
...
- `DOCKER_WEB_STATIC_GENERATION_MAX_CONCURRENCY=8`
- `DOCKER_WEB_NEXT_BUILD_CPUS=8`
...
- The same max-parallelism value is also forwarded as `COMPOSE_PARALLEL_LIMIT`
  when that variable is not already set, so Compose does not build multiple
  services at once under the capped builder.
- Docker web builds use `bun run build:web:docker`, which keeps the normal web
  build dependency graph, caps the web app's V8 heap at 8 GB (same
  `--max-old-space-size` target as local `build`), and still uses the Turbopack
  production build. The Docker `builder` stage is based on
  `node:22-bookworm-slim` and copies the Bun binary in only for workspace script
  orchestration, so the actual `next build` process runs under real Node instead
  of Bun's `node` shim. This avoids Bun runtime crashes while loading native
  Next SWC modules.
- Docker standalone Next builds default static page generation to a 180 second
  timeout, static generation concurrency capped at 8, and Next build CPU count
  capped at 8. This avoids false `Failed to build ... because it took more than 60
  seconds` failures when the same host is also running blue/green sidecars and
  BuildKit. Override those with `DOCKER_WEB_STATIC_PAGE_GENERATION_TIMEOUT`,
