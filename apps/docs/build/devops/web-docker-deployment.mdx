...
bearer-token headers to the upload URL. The web proxy intentionally lets the
...
routes still validate the shared unzip token before creating folders or signing
file uploads. The proxy currently buffers the downloaded archive and each
extracted file in memory, so the default caps stay conservative: 100 MiB ZIP
downloads, 50 MiB per extracted file, and 250 MiB total extracted output.
