...
3. Polls the tracked upstream every `5000ms`.
4. Skips pulls if the worktree is dirty.
5. Uses `git pull --ff-only` only when the local branch is strictly behind the
   locked upstream.
6. Runs `bun serve:web:docker:bg` automatically after a successful fast-forward
   pull.
7. Stops immediately if the checked-out branch changes while the watcher is
   running.
