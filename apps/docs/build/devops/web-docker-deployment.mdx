...
Security-sensitive one-time state such as CLI refresh-token replay protection
still fails closed until Redis is restored or the CLI user signs in again.
