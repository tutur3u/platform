...
8. Waits through a short drain window before stopping the old color so
   long-lived upstream connections from Cloudflare Tunnel or browsers can roll
   over to the reloaded proxy workers cleanly.
9. Stops the old color after traffic has moved.
