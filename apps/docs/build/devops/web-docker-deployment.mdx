...
override it with `TUTURUUU_CI_CHECK_NAME`. The publisher uses
`TUTURUUU_CI_GITHUB_TOKEN` first, then `GITHUB_TOKEN`, and the token must be
able to create and update Check Runs for the repository. The watcher stores the
latest check-run id per commit in
...
16. Infrastructure operators can queue
    ...
    leaves normal post-switch blue/green health checks in the deploy path. When
    `GITHUB_TOKEN` is configured, the watcher updates the GitHub ref through the
    API; otherwise it uses the host's configured Git credentials with
...
17. Infrastructure operators can queue
    ...
    cached revert retags the selected image, starts active/standby with
    `--no-build`, health-checks through the normal proxy path, records
    deployment kind `instant-revert`, and writes a deployment pin so automatic
    promotion does not immediately overwrite the rollback. Older retained
    deployments remain revertable through the existing rollback pin path, which
    may rebuild because no cached image is available.
