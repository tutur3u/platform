...
- `log-drain-postgres` stores deployment telemetry only. Blue/green startup
  still starts it and retries once after removing only the service container,
  but an unhealthy log-drain database no longer blocks web promotion by
  default. The helper continues with `PLATFORM_LOG_DRAIN_ENABLED=false`, prints
  the recent service state/log diagnostics, and leaves the
  `platform-log-drain-postgres` volume intact. Set
...
- The TanStack migration services use `apps/tanstack-web/Dockerfile`, direct
  ...
  proxy-routing experiment. Production TanStack services wait on the Rust
  ...
  The TanStack image and production Compose healthchecks also require the root
  readiness page to include `Backend reachable`, so a status-only shell cannot
  satisfy Docker cutover readiness when the Rust backend is unreachable.
  Use `DOCKER_WEB_FRONTEND=next|tanstack` to select the default web frontend in
  migration-aware scripts. Cloudflare preview deploys use
...
2. Deploy the Rust backend Worker first:
   ...
   Discord cron proxy preview readiness, and `AURORA_EXTERNAL_URL` is required
   for the Rust-owned Aurora health probe; configure them even when the first
   smoke target is only `/healthz`.
...
- If `docker compose up` hits a transient Docker registry or Docker Hub auth
  ...
  fail immediately.
- If `log-drain-postgres` starts but remains unhealthy before blue/green
  promotion, the helper removes and recreates only that service container once,
  collects `docker compose ps`, container inspect state, recent service logs,
  and matching Compose volume names, then continues the rollout with
  `PLATFORM_LOG_DRAIN_ENABLED=false` for that run. The production `web` and
  `web-blue-green-watcher` services keep start-order dependencies on
  `log-drain-postgres`, but the dependency is `required: false` and does not
  wait for `service_healthy`; platform traffic can promote while telemetry and
  Infrastructure project automation are degraded.
