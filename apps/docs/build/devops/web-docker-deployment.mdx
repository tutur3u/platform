...
- `DOCKER_WEB_STATIC_GENERATION_MAX_CONCURRENCY=4`
- `DOCKER_WEB_NEXT_BUILD_CPUS=4`
- `DOCKER_WEB_NODE_MAX_OLD_SPACE_SIZE=4096`
...
- Docker web builds use `bun run build:web:docker`, which keeps the normal web
  build dependency graph, caps the Docker-only web app V8 heap at 4 GB, and
  still uses the Turbopack production build. The Docker `builder` stage is based
  on `node:24-bookworm-slim` and copies the Bun binary in only for workspace
  script orchestration, so the actual `next build` process runs under real Node
  instead of Bun's `node` shim. The `@tuturuuu/web` `build:docker` script
  delegates to `scripts/run-web-docker-next-build.js`, which spawns
  `DOCKER_WEB_NODE_BINARY` (pinned by the Dockerfile to `/usr/local/bin/node`)
  for the Next CLI and honors `DOCKER_WEB_NODE_MAX_OLD_SPACE_SIZE` when a larger
  Docker host can safely allocate more heap. This avoids Bun runtime crashes
  while loading native Next SWC modules and avoids host OOM kills on smaller
  Docker Desktop allocations.
- Docker standalone Next builds default static page generation to a 180 second
  timeout, static generation concurrency capped at 4, and Next build CPU count
  capped at 4. This avoids false `Failed to build ... because it took more than 60
