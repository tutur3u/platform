...
Watcher startup also keeps the cron runner present. `bun serve:web:docker:bg`,
`bun serve:web:docker:bg:watch`, and watcher recovery recreate or resume
`web-blue-green-watcher` first, then ensure `web-cron-runner` exists with a
no-recreate Compose start. A healthy existing cron runner is left running; a
missing runner fails watcher bootstrap instead of leaving the stack without
native cron execution.
...
If a managed-cron status or setup request still returns HTTP 500 after the cron
runner is available, check the Tuturuuu managed-cron prerequisites first:
`PLATFORM_DATABASE_URL` or an equivalent private database URL must be present,
and the managed-cron migrations must have created `workspace_cron_jobs` plus
`private.managed_cron_whitelisted_domains`.
...
exhausted. Each downgrade persists the selected profile for later runs, skips
fixed profiles that exceed the effective Docker memory budget, recreates the
...
`docker buildx inspect tuturuuu` reports `Status: inactive`. The profile ladder
is:
...
budget-derived `default` profile before surfacing the failure. This prevents a
machine from getting stuck starting every future deploy at `floor`. Delete
`tmp/docker-web/buildkit/resource-profile.json` only as an emergency manual
reset back to the default profile. Explicit build cap flags or
...
3. Builds and starts `web-cron-runner` with `--no-recreate` so native Docker
   cron stays available without interrupting a healthy runner.
4. Tails the watcher container's live logs so the terminal still shows the
   watcher dashboard.
