...
The MarkItDown endpoint accepts exactly one conversion source per request:
either a signed Storage URL for uploaded files or a direct HTTPS YouTube URL in
the `url` field. Direct URL conversion is intentionally limited to YouTube
hosts, so Mira can summarize YouTube links through MarkItDown without falling
back to Google Search or broad URL fetching.
