...
two fixed paths directly to `https://calendar.tuturuuu.com`; the old web-origin
paths no longer have handlers. Other jobs continue using
`INTERNAL_WEB_API_ORIGIN`.

Provision `CALENDAR_CRON_GATEWAY_SECRET` in the scheduler's server environment
from the infrastructure vault's `mobile_calendar_gateway:v1` credential. It is
sent only to the two fixed Calendar paths as `x-tuturuuu-calendar-gateway`,
allowing the existing narrow hosting rule. `CRON_SECRET` (or
`VERCEL_CRON_SECRET`) must also match Calendar's server configuration. Neither
credential belongs in mobile builds. Missing hosting credentials fail closed;
redirects are never followed with credentials. Verify actual successful job
executions after rollout, rather than relying on the next-run estimate.
