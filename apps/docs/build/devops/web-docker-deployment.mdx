...
1. writes `tmp/e2e/web.env` with local-only Supabase, app-session JWT values,
   and a local-only E2E auth bypass for Turnstile/dev-session,
...
cloud Supabase project. The auth bypass is guarded by the local E2E web and
Supabase origins before server-side auth code honors it, so it must not be used
as a general production configuration.
...
4. Builds and starts the shared `markitdown`, `pronunciation-assessor`,
   `storage-unzip-proxy`, and `web-cron-runner` helper containers alongside the
   target color.
...
The same values can be updated from Infrastructure Monitoring in the web
dashboard. The dashboard writes
...
- The **`apps/web` Dockerfile `deps` stage** retries `bun install
  ...
  after a `git pull`, run `bun install` at the repo root on the host (or in the
  same checkout the image build uses) so `bun.lock` matches `package.json`, then
  retry the deploy. If tarball extraction still fails (for example
...
- After each blue/green deployment handoff finishes, the helper runs
  `docker buildx prune --builder <builder> --all --force` against the active
  Compose-owned builder so the `platform-buildkit-state` volume does not keep
  multi-gigabyte build cache indefinitely. The CLI runs this cleanup again
  after starting the watcher container, so watcher-image build cache is also
  removed before the command exits. The cleanup then stops and removes the
  Compose-owned `buildkit` service by default so it does not keep CPU or memory
  reserved or appear as a stopped container in the `tuturuuu` Docker Desktop
  stack while no deployment is building. Set
  `DOCKER_WEB_BUILDKIT_PRUNE_AFTER_BUILD=0` only when you are intentionally
  preserving BuildKit cache for repeated local diagnostics, and set
  `DOCKER_WEB_BUILDKIT_STOP_AFTER_BUILD=0` only when you intentionally want to
  keep the builder warm between back-to-back deploys.
- The same max-parallelism value is also forwarded as `COMPOSE_PARALLEL_LIMIT`
  ...
  workflow builds each service image in its own Compose build call so image
  export and web compilation do not overlap on memory-constrained hosts.
...
- Docker web builds use `bun run build:web:docker`, which keeps the normal web
  ...
  4 GB, and auto-scales the heap to 4 GB, 6 GB, 8 GB, 12 GB, or 16 GB based on
  Docker's reported memory limit when available, falling back to
  `DOCKER_WEB_BUILD_MEMORY` for environments where Docker memory cannot be
  detected. The helper reads Docker's `MemTotal` and forwards it as
  `DOCKER_WEB_DOCKER_MEMORY_LIMIT`; auto buckets reserve 4 GB of Docker memory
  for BuildKit, the active runtime lane, and sidecar overhead before selecting
  the Node heap bucket. Docker production builds default to Turbopack under the
  real Node 24 runtime with App Router-only compilation and React
  Compiler disabled. This avoids Bun runtime crashes while loading native Next
  SWC modules and avoids Webpack heap exhaustion on smaller watcher hosts. The
  Docker `builder` stage is based on
  ...
  4096 MB are rejected. Set `DOCKER_WEB_NEXT_APP_ONLY=0`,
  `DOCKER_WEB_REACT_COMPILER=1`, or `DOCKER_WEB_NEXT_BUILD_ENGINE=webpack` only
  on hosts with enough memory headroom for those build paths. Keep the default
  Turbopack path for local Docker Desktop allocations and watcher hosts unless
  Webpack-specific diagnostics are needed.
...
- Hive Docker images use a filtered workspace install and a Next standalone
  ...
  `tmp/**`, `apps/mobile/build/**`, and `apps/backend/target/**`; otherwise
  previous local builds can be copied into the next Docker context and inflate
  small sidecar images by several gigabytes.
...
10. Uses `git pull --ff-only` only when the local branch is strictly behind the
   locked upstream.
11. Runs `bun upgrade` and then `bun i` automatically after every successful
   fast-forward pull so the host Bun runtime and installed dependencies match
   the fetched revision before the deploy handoff continues. If Bun's upgrade
   endpoint returns `HTTPForbidden`, the watcher logs a warning and continues
   with `bun i`; dependency sync is required, but a blocked self-upgrade should
   not trap the watcher in a restart loop.
12. Treats a worktree that is dirty only because of `bun.lock` as watchable, and
    if a fast-forward pull says that only this generated lockfile would be
    overwritten, restores `bun.lock` from `HEAD` and retries the pull. This keeps
    host-side install churn from blocking subsequent polling cycles.
...
15. The replacement watcher refreshes the live `web-proxy` nginx config and
...
14. If compose or helper-image wiring changed, including
    `docker-compose.web.prod.yml`, MarkItDown service files,
    `apps/pronunciation-assessor` source files, or `apps/storage-unzip-proxy`
    package/source files, the containerized watcher recreates its own compose
    service before the pending deploy handoff. That lets a running watcher pull
    this update and then refresh `markitdown`, `pronunciation-assessor`, and
...
16. Caps deployment attempts at 3 failures per commit. A recovered pending
...
17. Stops immediately if the checked-out branch changes while the watcher is
    running.
18. If another watcher already owns the lock, a new invocation can fail with
...
- Hive is promoted with the web blue/green color: `hive-blue` and `hive-green`
  ...
  **Deploy coordination:** `scripts/docker-web/blue-green.js` still builds every
  ...
  first the target `web-{color}`, MarkItDown, storage-unzip-proxy,
...
- Host dependency refreshes can rewrite `bun.lock`; the watcher ignores that
  file as a sole dirty-worktree signal and auto-restores it before retrying a
  fast-forward pull when Git reports that only `bun.lock` would be overwritten.
  This keeps polling alive after the automatic `bun upgrade` / `bun i` step
  without discarding unrelated local edits.
- A recoverable `bun upgrade` failure such as `HTTPForbidden` does not abort the
  deployment watcher. The watcher continues with `bun i` because the
  containerized watcher already pins its Bun runtime in the Dockerfile.
...
- When the watcher receives a shutdown signal while it is temporarily detached,
  ...
  the worktree is clean except for generated `bun.lock` changes. This keeps
  manual operator commands such as `git pull && bun serve:web:docker:bg` from
  inheriting a detached checkout after a stopped watcher.
