...
- Supermemory needs `SUPERMEMORY_API_KEY`, `SUPERMEMORY_DATABASE_URL` or the
  bundled `supermemory-postgres` pgvector database, `SUPERMEMORY_ENABLED=true`,
  and provider keys such as `OPENAI_API_KEY` or `GEMINI_API_KEY` depending on
  the deployment package configuration. Keep `SUPERMEMORY_BASE_URL` internal
  unless you intentionally operate a separate managed endpoint.
