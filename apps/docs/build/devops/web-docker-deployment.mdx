...
- Production `web`, `web-blue`, and `web-green` containers mount
  ...
  work.
- The watcher image must also include the Docker Compose CLI plugin
  (`docker-cli-compose` on Alpine) because the rollout handoff still shells
  into `docker compose ...` from inside the watcher container.
