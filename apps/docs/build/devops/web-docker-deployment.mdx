...
configured buildx builder. Remaining support-service builds use local
`docker compose build` in native mode by default; set
`DOCKER_WEB_NATIVE_SUPPORT_BUILDX=1` only when those support builds should keep
using the configured buildx builder.
