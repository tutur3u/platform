...
- After each blue/green deployment handoff finishes, the helper runs
  ...
  removed before the command exits. Set
  `DOCKER_WEB_BUILDKIT_PRUNE_AFTER_BUILD=0` only when you are intentionally
  preserving BuildKit cache for repeated local diagnostics.
...
- Hive Docker images use a filtered workspace install and a Next standalone
  runner, while Hive realtime installs its filtered production workspace with
  Bun's hoisted linker so the direct `bun apps/hive-realtime/src/index.ts`
  runtime can resolve top-level production packages such as `postgres` and
  `@tuturuuu/realtime`. Keep `.dockerignore` explicit about recursive generated
  directories such as `**/.next/**`, `tmp/**`, `apps/mobile/build/**`, and
  `apps/backend/target/**`; otherwise previous local builds can be copied into
  the next Docker context and inflate small sidecar images by several
  gigabytes.
...
- Hive is promoted with the web blue/green color: `hive-blue` and `hive-green`
  ...
  (`7803` and `7814`) before reusing it; if an older proxy was created before
  Hive moved behind blue/green, the next deploy force-recreates the proxy so the
  host-level Cloudflare Tunnel route can reach Hive.
