...
that users operate. Set `E2E_TASKS_SATELLITE_ENABLED=0` only when a focused
local spec cannot reach Tasks and does not exercise cross-app navigation; CI
keeps the satellite enabled for every shard so test redistribution cannot
silently strand the Tasks lifecycle on an unserved hostname.
