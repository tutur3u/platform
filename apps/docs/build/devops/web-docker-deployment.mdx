...
```bash
bun serve:web:docker:bg -- --build-memory 12g --build-cpus 2 --build-max-parallelism 1
```

Current root-script defaults:

- `bun serve:web:docker` defaults to `--build-memory 12g --build-cpus 2 --build-max-parallelism 1`
- `bun serve:web:docker:bg` defaults to `--build-memory 12g --build-cpus 2 --build-max-parallelism 1`

The Compose-owned BuildKit service also defaults `mem_limit` to **12g** when
`DOCKER_WEB_BUILD_MEMORY` is unset, which balances large monorepo builds with
shared hosts that still run the web stack, Redis, log drain, and other sidecars
alongside builds. Raise or lower the cap with env vars or `--build-memory` when
your machine is tighter or has spare RAM.
...
- Docker standalone Next builds default static page generation to a 180 second
  timeout with static generation concurrency and Next build CPU count capped at
  4. This avoids false `Failed to build ... because it took more than 60
...
- Hive is part of the web blue/green support-service set: `hive` is routed from
  `hive.tuturuuu.com`, and `hive-realtime` serves `/realtime` with
  `HIVE_REALTIME_TOKEN_SECRET`, `HIVE_REALTIME_URL`, and
  `NEXT_PUBLIC_HIVE_REALTIME_URL` configured in the same production stack.
  Production compose publishes **`7814:7814`** so Hive is reachable on the host
  at `http://localhost:7814` for local debugging, not only via the Docker
  network and nginx.
  `hive` uses the same Supabase env source as `apps/web`: runtime env files are
  shared, and production image builds mount the `web_env` BuildKit secret so
  hidden-locale auth pages can prerender with the platform Supabase URL.
  **Deploy coordination:** `scripts/docker-web/blue-green.js` still builds every
  prod service in one pass, but `docker compose up` runs in two phases—first
  `web-proxy`, the target `web-{color}`, MarkItDown, storage-unzip-proxy,
  `web-cron-runner`, and (when enabled) Redis—then `hive` and `hive-realtime`.
  Promotion waits only on the first-phase health gates so the main site and
  proxy can go healthy while Hive finishes booting; nginx upstreams tolerate
  Hive being temporarily down.
...
- `SRH_TOKEN` falls back to `platform-local-redis-token` in the Compose files
  when `UPSTASH_REDIS_REST_TOKEN` is not exported. Keep that fallback so
  watcher and cron recovery can run Docker Compose commands before the web
  helper has injected a generated token.
