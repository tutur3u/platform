...
Hive, proxy, and support services, and `bun sb:status`. The workflow also has a
...
For Hive, the browser-state recovery path must follow the same shape as the web
app route: GET is a no-store confirmation page only, and the destructive
`Clear-Site-Data` response happens only after a same-origin POST. Do not serve a
direct side-effecting GET from nginx, even for stale-cookie recovery. Hive's
`next start` command also runs with `NODE_OPTIONS=--max-http-header-size=65536`
so ordinary Supabase auth-cookie chunking has more headroom before the app
server returns `431`.
