...
- Hive Docker images use a filtered workspace install and a Next standalone
  runner, while Hive realtime installs only its filtered production workspace
  with Bun's hoisted linker so the direct `bun apps/hive-realtime/src/index.ts`
  runtime can resolve top-level production packages such as
  `@supabase/supabase-js`. Keep `.dockerignore` explicit about recursive
  generated directories such as `**/.next/**`, `tmp/**`, `apps/mobile/build/**`,
  and `apps/backend/target/**`; otherwise previous local builds can be copied
  into the next Docker context and inflate small sidecar images by several
  gigabytes.
