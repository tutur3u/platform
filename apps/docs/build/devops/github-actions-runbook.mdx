...
- Each beta upload increments the app version suffix from App Store Connect
  history (`0.11.0`, `0.11.1`, `0.11.2`, etc.). The workflow calculates a separate
  build number from its run number; both stores receive the same version name.
