...
dispatches only the affected `vercel-production-<app>.yaml` workflows. Do not
restore `push` triggers or `check-ci` jobs in the per-app production workflows;
doing so recreates the queued preflight fan-out the planner replaces.
...
CodeQL runs once for canonical `main` pushes, for pull requests targeting
`main` or `production`, and on its daily schedule. Do not add `production` back
to the push trigger: `bun git-sync` mirrors the already-scanned `main` SHA to
`production`, which otherwise creates duplicate analysis runs. The security
scan launches its analysis job directly and cancels stale same-ref work instead
of allocating an always-enabled switchboard preflight.
...
bundle and six downstream consumers. The nightly `main` run provides broad
regression coverage, and maintainers can still use manual dispatch for an
