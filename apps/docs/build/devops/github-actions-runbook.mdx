...
and `apps/tanstack-web` type-check/test validation.
...
summary, and skips deploy jobs after CI validation has completed. A non-empty
but invalid token still fails during Wrangler operations.

Worker runtime secrets live in Cloudflare, not GitHub. The workflow checks them
with `wrangler secret list` before upload and fails with exact setup commands
when any required name is missing:

| Worker                   | Required Cloudflare Worker secrets                                                                                                                  |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tuturuuu-backend`       | `BACKEND_INTERNAL_TOKEN`, `TUTURUUU_APP_COORDINATION_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `CRON_SECRET`, `DISCORD_APP_DEPLOYMENT_URL` |
...
exist. `deployment_mode=dry-run` compiles and runs Wrangler checks without
uploading; `deployment_mode=deploy` uploads the selected Worker version.
