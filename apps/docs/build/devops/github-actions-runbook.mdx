...
they must not recreate checksum or PR-title version bump automation. For the SDK
release workflow, manual dispatch is only valid with the branch selector set to
`production`; non-production refs are rejected before the package or publish jobs
can start.
