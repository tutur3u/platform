...
- Manual `workflow_dispatch` runs bypass affected-app gating when the workflow is
  enabled.
- Vercel push workflows first look for the latest successful GitHub Deployment
  marker for the same Vercel workflow and branch, then diff that marker SHA to
  the current `GITHUB_SHA`.
...
- bind deploy jobs to `vercel-preview-<app>` or `vercel-production-<app>`
  GitHub Environments
- reject manual dispatches from the wrong branch before install/build/deploy work
  starts
...
- record a non-blocking GitHub Deployment marker after a successful Vercel
  build/deploy so the next push can evaluate every change since the last
  successful run
...
- `supabase-production.yaml` checks both the latest production Vercel result and the latest `main`-driven staging migration result before running.
