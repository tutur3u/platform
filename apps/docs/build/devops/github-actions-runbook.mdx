...
CodeQL uses the checked-in `codeql.yml` for JavaScript/TypeScript and Python.
Keep GitHub's legacy `dynamic/github-code-scanning/codeql` workflow disabled or
both owners will analyze the same SHA. Automatic scans are commit-driven on
canonical `main` and pull requests; `bun git-sync` mirrors the already-scanned
commit to `production`, so production pushes are excluded.
