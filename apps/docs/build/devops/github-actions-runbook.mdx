...
statuses where unselected jobs should be skipped.
...
with `cancel-in-progress: true`. A newer production push cancels only the older
run for that same app instead of cancelling sibling app deployments or allowing
an obsolete run to finish green. The newest run stays queued or active and
deploys the cumulative branch state.
