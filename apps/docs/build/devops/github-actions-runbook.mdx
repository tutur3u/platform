...
workflows must rewrite the checked-out manifest immediately before `npm pack`
with `node scripts/ci/prepare-npm-package-manifest.js packages/<name>`. That
temporary rewrite replaces `workspace:` protocol ranges with the current
workspace package versions so npm consumers can install the tarball. If a
published package depends on another release-please-managed Tuturuuu package,
that dependency also needs its own `release-*-package.yaml` workflow and
matching `tuturuuu.ts` entry.
