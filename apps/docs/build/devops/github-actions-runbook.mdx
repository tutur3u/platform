...
dependency versions known to fail Apple CI before the native build starts. Keep
`connectivity_plus` pinned to `7.0.0` while the iOS/macOS build workflows use
Xcode 16.4, because `7.1.1` references `NWPath.isUltraConstrained`, which is not
