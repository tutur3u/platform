...
- `biome-check.yaml`
- GitHub-managed CodeQL (`dynamic/github-code-scanning/codeql`)
- `codecov.yaml`
- `i18n-check.yaml`
...
CodeQL uses GitHub's managed `dynamic/github-code-scanning/codeql` workflow.
Do not add a checked-in `codeql.yml`: GitHub would run both definitions for the
same SHA. The managed workflow covers JavaScript/TypeScript and Python, while
`bun git-sync` mirrors the already-scanned `main` commit to `production`.
