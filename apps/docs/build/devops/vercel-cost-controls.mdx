...
Preview, production and external-package build workflows pin Vercel CLI to
`59.25.0`. Keep these installation steps aligned when upgrading; do not use
`vercel@latest` in release jobs. A newly published CLI can reference builder
packages whose tarballs are not yet available, causing every deployment retry
to fail before the application build starts.

Before changing the pin, install the exact candidate in a temporary directory,
run `vercel --version`, and verify a preview build. Then update the workflow
family together and check both preview and production deployment jobs.
