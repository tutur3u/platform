...
- Platform preview and production build validation workflows cross-credit
  successful same-SHA build markers. If one has already passed for a commit, the
  other can record success without reinstalling dependencies or rebuilding.
...
- `apps/web` production build validation is handled by
  `vercel-production-platform.yaml`; on-premise machines handle the actual
  runtime deployment.
...
Deployment` completes its build validation on `production` or
   `Supabase Staging Migration` completes on `main`.
2. The production platform build validation for the target commit must have
   concluded successfully on `production` and recorded the successful
   `vercel-production-platform` build marker for the same SHA. A workflow run
   that skipped because package releases were still publishing is not enough.
...
build validation finishes before staging migration does. The production
...
5. Confirm the follow-up Supabase workflow after production build validation.
...
- Do not manually dispatch production Supabase migration from `main`; the only
  ...
  production platform build and a completed successful `main` staging migration.
