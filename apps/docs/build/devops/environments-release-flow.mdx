...
| `main` push                         | Platform Vercel preview deployment                                                                                                | `supabase-staging.yaml` can apply staging migrations after preview succeeds                                                          |
...
- Most preview workflows are manual-only from `main` with a required
  ...
  `main` pushes so the staging database workflow receives a same-SHA signal.
...
1. It re-evaluates automatically after either `Vercel Platform Production
   Deployment` completes on `production` or `Supabase Staging Migration`
   completes on `main`.
...
deployment finishes before staging migration does. The production migration job
still checks out the verified target commit explicitly before running
`supabase db push --include-all`. This keeps production schema changes behind
...
- Do not manually dispatch production Supabase migration from `main`; the only
  ...
  production deployment and a completed successful `main` staging migration.
