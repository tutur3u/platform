import { createFileRoute, redirect } from '@tanstack/react-router';
import { createServerFn } from '@tanstack/react-start';
import { getRequestHeaders } from '@tanstack/react-start/server';
import { Bot } from '@tuturuuu/icons';
import {
  resolveInternalApiUrl,
  withForwardedInternalApiAuth,
} from '@tuturuuu/internal-api/client';
import {
  type GitHubBotState,
  getGitHubBotState,
} from '@tuturuuu/internal-api/infrastructure/github-bot';
import { Separator } from '@tuturuuu/ui/separator';
import { ROOT_WORKSPACE_ID } from '@tuturuuu/utils/constants';
import { useTranslations } from 'use-intl';
import { GitHubBotClient } from '@/components/infrastructure/github-bot/github-bot-client';
import { requireCurrentUser } from '@/lib/platform/auth-gate';
import { createPageHead } from '@/lib/platform/head';
import { resolveMessagesLocale } from '@/lib/platform/messages';
import { resolveWorkspace } from '@/lib/platform/workspace';
import { hasWorkspacePermission } from '@/lib/platform/workspace-permission';

type GitHubBotRouteData = {
  state: GitHubBotState;
  tokenEndpointUrl: string;
};

const loadGitHubBotState = createServerFn({ method: 'GET' }).handler(
  async (): Promise<GitHubBotState> =>
    getGitHubBotState(withForwardedInternalApiAuth(getRequestHeaders()))
);

function redirectToWorkspaceSettings(locale: string, wsId: string): never {
  throw redirect({
    href: `/${locale}/${wsId}/settings`,
    statusCode: 307,
  });
}

export const Route = createFileRoute(
  '/$locale/$wsId/infrastructure/github-bot'
)({
  component: InfrastructureGitHubBotRoute,
  head: ({ params }) => {
    const locale = resolveMessagesLocale(params.locale);

    return createPageHead({
      description: 'Manage the Tuturuuu CI GitHub App token issuer.',
      locale,
      title: 'GitHub Bot',
    });
  },
  loader: async ({ params }): Promise<GitHubBotRouteData> => {
    await requireCurrentUser({
      locale: params.locale,
      nextPath: `/${params.wsId}/infrastructure/github-bot`,
    });

    const workspace = await resolveWorkspace({ data: { wsId: params.wsId } });
    if (!workspace.exists || workspace.workspaceId !== ROOT_WORKSPACE_ID) {
      redirectToWorkspaceSettings(params.locale, params.wsId);
    }

    const canManageSecrets = await hasWorkspacePermission({
      data: {
        permission: 'manage_workspace_secrets',
        wsId: ROOT_WORKSPACE_ID,
      },
    });

    if (!canManageSecrets) {
      redirectToWorkspaceSettings(params.locale, params.wsId);
    }

    return {
      state: await loadGitHubBotState(),
      tokenEndpointUrl: resolveInternalApiUrl(
        '/api/v1/infrastructure/github-bot/installation-token'
      ),
    };
  },
});

function InfrastructureGitHubBotRoute() {
  const { state, tokenEndpointUrl } =
    Route.useLoaderData() as GitHubBotRouteData;
  const t = useTranslations('github-bot-settings');

  return (
    <>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Bot className="h-5 w-5 text-primary" />
            <h1 className="font-bold text-2xl">{t('title')}</h1>
          </div>
          <p className="text-foreground/80">{t('description')}</p>
        </div>
      </div>

      <Separator className="my-4" />

      <GitHubBotClient
        initialData={state}
        tokenEndpointUrl={tokenEndpointUrl}
      />
    </>
  );
}
