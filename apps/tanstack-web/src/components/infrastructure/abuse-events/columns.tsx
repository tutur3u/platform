...
import type { BackendInfrastructureAbuseEvent } from '@tuturuuu/internal-api';
...
const getEventTypeBadge = (eventType: string) => {
  const colors: Record<string, string> = {
    ...
    manual: 'bg-dynamic-slate/10 text-dynamic-slate',
...
export const getAbuseEventsColumns = ({
  t,
  namespace,
}: ColumnGeneratorOptions<BackendInfrastructureAbuseEvent>): ColumnDef<BackendInfrastructureAbuseEvent>[] => [
  {
    accessorKey: 'id',
    header: ({ column }) => (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const ip = row.getValue<string>('ip_address');
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const metadata = row.getValue<Record<string, unknown> | null>('metadata');
