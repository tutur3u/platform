...
import { ProductManufacturerForm } from './manufacturer-form';
import { ProductUnitForm } from './unit-form';
import { ProductWarehouseForm } from './warehouse-form';
...
export function ProductForm({
  ...
  async function onSubmit(formData: z.infer<typeof FormSchema>) {
    ...
    try {
      ...
      let productPayload: Partial<InventoryProductPayload> = {
        name: formData.name,
      ...
      // The form schema permits `min_amount: number | null`, while the API
      // payload type uses `min_amount?: number`. Runtime values are compatible
      // (a null min_amount is treated as unset); cast to align the types.
      let inventoryPayload: InventoryProductPayload['inventory'] =
        normalizedInventory as InventoryProductPayload['inventory'];
      ...
      if (data?.id) {
        ...
        if (canUpdateStockQuantity) {
          ...
          if (changedInventoryItems.length > 0 || hasRemovedItems) {
            // Send the entire new inventory array (see cast note above).
            inventoryPayload =
              newInventory as InventoryProductPayload['inventory'];
        ...
        if (hasProductChanges || hasInventoryChanges) {
          const updatePayload: Partial<InventoryProductPayload> = {
            ...productPayload,
            ...(hasInventoryChanges && { inventory: inventoryPayload }),
          };

          await updateInventoryProduct(wsId, data.id, updatePayload);
        }
      } else {
        // For new products, create product and inventory in one request
        const createPayload: InventoryProductPayload = {
          ...(productPayload as InventoryProductPayload),
          ...(inventoryPayload &&
            inventoryPayload.length > 0 && { inventory: inventoryPayload }),
        };

        await createInventoryProduct(wsId, createPayload);
      }
