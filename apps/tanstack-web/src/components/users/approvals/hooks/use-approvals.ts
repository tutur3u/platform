...
export function useApprovals({
  ...
  const reportsQuery = useQuery({
    ...
    queryFn: (): Promise<PaginatedResult<ReportApprovalItem>> =>
      listWorkspaceUserApprovals<ReportApprovalItem>(wsId, {
        kind: 'reports',
        status,
        page,
        limit,
  ...
  const postsQuery = useQuery({
    ...
    enabled: kind === 'posts',
    queryFn: (): Promise<PaginatedResult<PostApprovalItem>> =>
      listWorkspaceUserApprovals<PostApprovalItem>(wsId, {
        kind: 'posts',
        status,
        page,
        limit,
  ...
  const approveMutation = useMutation({
    mutationFn: async (itemId: string) => {
      ...
      await updateWorkspaceUserApproval(wsId, {
        action: 'approve',
        kind,
        itemId,
      });
...
export function useLatestApprovedLog(wsId: string, reportId: string | null) {
  return useQuery({
    ...
    enabled: !!reportId,
    queryFn: (): Promise<ReportLogEntry | null> => {
      if (!reportId) return Promise.resolve(null);

      return listWorkspaceUserApprovalLogs<ReportLogEntry>(wsId, {
        kind: 'reports',
        reportId,
      });
    },
...
export function useLatestApprovedPostLog(wsId: string, postId: string | null) {
  return useQuery({
    ...
    enabled: !!postId,
    queryFn: (): Promise<PostLogEntry | null> => {
      if (!postId) return Promise.resolve(null);

      return listWorkspaceUserApprovalLogs<PostLogEntry>(wsId, {
        kind: 'posts',
        postId,
      });
    },
