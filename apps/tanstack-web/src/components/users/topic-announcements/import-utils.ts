...
export interface TopicAnnouncementImportPreviewRow {
  errors: TopicAnnouncementImportPreviewError[];
  row: TopicAnnouncementImportRow;
  rowNumber: number;
}

export type TopicAnnouncementImportPreviewError =
  | 'invalid_date'
  | 'invalid_email'
  | 'missing_email'
  | 'missing_topic';
...
export function validateTopicAnnouncementImportRows(
  ...
  return rows.map((row, index) => {
    const errors: TopicAnnouncementImportPreviewError[] = [];
    ...
    if (row.sessionDate?.trim() && !isValidIsoDate(row.sessionDate)) {
