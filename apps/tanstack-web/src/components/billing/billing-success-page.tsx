...
export type BillingSuccessCheckout = {
  amount: number;
  createdAt: string;
  id: string;
  productName: string;
...
function formatUsdCents(amount: number) {
  return `$${minorToMajor(amount, 'USD').toFixed(2)}`;
}

function formatCheckoutDate(value: string, locale: string) {
...
export function BillingSuccessPage({
  ...
  const paymentDetails = {
    amount: formatUsdCents(checkout.amount),
    checkoutId: checkout.id,
    date: formatCheckoutDate(checkout.createdAt, locale),
    paymentMethod: 'Card',
    planName: checkout.productName || 'Unknown Plan',
  };
  ...
  return (
    <div className="fade-in container mx-auto max-w-6xl animate-in px-4 py-8 duration-700">
      ...
      <div className="slide-in-from-bottom-6 mb-8 animate-in rounded-lg border border-border bg-card p-8 shadow-sm transition-all delay-1000 duration-700 hover:scale-[1.02] hover:shadow-lg dark:bg-card/80">
        <h2 className="mb-6 font-semibold text-2xl text-card-foreground">
          {t('success.what-next')}
        </h2>
        <div className="space-y-4">
          <div className="slide-in-from-left-4 -m-3 flex animate-in items-start gap-3 rounded-lg p-3 transition-all delay-1200 duration-500 hover:translate-x-2 hover:bg-muted/30">
          ...
          <div className="slide-in-from-left-4 -m-3 flex animate-in items-start gap-3 rounded-lg p-3 transition-all delay-1400 duration-500 hover:translate-x-2 hover:bg-muted/30">
            <CheckCircle className="mt-0.5 h-5 w-5 text-green-600 transition-transform duration-300 hover:rotate-12 hover:scale-125 dark:text-green-400" />
            <div>
              <p className="font-medium text-card-foreground">
          ...
          <div className="slide-in-from-left-4 -m-3 flex animate-in items-start gap-3 rounded-lg p-3 transition-all delay-1600 duration-500 hover:translate-x-2 hover:bg-muted/30">
            <CheckCircle className="mt-0.5 h-5 w-5 text-green-600 transition-transform duration-300 hover:rotate-12 hover:scale-125 dark:text-green-400" />
            <div>
              <p className="font-medium text-card-foreground">
      ...
      <div className="fade-in mt-8 animate-in rounded-lg border border-border bg-muted/30 p-6 text-center transition-all delay-2000 duration-800 hover:scale-105 hover:bg-muted/50">
        <p className="text-muted-foreground text-sm">
