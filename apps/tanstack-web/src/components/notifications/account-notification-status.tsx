'use client';

import { AlertTriangle, ExternalLink, Settings } from '@tuturuuu/icons';
import type { AccountNotificationPreference } from '@tuturuuu/internal-api';
import { Alert, AlertDescription } from '@tuturuuu/ui/alert';
import { Button } from '@tuturuuu/ui/button';
import {
  Card,
  CardDescription,
  CardHeader,
  CardTitle,
} from '@tuturuuu/ui/card';
import Link from 'next/link';
import { useTranslations } from 'use-intl';

type AccountNotificationStatusProps = {
  preferences: AccountNotificationPreference[];
};

export function AccountNotificationStatus({
  preferences,
}: AccountNotificationStatusProps) {
  const t = useTranslations('notifications.settings.account-status');
  const hasEnabledNotifications = preferences.some((pref) => pref.enabled);
  const uniqueDisabledChannels = [
    ...new Set(
      preferences.filter((pref) => !pref.enabled).map((pref) => pref.channel)
    ),
  ];

  if (!hasEnabledNotifications) {
    return (
      <Alert
        className="border-dynamic-red/50 bg-dynamic-red/10"
        variant="destructive"
      >
        <AlertTriangle className="h-4 w-4" />
        <AlertDescription className="flex items-center justify-between">
          <div className="flex-1">
            <p className="font-semibold">{t('all-disabled-title')}</p>
            <p className="text-sm">{t('all-disabled-description')}</p>
          </div>
          <Button asChild className="ml-4" size="sm" variant="outline">
            <Link href="/settings/account/notifications">
              <Settings className="mr-2 h-4 w-4" />
              {t('configure-account')}
              <ExternalLink className="ml-2 h-3 w-3" />
            </Link>
          </Button>
        </AlertDescription>
      </Alert>
    );
  }

  if (uniqueDisabledChannels.length === 0) {
    return null;
  }

  return (
    <Card className="border-dynamic-yellow/50 bg-dynamic-yellow/5">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-3">
          <div className="rounded-full bg-dynamic-yellow/20 p-2">
            <AlertTriangle className="h-4 w-4 text-dynamic-yellow" />
          </div>
          <div className="flex-1">
            <CardTitle className="text-base">
              {t('partial-disabled-title')}
            </CardTitle>
            <CardDescription className="text-sm">
              {t('partial-disabled-description', {
                channels: uniqueDisabledChannels.join(', '),
              })}
            </CardDescription>
          </div>
          <Button asChild size="sm" variant="ghost">
            <Link href="/settings/account/notifications">
              <Settings className="mr-2 h-4 w-4" />
              {t('view-account')}
              <ExternalLink className="ml-2 h-3 w-3" />
            </Link>
          </Button>
        </div>
      </CardHeader>
    </Card>
  );
}
