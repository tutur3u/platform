...
import {
  createRandomGeneratorTranslator,
  type RandomGeneratorMessages,
} from './random-generator-messages';
...
interface RandomGeneratorPageProps {
  messages: RandomGeneratorMessages;
}

export function RandomGeneratorPage({ messages }: RandomGeneratorPageProps) {
  ...
  const translate = useMemo(
    () => createRandomGeneratorTranslator(messages),
    [messages]
  );
  ...
  const copyAll = useCallback(() => {
    void copyText(
      ...
      translate('actions.copy_all')
    );
  }, [copyText, translate, values]);
      ...
      >
        <section className="grid content-start gap-4">
          <TabsList className="grid h-auto w-full grid-cols-3">
            ...
            <TabsTrigger value="tokens">
              ...
              {translate('tabs.tokens')}
            </TabsTrigger>
            <TabsTrigger value="passwords">
              <Lock className="size-4" />
              {translate('tabs.passwords')}
            </TabsTrigger>
          </TabsList>
          ...
          {lastCopyLabel && (
            <p aria-live="polite" className="text-muted-foreground text-sm">
              {translate('actions.copied', { label: lastCopyLabel })}
            </p>
          )}
        </section>
        ...
        <section className="min-w-0">
          <RandomGeneratorOutputList
            ...
            onCopyAll={copyAll}
            onCopyValue={(value) =>
              void copyText(value, translate('actions.copy'))
            }
          />
        </section>
      </Tabs>
