...
import { buildSharedFormMetadata } from '@/features/forms/shared-form-data';
import {
  loadSharedFormForPage,
  loadSharedFormSnapshot,
} from '@/features/forms/shared-form-loader';
...
export default async function SharedFormPage({ params }: PageProps) {
  // Opt into request-time rendering: this page reads the respondent's session
  // cookie to restore in-progress and already-submitted answers, and under
  // cacheComponents a page with no dynamic signal would be prerendered without
  ...
  if (status === 401) {
    redirect(`/login?next=/f/${shareCode}`);
  }
