...
/**
 * Resolves the signed-in respondent, if any, for the public shared-form routes.
 *
 * Satellite-specific: apps/forms authenticates with a Tuturuuu app-session JWT,
 * so `supabase.auth.getUser()` (what apps/web used) always resolves null here
 * and the "email me a copy of my response" flow would permanently 401. Read the
 * actor from the app session instead. Returning a null user is expected and
 * fine — these routes are public and anonymous respondents are the common case.
 */
