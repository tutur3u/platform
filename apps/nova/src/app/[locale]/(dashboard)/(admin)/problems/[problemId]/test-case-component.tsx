    ...
    <div className="pt-3 text-foreground">
      <Card className="min-h-[300px] overflow-y-auto bg-foreground/10 p-4">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          These are the default test cases for this problem. You can also create custom test cases
          to test your prompt in the prompt section.
        </p>
          ...
          <Tabs defaultValue="all">
              ...
              <pre className="rounded-md bg-foreground/10 p-2 whitespace-pre-wrap">
                {testcases?.map((testcase, index) => (
                  <div key={testcase.id} className="mb-4 text-sm">
                    <div className="mb-1 font-medium">Test Case {index + 1}:</div>
                    {testcase.input}
                  </div>
                ))}
              </pre>
            ...
            {testcases.map((testcase, index) => (
              <TabsContent key={index} value={`test-${index}`}>
                <pre className="rounded-md bg-foreground/10 p-2 whitespace-pre-wrap text-sm">
                  {testcase.input}
                </pre>
              </TabsContent>
            ))}
