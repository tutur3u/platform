...
const getLeadingLabel = (t: (key: string) => string, op: Operation, table: string) => {
  switch (op) {
    case 'INSERT':
...
const getTrailingLabel = (t: (key: string, options?: { count: number }) => string, data: AuditLog) =>
  t(`trailing_label_${data.table_name}`, { count: getAmount(data) });

export const getLabel = (t: (key: string, options?: { count: number }) => string, data: AuditLog) => {
