...
const BatchEditModal = ({
  ...
  useEffect(() => {
    ...
    mutate(`/api/workspaces/${wsId}/inventory/batches/${batch.id}`);
    mutate(`/api/workspaces/${wsId}/inventory/batches/${batch.id}/products`);

    showNotification({
      title: 'Thành công',
      message: 'Đã cập nhật lô hàng',
      color: 'green',
    });
  }, [hasSuccess, wsId, batch.id]);

  const updateDetails = async () => {
    // If the batch is not changed, skip this step
    if (
      oldBatch.price === batch.price &&
      oldBatch.supplier_id === batch.supplier_id &&
      oldBatch.warehouse_id === batch.warehouse_id
    ) {
      setProgress((progress) => ({ ...progress, updateDetails: 'success' }));
      return true;
    }

    const { id, price, supplier_id, warehouse_id } = batch;

    const res = await fetch(`/api/workspaces/${wsId}/inventory/batches/${id}`, {
      method: 'PUT',
      headers: {
  ...
  const updateProducts = async (products: Product[]) => {
    const updatePromises = products.map((product) => {
      return fetch(
        `/api/workspaces/${wsId}/inventory/batches/${batch.id}/products/${product.id}?unitId=${product.unit_id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const addProducts = async (products: Product[]) => {
    const res = await fetch(
      `/api/workspaces/${wsId}/inventory/batches/${batch.id}/products`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
