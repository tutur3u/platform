...
const WarehouseEditModal = ({ wsId, oldWarehouse, warehouse }: Props) => {
  ...
  useEffect(() => {
    ...
    mutate(`/api/workspaces/${wsId}/inventory/warehouses/${warehouse.id}`);

    showNotification({
      title: 'Thành công',
      message: 'Đã cập nhật kho chứa',
      color: 'green',
    });
  }, [hasSuccess, wsId, warehouse.id]);

  const updateDetails = async () => {
    // If the warehouse is not changed, skip this step
    ...
    const res = await fetch(
      `/api/workspaces/${wsId}/inventory/warehouses/${id}`,
      {
        method: 'PUT',
        headers: {
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể cập nhật kho chứa',
        color: 'red',
      });
