...
const CategoryDeleteModal = ({ wsId, categoryId }: Props) => {
  ...
  useEffect(() => {
    ...
    showNotification({
      title: 'Thành công',
      message: 'Đã xoá danh mục sản phẩm',
      color: 'green',
    });
  ...
  const removeDetails = async () => {
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể xoá danh mục sản phẩm',
        color: 'red',
      });
      ...
      >
        ...
        >
          {progress.removed === 'success' ? (
            <div className="text-green-300">Đã xoá danh mục sản phẩm</div>
          ) : progress.removed === 'error' ? (
            <div className="text-red-300">Không thể xoá danh mục sản phẩm</div>
          ) : progress.removed === 'loading' ? (
            <div className="text-blue-300">Đang xoá danh mục sản phẩm</div>
          ) : (
            <div className="text-zinc-400/80">
              Đang chờ xoá danh mục sản phẩm
            </div>
          )}
        </Timeline.Item>
