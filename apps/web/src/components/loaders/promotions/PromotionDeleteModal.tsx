...
const PromotionDeleteModal = ({ wsId, promotionId }: Props) => {
  ...
  const removeDetails = async () => {
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể xoá mã giảm giá',
        color: 'red',
      });
      ...
      >
        ...
        >
          {progress.removed === 'success' ? (
            <div className="text-green-300">Đã xoá mã giảm giá</div>
          ) : progress.removed === 'error' ? (
            <div className="text-red-300">Không thể xoá mã giảm giá</div>
          ) : progress.removed === 'loading' ? (
            <div className="text-blue-300">Đang xoá mã giảm giá</div>
          ) : (
            <div className="text-zinc-400/80">Đang chờ xoá mã giảm giá</div>
          )}
        </Timeline.Item>
