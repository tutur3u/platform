...
const PromotionEditModal = ({ wsId, oldPromotion, promotion }: Props) => {
  ...
  useEffect(() => {
    ...
    mutate(`/api/promotions/${promotion.id}`);
  ...
  }, [hasSuccess, promotion.id]);

  const updateDetails = async () => {
    // If the promotion is not changed, skip this step
    if (oldPromotion.name === promotion.name) {
      setProgress((progress) => ({ ...progress, updated: 'success' }));
      return true;
    }

    const { id, name } = promotion;
