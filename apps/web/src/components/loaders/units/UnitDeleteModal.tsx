...
const UnitDeleteModal = ({ wsId, unitId }: Props) => {
  ...
  const removeDetails = async () => {
    const res = await fetch(
      `/api/workspaces/${wsId}/inventory/units/${unitId}`,
      {
        method: 'DELETE',
      }
    );
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể xoá đơn vị tính',
        color: 'red',
      });
      ...
      >
        ...
        >
          {progress.removed === 'success' ? (
            <div className="text-green-300">Đã xoá đơn vị tính</div>
          ) : progress.removed === 'error' ? (
            <div className="text-red-300">Không thể xoá đơn vị tính</div>
          ) : progress.removed === 'loading' ? (
            <div className="text-blue-300">Đang xoá đơn vị tính</div>
          ) : (
            <div className="text-zinc-400/80">Đang chờ xoá đơn vị tính</div>
          )}
        </Timeline.Item>
