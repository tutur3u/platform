...
const SupplierDeleteModal = ({ wsId, supplierId }: Props) => {
  ...
  const removeDetails = async () => {
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể xoá nhà cung cấp',
        color: 'red',
      });
      ...
      >
        ...
        >
          {progress.removed === 'success' ? (
            <div className="text-green-300">Đã xoá nhà cung cấp</div>
          ) : progress.removed === 'error' ? (
            <div className="text-red-300">Không thể xoá nhà cung cấp</div>
          ) : progress.removed === 'loading' ? (
            <div className="text-blue-300">Đang xoá nhà cung cấp</div>
          ) : (
            <div className="text-zinc-400/80">Đang chờ xoá nhà cung cấp</div>
          )}
        </Timeline.Item>
