...
const VitalEditModal = ({ wsId, vital }: Props) => {
  ...
  useEffect(() => {
    ...
    mutate(`/api/workspaces/${wsId}/healthcare/vitals/${vital.id}`);
  ...
  }, [hasSuccess, wsId, vital.id]);

  const updateDetails = async () => {
    const res = await fetch(
      `/api/workspaces/${wsId}/healthcare/vitals/${vital.id}`,
      {
        method: 'PUT',
        headers: {
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể cập nhật chỉ số',
        color: 'red',
      });
