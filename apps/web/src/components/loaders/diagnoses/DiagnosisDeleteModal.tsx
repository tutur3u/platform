...
const DiagnosisDeleteModal = ({ wsId, diagnosisId }: Props) => {
  ...
  useEffect(() => {
    ...
    showNotification({
      title: 'Thành công',
      message: 'Đã xoá chẩn đoán',
      color: 'green',
    });
  }, [hasSuccess, diagnosisId]);

  const removeDetails = async () => {
    const res = await fetch(
      `/api/workspaces/${wsId}/healthcare/diagnoses/${diagnosisId}`,
      {
        method: 'DELETE',
      }
    );
    ...
    if (res.ok) {
      ...
      showNotification({
        title: 'Lỗi',
        message: 'Không thể xoá chẩn đoán',
        color: 'red',
      });
      ...
      >
        ...
        >
          {progress.removed === 'success' ? (
            <div className="text-green-300">Đã xoá chẩn đoán</div>
          ) : progress.removed === 'error' ? (
            <div className="text-red-300">Không thể xoá chẩn đoán</div>
          ) : progress.removed === 'loading' ? (
            <div className="text-blue-300">Đang xoá chẩn đoán</div>
          ) : (
            <div className="text-zinc-400/80">Đang chờ xoá chẩn đoán</div>
          )}
        </Timeline.Item>
...
export default DiagnosisDeleteModal;
