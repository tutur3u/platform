...
export async function POST(req: Request, { params }: Params) {
    ...
    try {
        ...
        if (!customer_id || !products || products.length === 0 || !wallet_id || !category_id) {
            return NextResponse.json(
                { message: 'Missing required fields: customer_id, products, wallet_id, and category_id' },
                { status: 400 }
            );
        }
        ...
        const { data: productsData, error: productsError } = await supabase
            ...
            .select('name, id')
            .in('id', Array.from(productMap.values()).map(product => product.product_id))
            .eq('ws_id', wsId);

        if (productsError) {
            console.error('Error getting products information:', productsError);
            return NextResponse.json(
                { message: 'Error getting products information', details: productsError.message },
                { status: 500 }
            );
        }
