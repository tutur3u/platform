...
// Group storage is reached from two satellites: the Contacts app owns the whole
// workspace_users CRM surface (including group files), and Teach uploads module
// material through the same route. Authorization is the workspace
// view/update_user_groups permission below, not the calling app, so both app
// sessions are accepted — leaving this at 'teach' only made every Contacts group
// storage request fail with 401.
const USER_GROUP_STORAGE_APP_SESSION_TARGETS = ['contacts', 'teach'] as const;
...
export const GET = withSessionAuth<{
  ...
  { allowAppSessionAuth: { targetApp: USER_GROUP_STORAGE_APP_SESSION_TARGETS } }
);
