import type { WorkspacePermissionsSummary } from '@tuturuuu/internal-api/settings';
...
export const GET = withSessionAuth<{ wsId: string }>(
  async (_request, { supabase, user }, { wsId }) => {
    try {
      ...
      // Typed against the shared contract so dropping a field here is a
      // compile error rather than a settings dialog that silently renders as
      // read-only for everyone.
      const summary: WorkspacePermissionsSummary = {
...
// Preserves the HEAD handling the generated wrapper used to add for GET routes.
