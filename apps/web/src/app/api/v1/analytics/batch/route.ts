/**
 * Analytics Batch Event Tracking Endpoint
 * POST /api/v1/analytics/batch
 *
 * Tracks multiple analytics events in a single request (max 100 events).
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validatedData = batchTrackSchema.parse(body);

    // Extract workspace ID (same logic as track endpoint)
    ...
    for (const event of validatedData.events) {
      const { data: eventId, error: eventError } = await supabase.rpc(
        'track_analytics_event',
        {
          ...
          p_event_properties: event.event_properties || null,
    ...
    // Return success if at least some events were tracked
    if (results.length > 0) {
      return NextResponse.json(
        {
          message: `Successfully tracked ${results.length} of ${validatedData.events.length} events`,
          data: {
    ...
    return NextResponse.json(
      {
        error: 'Internal Server Error',
        message: 'Failed to track any events',
        errors,
      },
      { status: 500 }
    );
  } catch (error) {
    console.error('Error in batch track endpoint:', error);
