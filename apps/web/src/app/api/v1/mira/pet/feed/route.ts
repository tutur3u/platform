/**
 * Mira Feed API
 * POST /api/v1/mira/pet/feed - Feed Mira
 */
...
export async function POST() {
  try {
    ...
    if (hoursSinceLastFed < FEED_COOLDOWN_HOURS) {
      ...
      return NextResponse.json(
        {
          error: `Mira is not hungry yet! Wait ${minutesRemaining} more minutes.`,
    ...
    const { error: xpError } = await supabase.rpc('award_mira_xp', {
      p_user_id: user.id,
      p_xp: FEED_XP_REWARD,
      p_source: 'feeding',
    });
    ...
    const messages = [
      'Yum yum! Mira loves the treat!',
      'Mira gobbles it up happily!',
      '*happy fish noises*',
      'Mira does a little happy wiggle!',
    ...
    console.error('Unexpected error in POST /api/v1/mira/pet/feed:', error);
    return NextResponse.json(
