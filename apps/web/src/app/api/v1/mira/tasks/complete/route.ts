/**
 * Mira Task Complete API
 * POST /api/v1/mira/tasks/complete - Complete a task and award XP
 */
...
export async function POST(request: Request) {
  try {
    ...
    const { data: pet, error: xpError } = await supabase.rpc('award_mira_xp', {
      p_user_id: user.id,
      p_xp: xpEarned,
    ...
    if (existingStats) {
      ...
      await supabase.from('mira_daily_stats').insert({
        user_id: user.id,
        date: today,
        tasks_completed: 1,
      });
    ...
    console.error(
      'Unexpected error in POST /api/v1/mira/tasks/complete:',
      error
    );
    return NextResponse.json(
