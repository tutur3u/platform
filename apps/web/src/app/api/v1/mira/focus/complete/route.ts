/**
 * Mira Focus Session Complete API
 * POST /api/v1/mira/focus/complete - Complete/end a focus session
 */
  ...
  try {
    ...
    const checkAndUnlock = async (code: string, condition: boolean) => {
      if (condition && !unlockedCodes.has(code)) {
        ...
        if (achievement) {
          await supabase.from('mira_user_achievements').insert({
            user_id: user.id,
    ...
    if (newlyUnlocked.length > 0) {
      ...
      for (const achievement of unlockedAchievementDetails as {
        ...
        if (achievement.xp_reward > 0) {
          await supabase.rpc('award_mira_xp', {
            p_user_id: user.id,
    ...
    console.error(
      'Unexpected error in POST /api/v1/mira/focus/complete:',
      error
    );
    return NextResponse.json(
