...
function replaceLegacyOnboardingDestination(url: URL) {
  const segments = url.pathname.split('/').filter(Boolean);
  const onboardingIndex = segments[0] === 'onboarding' ? 0 : 1;
  if (segments[onboardingIndex] !== 'onboarding') return url;

  if (onboardingIndex === 0) {
    url.pathname = '/personal';
  } else if (segments.length === 2) {
    url.pathname = `/${segments[0]}/personal`;
  }
  return url;
}
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (_returnUrl) {
    ...
    if (validated) {
      ...
      return NextResponse.redirect(
        replaceLegacyOnboardingDestination(redirectUrl)
      );
    }
  ...
  const defaultPath = normalizedNextUrl ? `/${normalizedNextUrl}` : '/personal';
  const redirectUrl = replaceLegacyOnboardingDestination(
    new URL(defaultPath, redirectOrigin)
  );
