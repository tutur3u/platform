...
export default function PlanLogin({
	...
	const { setValue, setError, watch, control, handleSubmit, reset } = form;
	...
	const onSubmit = useCallback(
		async (values: z.infer<typeof formSchema>) => {
			...
			if (res.ok) {
				...
				const data = await res.json();
				setValue("guestPassword", "");
				setError("guestPassword", { message: data.message });
				setLoading(false);
			}
		},
		[plan.id, setUser, setDisplayMode, setValue, setError],
	);
	...
	useEffect(() => {
		...
		try {
			...
			if (savedCredentialsJSON) {
				...
				setValue("guestName", savedCredentials.name || "");
				setValue("guestPassword", savedCredentials.password || "");
	...
	}, [plan.id, onSubmit, user, setValue]);
	...
	const missingFields = !watch("guestName");
