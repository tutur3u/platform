...
export default function EmptyState({ onClearFilters }: EmptyStateProps) {
  return (
    <div className="py-12 text-center">
      <div className="mb-6">
        <Search className="mx-auto h-16 w-16 text-brand-light-yellow md:h-20 md:w-20" />
      </div>
      ...
      <p className="mb-6 flex items-center justify-center gap-2 font-bold text-lg text-muted-foreground leading-normal md:text-xl lg:text-2xl">
        ...
        <Smile className="h-6 w-6 text-brand-light-yellow md:h-8 md:w-8" />
      </p>
      <button
        type="button"
        onClick={onClearFilters}
        className="inline-flex items-center gap-2 rounded-xl bg-linear-to-r from-brand-light-blue to-dynamic-cyan px-6 py-3 font-bold text-lg text-primary-foreground transition-all duration-300 hover:scale-105 hover:shadow-lg"
      >
        Clear Filters
      </button>
    </div>
