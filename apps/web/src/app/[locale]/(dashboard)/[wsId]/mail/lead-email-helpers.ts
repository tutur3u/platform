...
export function extractLeadEmailConfig(
  ...
  const tableHeaderScore = getConfig(
  ...
  if (!emailGreeting) missingConfigs.push(LEAD_EMAIL_CONFIG_KEYS.EMAIL_GREETING);
  ...
  return {
    ...
    // Optional
    brandLocation: getConfig(LEAD_EMAIL_CONFIG_KEYS.BRAND_LOCATION) ?? undefined,
    tableScoreScale: getConfig(LEAD_EMAIL_CONFIG_KEYS.TABLE_SCORE_SCALE) ?? undefined,
    brandLogoWidth: getConfig(LEAD_EMAIL_CONFIG_KEYS.BRAND_LOGO_WIDTH) ?? undefined,
    brandLogoHeight: getConfig(LEAD_EMAIL_CONFIG_KEYS.BRAND_LOGO_HEIGHT) ?? undefined,
    titleColor: getConfig(LEAD_EMAIL_CONFIG_KEYS.TITLE_COLOR) ?? undefined,
    emptyCommentsPlaceholder: getConfig(LEAD_EMAIL_CONFIG_KEYS.EMPTY_COMMENTS) ?? undefined,
    emptyScorePlaceholder: getConfig(LEAD_EMAIL_CONFIG_KEYS.EMPTY_SCORE) ?? undefined,
  };
}
 ...
 */
export function validateLeadEmailConfigs(
  configs: WorkspaceConfig[]
): string[] {
