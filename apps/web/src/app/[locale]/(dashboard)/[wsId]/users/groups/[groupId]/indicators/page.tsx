...
import { CalendarIcon, ChartColumn, FileUser, UserCheck } from "@tuturuuu/ui/icons"
  ...
  return (
    <>
      <FeatureSummary
        title={
          <>
            <h1 className="w-full font-bold text-2xl">{group.name || t("ws-user-groups.singular")}</h1>
...
async function getIndicators(groupId: string) {
  ...
  const { data: rawData, error } = await supabase
    ...
    .select("user_id, indicator_id, value, healthcare_vitals!inner(group_id)")
    .eq("healthcare_vitals.group_id", groupId)

  if (error) throw error
  if (!rawData) return []

  const data = rawData.map((d) => ({ user_id: d.user_id, indicator_id: d.indicator_id, value: d.value }))
