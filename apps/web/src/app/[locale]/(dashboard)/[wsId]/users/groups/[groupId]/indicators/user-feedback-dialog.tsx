...
import { ChevronLeft, ChevronRight, AlertCircle, CheckCircle, ShieldUserIcon } from '@tuturuuu/ui/icons';
...
export default function UserFeedbackDialog({
    ...
    const { data: feedbackHistory, isLoading: isLoadingHistory } = useQuery({
        ...
        `, { count: 'exact' })
            ...
            const feedbacksWithCreator = data?.map(feedback => ({
                ...feedback,
    ...
    const createFeedbackMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['user-feedbacks', user?.id, groupId] });
        },
        onError: (error) => {
            ...
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col overflow-hidden">
                    <TabsList className="grid w-full grid-cols-2">
                        ...
                        <TabsTrigger value="view">{tFeedback('view_feedbacks')}</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="view" className="space-y-4 overflow-y-auto flex-1">
                        {isLoadingHistory ? (
                                                            ...
                                                            <div className="flex items-center gap-1">
                                                                {feedback.require_attention ? (
                                                                    <>
                                                                        ...
                                                                        <span className="text-dynamic-red">{tFeedback('requires_attention')}</span>
                                                                    </>
                                                                ) : (
                                                                    <>
                                                                        <CheckCircle className="w-4 h-4 text-dynamic-green" />
                                                                        <span className="text-dynamic-green">{tFeedback('ok')}</span>
                                                                    </>
                                                                )}
                                                            </div>
                                                        </div>
                                                        <div className="text-dynamic-blue truncate max-w-sm flex items-center gap-1">
                                                            <ShieldUserIcon className="w-4 h-4" /> {getCreatorName(feedback)}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t">
                                        <div className="text-sm text-muted-foreground">
                                            {tFeedback('showing_feedbacks', {
                                                ...
                                                end: Math.min(currentPage * FEEDBACKS_PER_PAGE, totalCount),
                                                total: totalCount,
                                            })}
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            >
                                                ...
                                                <span className="hidden sm:inline">{tFeedback('previous')}</span>
                                            </Button>
                                            <span className="text-sm text-muted-foreground whitespace-nowrap">
                                                {tFeedback('page_of', { current: currentPage, total: Math.ceil(totalCount / FEEDBACKS_PER_PAGE) })}
                                            </span>
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                ...
                                                disabled={!hasMore}
                                            >
                                                <span className="hidden sm:inline">{tFeedback('next')}</span>
