...
describe('EntryDetailClient', () => {
  ...
  function renderClientWithExistingTaxonomy() {
    render(
      <EntryDetailClient
        binding={
          {
  ...
  function renderClientWithGalleryTaxonomyConfig({
    entryId = 'entry-1',
    galleryProfileData = {},
  }: {
    entryId?: string;
    galleryProfileData?: Record<string, unknown>;
  } = {}) {
    render(
      <EntryDetailClient
        binding={
          ...
          entries: [
            {
              ...
              id: 'entry-gallery',
              metadata: {},
              profile_data: galleryProfileData,
              published_at: null,
              scheduled_for: null,
              slug: 'gallery',
              status: 'draft',
  ...
  it('saves entry category and tags through structured taxonomy controls', async () => {
    renderClient();

    fireEvent.click(
      screen.getAllByRole('button', { name: 'epm.category_create_action' })[0]!
    );
    const categoryInput = screen.getByPlaceholderText(
      'epm.category_create_placeholder'
    );
    ...
    fireEvent.click(
      screen.getAllByRole('button', { name: 'epm.tags_create_action' })[0]!
    );
    const tagInput = screen.getByPlaceholderText('epm.tags_create_placeholder');
    fireEvent.change(tagInput, {
      target: { value: 'hero, featured' },
    });
  ...
  it('clears category and tags through explicit taxonomy actions', async () => {
    renderClientWithExistingTaxonomy();

    fireEvent.click(
      screen.getByRole('button', { name: 'epm.category_clear_action' })
    );
    fireEvent.click(
      screen.getByRole('button', { name: 'epm.tags_clear_action' })
    );
    fireEvent.click(screen.getByRole('button', { name: 'epm.save_action' }));
  ...
  it('reads category and tag suggestions from the gallery config instead of peer entries', async () => {
    renderClientWithGalleryTaxonomyConfig({
      galleryProfileData: {
        categoryOptions: ['Power'],
        tagOptions: ['canon'],
      },
    });
    ...
    fireEvent.click(screen.getByText('epm.tags_picker_placeholder'));
  ...
  it('saves taxonomy libraries from the gallery singleton section entry', async () => {
    ...
    renderClientWithGalleryTaxonomyConfig({ entryId: 'entry-gallery' });
  ...
  it('persists ordered featured writing entries from the section editor', async () => {
    ...
    fireEvent.click(screen.getByText('Second Lore').closest('button')!);
    fireEvent.click(
      screen
