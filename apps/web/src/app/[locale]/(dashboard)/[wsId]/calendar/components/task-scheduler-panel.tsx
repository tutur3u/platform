...
export function TaskSchedulerPanel({
  ...
  const refreshSchedulingViews = async () => {
    await Promise.all([
      refetch(),
      invalidatePlanningQueries(queryClient as any, wsId, {
  ...
  const handleScheduleAll = async () => {
    ...
    try {
      const previewResult = await previewWorkspaceCalendarSchedule<{
        preview: {
          events: Array<{
            id: string;
            title: string;
      ...
      const result = await applyWorkspaceCalendarSchedule<{
        ...
        wsId,
        {
          ...
          mode: 'full-apply',
          scope: 'impacted-only',
          previewEvents: previewResult.preview.events,
        },
        {
          fetch: (input, init) => fetch(input, { ...init, cache: 'no-store' }),
        }
      ...
      if (result.events?.length || result.deletedEventIds?.length) {
        const optimisticEvents = (result.events ?? []).map((event) => ({
          id: event.id,
          title: event.title,
          start_at: event.start_at,
