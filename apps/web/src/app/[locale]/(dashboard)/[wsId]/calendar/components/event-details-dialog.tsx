...
interface EventDetailsDialogProps {
  ...
  isLoading?: boolean
  wsId: string
  onEventUpdate?: (eventId: string, updatedFields?: Partial<WorkspaceScheduledEventWithAttendees>) => void
...
const createDateTimeInTimezone = (date: Date, timeString: string, userTimezone: string): string => {
...
export default function EnhancedEventDetailsDialog({
  ...
  const [localEvent, setLocalEvent] = useState<WorkspaceScheduledEventWithAttendees | null>(null)
  ...
  useEffect(() => {
    if (localEvent && isEditing) {
      ...
      setEditFormData({
        ...
        selected_attendees: localEvent.attendees?.map((a) => a.user_id).filter(Boolean) || [],
      })
    }
  ...
  const handleInputChange = useCallback(
    (field: keyof EventFormData, value: string | boolean | Date | number | undefined) => {
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      if (editFormData.is_all_day) {
        ...
        end_at = createDateTimeInTimezone(editFormData.end_date, hourToTimeString(editFormData.end_time), userTimezone)
      }
      ...
      const response = await fetch(`/api/v1/workspaces/${wsId}/scheduled-events/${localEvent.id}`, {
        method: "PUT",
        headers: {
      ...
      const currentAttendeeIds = new Set(localEvent.attendees?.map((a) => a.user_id).filter(Boolean) || [])
      ...
      if (
        ...
        if (attendeesToAdd.length > 0) {
          const addResponse = await fetch(`/api/v1/workspaces/${wsId}/scheduled-events/${localEvent.id}/attendees`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
        ...
        if (attendeesToRemove.length > 0) {
          const results = await Promise.allSettled(
            attendeesToRemove.map((uid) =>
              fetch(`/api/v1/workspaces/${wsId}/scheduled-events/${localEvent.id}/attendees/${uid}`, { method: "DELETE" }),
            ),
          )
  ...
  }, [wsId, localEvent, editFormData, onEventUpdate, refreshScheduledEvents, event])
  ...
  const isCreator = !userLoading && currentUserId && localEvent?.creator_id === currentUserId
  const userAttendee = localEvent?.attendees?.find((attendee) => attendee.user_id === currentUserId)
  ...
  const handleUpdateAttendeeStatus = useCallback(
    async (status: "accepted" | "declined" | "tentative") => {
      ...
      try {
        const response = await fetch(`/api/v1/workspaces/${wsId}/scheduled-events/${localEvent.id}/attendees/respond`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleConfirmEvent = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/v1/workspaces/${wsId}/scheduled-events/${localEvent.id}/confirm`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        ...
        setLocalEvent(prev => prev ? { ...prev, status: event?.status } : null)
        onEventUpdate?.(localEvent.id, { status: event?.status })
        const errorData = await response.json()
        throw new Error(errorData.error || "Failed to confirm event")
      }

      toast.success("Event confirmed successfully. No further changes can be made.")
      ...
      toast.error(error instanceof Error ? error.message : "Failed to confirm event. Please try again.")
    } finally {
      setIsConfirming(false)
    }
  }, [wsId, localEvent, isCreator, refreshScheduledEvents, onEventUpdate, event])
  ...
  const handleUnconfirmEvent = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to unconfirm event. Please try again.")
    } finally {
      setIsConfirming(false)
    }
  }, [wsId, localEvent, isCreator, refreshScheduledEvents, onEventUpdate, event])

  const formatEventTime = useCallback((eventData: WorkspaceScheduledEventWithAttendees) => {
  ...
  const getStatusStats = useCallback(() => {
    ...
    const { total, accepted, declined, pending, tentative } = localEvent.attendee_count
    return {
      total,
      accepted,
      declined,
          ...
          <div className="flex h-full flex-col">
            ...
            <DialogHeader className="space-y-6 pb-6">
              <div className="space-y-4">
                   ...
                   <DialogTitle className="text-2xl leading-tight font-bold flex-1 pr-4">{localEvent.title}</DialogTitle>
                 </div>
                ...
                <div className="space-y-3">
                 ...
                 {localEvent.creator && (
                   <div className="flex items-center space-x-3 text-sm text-muted-foreground">
                     ...
                     <span>•</span>
                     <span>{format(new Date(localEvent.created_at || ""), "MMM d, yyyy")}</span>
                   </div>
                 )}
              </div>
              ...
              {isAttendee && (
                <div className="space-y-4 rounded-lg bg-muted/30 p-4">
                  ...
                  {userAttendee?.status === "pending" ? (
                    ...
                    <div className="flex items-center justify-between">
                      <Badge
                        className={cn("px-3 py-1 text-sm", STATUS_CONFIG[userAttendee?.status || "pending"].color)}
                      >
                        <div
                          className={cn(
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={handleResendInvitations}>
                              ...
                              Resend Invitations
                                                         ...
                                                         <DropdownMenuItem onClick={() => setIsEditing(true)} disabled={localEvent.status === "confirmed"} className={localEvent.status === "confirmed" ? "opacity-50" : ""}>
                               <Pencil className="mr-2 h-4 w-4" />
                               Edit Details
                             </DropdownMenuItem>
                             {/* Confirmation Actions */}
                             {localEvent.status === "confirmed" ? (
                               <DropdownMenuItem onClick={handleUnconfirmEvent} disabled={isConfirming}>
                                 <X className="mr-2 h-4 w-4" />
                                 Remove Confirmation
                               </DropdownMenuItem>
                             ) : (
                               <DropdownMenuItem onClick={() => setShowConfirmDialog(true)} disabled={isConfirming}>
                                 <Check className="mr-2 h-4 w-4" />
                                 Confirm Event
                               </DropdownMenuItem>
                             )}
                             ...
                             <DropdownMenuItem 
                               ...
                               disabled={isDeleting || localEvent.status === "confirmed"}
            ...
            {isEditing ? (
              <div className="space-y-4">
                ...
                />
                 ...
                 <div className="pb-6">
                   <p className="leading-relaxed text-muted-foreground">{localEvent.description}</p>
                 </div>
               )
            )}
                      ...
                      {isCreator && (
                        ...
                        >
                         ...
                         >
                           ...
                           <ChevronDown
                             className={cn("ml-1 h-4 w-4 transition-transform", showAllAttendees && "rotate-180")}
                           />
                         </Button>
                       )}
                    </div>
      ...
      <Dialog open={showDeleteConfirmation} onOpenChange={setShowDeleteConfirmation}>
        <DialogContent className="max-w-sm">
          ...
          <div className="mt-6 flex justify-end gap-2">
            ...
            <Button variant="destructive" onClick={handleDeleteEvent} disabled={isDeleting} aria-label="Delete event">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Once you confirm this event, it will be locked and no further changes can be made to:
            </p>
          ...
          <div className="mt-6 flex justify-end gap-2">
            <Button variant="outline" onClick={() => setShowConfirmDialog(false)} autoFocus>
              Cancel
            </Button>
            <Button
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate font-medium">{attendee.user?.display_name || "Unknown User"}</div>
                ...
                <Badge className={cn("px-2 py-1 text-xs font-medium", STATUS_CONFIG[attendee.status].color)}>
                  <div className={cn("mr-1.5 h-1.5 w-1.5 rounded-full", STATUS_CONFIG[attendee.status].dotColor)} />
