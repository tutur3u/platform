...
export default function EnhancedEventDetailsDialog({
  ...
  useEffect(() => {
    if (event && isEditing) {
      ...
      setEditFormData({
        ...
        selected_attendees:
          event.attendees?.map((a) => a.user_id).filter(Boolean) || [],
      });
    }
  }, [event, isEditing]);
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/workspaces/${wsId}/scheduled-events/${event.id}`,
        {
          method: 'PUT',
          headers: {
      ...
      if (
        ...
        if (attendeesToAdd.length > 0) {
          const addResponse = await fetch(
            `/api/v1/workspaces/${wsId}/scheduled-events/${event.id}/attendees`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
        ...
        if (attendeesToRemove.length > 0) {
          const results = await Promise.allSettled(
            attendeesToRemove.map((uid) =>
              fetch(
                `/api/v1/workspaces/${wsId}/scheduled-events/${event.id}/attendees/${uid}`,
                { method: 'DELETE' }
              )
            )
          );
  ...
  const isCreator =
    !userLoading && currentUserId && event?.creator_id === currentUserId;
