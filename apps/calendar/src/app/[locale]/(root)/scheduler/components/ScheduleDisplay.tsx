  ...
  const scheduleStats = useMemo(() => {
    ...
    const priorityStats = events.reduce(
      (acc, event) => {
        ...
        if (task) {
          acc[task.priority as TaskPriority] =
            (acc[task.priority as TaskPriority] || 0) + 1;
        }
        return acc;
      },
                              ...
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 space-y-3">
                                  ...
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {task && (
                                      <Badge
                                        className={`${getPriorityColor(task.priority as TaskPriority)} px-2 py-1 text-xs font-semibold`}
                                      >
                                        <span className="mr-1">
                                          {getPriorityIcon(
