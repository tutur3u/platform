...
function isPublicLettinApiRequest(request: NextRequest) {
  return (
    (request.method === 'GET' || request.method === 'HEAD') &&
    request.nextUrl.pathname.startsWith('/api/v1/lettin/')
  );
}
...
export async function proxy(request: NextRequest): Promise<NextResponse> {
  if (request.nextUrl.pathname.startsWith('/api')) {
    ...
    const isPublicSharedFormApi = isPublicLettinApiRequest(request);
