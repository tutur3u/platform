...
function HiveWorkflowStudioInner({
  ...
  useEffect(() => {
    const definition =
      ...
      createWorkflowTemplate('simulation_tick', translateWorkflowKey);
  ...
  }, [selectedWorkflow, setEdges, setNodes, t, translateWorkflowKey]);
  ...
  const addNode = useCallback(
    (type: HiveWorkflowNodeType, position = { x: 120, y: 120 }) => {
      ...
      const node: WorkflowFlowNode = {
        data: {
          ...
          label: translateWorkflowKey(`nodes.${catalogItem?.labelKey ?? type}`),
        },
        id: `${type}-${Date.now()}`,
        position,
        type,
    ...
    [isAdmin, setNodes, translateWorkflowKey]
  );

  const useTemplate = useCallback(
    (template: WorkflowTemplateKey) => {
      if (!isAdmin) return;
      const definition = createWorkflowTemplate(template, translateWorkflowKey);
      setNodes(fromDefinitionNodes(definition));
      setEdges(fromDefinitionEdges(definition));
      setDraftName(translateWorkflowKey(`templates.${template}.name`));
      setDraftDescription(
    ...
    [isAdmin, setEdges, setNodes, translateWorkflowKey]
  );
