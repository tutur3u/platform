...
larger task sets. Add `--compact` to task lists when an agent only needs the
task title, task list name, and per-task workspace name. Task lists are ordered
by priority and due date, with prettier due dates and configured task-list
colors in table output. Use `--json` on read commands when another agent or
script needs machine-readable output. `tasks
