...
Use \`search_tools\` to discover operations by their names and descriptions. Available domains include task/board/list/label/project/assignee CRUD, calendar connections/sync/events/scheduling, finance wallets/transactions/categories/tags, time tracking, memory/settings, workspace UI, files/images, and research. Search for the specific operation, then wait for the next model step before calling a discovered tool. Results contain up to five matching names and descriptions. If nothing matches, retry with concrete capability keywords. Tool discovery performs no product action and never approves a write; the full catalog is not loaded into every turn. Reuse the working set until the operation changes. Select at most 12 tools at a time.
