...
export const TASK_DIALOG_CREATE_PRESENTATION_CONFIG_ID =
  'TASK_DIALOG_CREATE_PRESENTATION';
export const TASK_DIALOG_EDIT_PRESENTATION_CONFIG_ID =
  'TASK_DIALOG_EDIT_PRESENTATION';
export const TASK_DOCUMENT_CREATE_PRESENTATION_CONFIG_ID =
  'TASK_DOCUMENT_CREATE_PRESENTATION';
export const TASK_DOCUMENT_EDIT_PRESENTATION_CONFIG_ID =
  'TASK_DOCUMENT_EDIT_PRESENTATION';

export type TaskDialogPresentation = 'compact' | 'focused' | 'fullscreen';
export type TaskDialogMode = 'edit' | 'create';

export interface TaskDialogPresentationPreferences {
  taskCreate: TaskDialogPresentation;
  taskEdit: TaskDialogPresentation;
  documentCreate: TaskDialogPresentation;
  documentEdit: TaskDialogPresentation;
}
...
export function resolveTaskDialogOpeningPresentation({
  ...
  preferences: TaskDialogPresentationPreferences;
  draftId?: string;
  mode?: TaskDialogMode;
  selectedListStatus?: string | null;
}): TaskDialogPresentation {
  if (draftId) return 'fullscreen';
  const isDocument = selectedListStatus === 'documents';
  if (mode === 'create') {
    return isDocument ? preferences.documentCreate : preferences.taskCreate;
  }
  return isDocument ? preferences.documentEdit : preferences.taskEdit;
}
