...
describe('BoardViews', () => {
  ...
  it('also searches by ticket identifier for bare display numbers', async () => {
    renderBoardViews();
    ...
    await waitFor(() => {
      expect(listWorkspaceTasksMock).toHaveBeenCalledWith(
        'ws-1',
        expect.objectContaining({ boardId: 'board-1', q: '115' })
      );
    });
    ...
    await waitFor(() => {
      expect(listWorkspaceTasksMock).toHaveBeenCalledWith(
        'ws-1',
        expect.objectContaining({ boardId: 'board-1', identifier: '115' })
      );
    });

    const identifierCall = listWorkspaceTasksMock.mock.calls.find(
      (call) => call[1]?.identifier
    );
    expect(identifierCall?.[1]?.q).toBeUndefined();
  });

  it('also searches by ticket identifier for prefixed queries', async () => {
    renderBoardViews();
    ...
    await waitFor(() => {
      expect(listWorkspaceTasksMock).toHaveBeenCalledWith(
        'ws-1',
        expect.objectContaining({ boardId: 'board-1', identifier: 'DEV-115' })
      );
    });
  });

  it('does not run an identifier query for plain text searches', async () => {
    renderBoardViews();
    ...
    expect(
      listWorkspaceTasksMock.mock.calls.some((call) => call[1]?.identifier)
    ).toBe(false);
  });

  it('merges name and ticket identifier matches with identifier hits first', async () => {
    const ticketMatch = {
      ...mockTasks[0]!,
      display_number: 115,
      id: 'task-ticket',
    ...
    const nameMatch = {
      ...
      display_number: 7,
      id: 'task-name',
      name: 'Sprint 115 planning',
    };

    listWorkspaceTasksMock.mockImplementation(async (_workspaceId, options) => {
      if (options?.includeListCounts) {
        return { listCounts: [{ count: 1, list_id: 'list-1' }], tasks: [] };
      }
      if (options?.identifier) {
        return { tasks: [ticketMatch] };
      }
      return { tasks: [nameMatch] };
    });
    ...
    await waitFor(() => {
      expect(kanbanBoardProps?.tasks.map((task) => task.id)).toEqual([
        'task-ticket',
        'task-name',
      ]);
    });
  ...
  it('de-duplicates a task matched by both search legs', async () => {
    const bothMatch = {
      ...mockTasks[0]!,
      display_number: 115,
      id: 'task-both',
  ...
  it('keeps lists visible when only the identifier leg reports a count', async () => {
    const ticketMatch = {
      ...mockTasks[0]!,
      display_number: 115,
      id: 'task-ticket',
    ...
    listWorkspaceTasksMock.mockImplementation(async (_workspaceId, options) => {
      if (options?.includeListCounts) {
        return options?.identifier
          ? { listCounts: [{ count: 1, list_id: 'list-1' }], tasks: [] }
          : { listCounts: [], tasks: [] };
      }
      return { tasks: options?.identifier ? [ticketMatch] : [] };
    });
  ...
  it('matches ticket identifiers client-side on read-only boards', async () => {
    renderBoardViews({
      props: {
        availableViews: ['kanban', 'list'],
        publicView: true,
      ...
      tasks: [{ ...mockTasks[0]!, display_number: 115, name: 'Fix login' }],
    });
  ...
  it('matches both name and ticket identifier hits on read-only boards', async () => {
    renderBoardViews({
      props: {
        availableViews: ['kanban', 'list'],
        publicView: true,
      ...
      tasks: [
        { ...mockTasks[0]!, display_number: 115, name: 'Fix login' },
        {
          ...mockTasks[0]!,
          display_number: 7,
          id: 'task-2',
    ...
    await waitFor(() => {
      expect(kanbanBoardProps?.tasks.map((task) => task.id).sort()).toEqual([
        'task-1',
        'task-2',
      ]);
    });
  ...
  it('never matches a synthesized identifier for tasks without a display number', async () => {
    renderBoardViews({
      props: {
        availableViews: ['kanban', 'list'],
        publicView: true,
    ...
    await waitFor(() => {
      expect(kanbanBoardProps?.tasks).toEqual([]);
    });
  });
