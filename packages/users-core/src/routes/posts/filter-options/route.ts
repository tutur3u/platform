import { createAdminClient } from '@tuturuuu/supabase/next/server';
import { NextResponse } from 'next/server';
import { getPostEmailMaxAgeCutoff } from '../../../lib/posts/date-range';
import { getUserGroupRoutePermissions } from '../../../lib/user-groups/route-auth';
import { resolveUserGroupRouteWorkspaceId } from '../../../lib/user-groups/route-helpers';

interface Params {
  params: Promise<{ wsId: string }>;
}

interface PostReviewFilterOption {
  amount: number | null;
  id: string;
  label: string;
  option_scope: string;
}

type FilterOptionsRpc = (
  fn: 'get_workspace_post_review_filter_options',
  args: {
    p_cutoff: string;
    p_included_group_ids?: string[];
    p_ws_id: string;
  }
) => Promise<{ data: PostReviewFilterOption[] | null; error: unknown }>;

export async function GET(request: Request, { params }: Params) {
  const { wsId: rawWsId } = await params;
  const wsId = await resolveUserGroupRouteWorkspaceId(rawWsId, request);
  const permissions = await getUserGroupRoutePermissions(wsId, request);
  if (!permissions) {
    return NextResponse.json({ message: 'Unauthorized' }, { status: 401 });
  }

  const includedGroups = new URL(request.url).searchParams.getAll(
    'includedGroups'
  );
  const sbAdmin = await createAdminClient();
  const privateClient = sbAdmin.schema('private') as unknown as {
    rpc: FilterOptionsRpc;
  };
  const { data, error } = await privateClient.rpc(
    'get_workspace_post_review_filter_options',
    {
      p_cutoff: getPostEmailMaxAgeCutoff(),
      p_included_group_ids:
        includedGroups.length > 0 ? includedGroups : undefined,
      p_ws_id: wsId,
    }
  );

  if (error) {
    console.error('Error loading post filter options', { error, wsId });
    return NextResponse.json(
      { message: 'Failed to load filter options' },
      { status: 500 }
    );
  }

  const options = data ?? [];
  return NextResponse.json({
    excludedUserGroups: options
      .filter((option) => option.option_scope === 'exclude_group')
      .map((option) => ({
        amount: Number(option.amount ?? 0),
        id: option.id,
        name: option.label,
      })),
    userGroups: options
      .filter((option) => option.option_scope === 'include_group')
      .map((option) => ({
        amount: Number(option.amount ?? 0),
        id: option.id,
        name: option.label,
      })),
    users: options
      .filter((option) => option.option_scope === 'user')
      .map((option) => ({ full_name: option.label, id: option.id })),
  });
}
