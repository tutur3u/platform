...
it('keeps private scenario configuration separate from truncated meeting history', () => {
  const scenarioInstructions = `${'x'.repeat(24000)} end-of-configuration`;
  const prompt = buildLiveInstructions({
    ...input,
    mode: 'room',
    sharedContext: 'h'.repeat(16000),
    scenarioInstructions,
  });
  expect(prompt).toContain('end-of-configuration');
  expect(prompt).not.toContain('h'.repeat(12001));
});
