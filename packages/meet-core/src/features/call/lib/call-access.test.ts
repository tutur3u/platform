...
vi.mock('@tuturuuu/meet-core/parley/repository', () => ({
  isParleySession: mocks.scenario,
}));
vi.mock('@tuturuuu/meet-core/parley-access', () => ({
  hasParleyAccess: mocks.parleyAccess,
}));
vi.mock('@tuturuuu/meet-core/runtime', () => ({
  get MEETING_APP() {
    return mocks.app;
  },
}));
...
beforeEach(() => {
  mocks.app = 'meet';
  vi.resetAllMocks();
  mocks.scenario.mockResolvedValue(false);
...
it('denies non-whitelisted access to a Parley room through Meet', async () => {
  mocks.scenario.mockResolvedValue(true);
  mocks.parleyAccess.mockResolvedValue(false);
  await expect(getMeetCallAccess(meetingId, 'Guest')).rejects.toMatchObject({
    status: 403,
  });
});
it('admits an approved Parley participant with an approved facilitator', async () => {
  mocks.app = 'parley';
  mocks.scenario.mockResolvedValue(true);
  mocks.parleyAccess.mockResolvedValue(true);
  expect(await getMeetCallAccess(meetingId, 'Guest')).toMatchObject({
    isHost: false,
  });
  expect(mocks.tier).not.toHaveBeenCalled();
...
it('keeps scenario AI policy isolated even for approved users visiting Meet', async () => {
  mocks.scenario.mockResolvedValue(true);
  mocks.parleyAccess.mockResolvedValue(true);
  await expect(getMeetCallAccess(meetingId, 'Guest')).rejects.toMatchObject({
    status: 403,
