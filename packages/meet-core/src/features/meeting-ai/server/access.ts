...
export async function meetAiAccess(
  ...
  if (MEETING_APP === 'parley' && !(await hasParleyAccess(user.id)))
    throw new MeetAiError(403, 'Parley access required');
  ...
  if (MEETING_APP === 'parley' && !parleySession)
    throw new MeetAiError(404, 'Scenario session not found');
  if (parleySession && MEETING_APP !== 'parley')
    throw new MeetAiError(403, 'Open this scenario in Parley');
