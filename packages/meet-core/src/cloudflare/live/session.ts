...
import {
  ...
  EMPTY_LIVE_JOURNAL,
  needsLiveCheckpoint,
} from '../../features/live-assistant/context';
import {
  acceptsLiveAudio,
  type LiveAssistantEvent,
  liveClientCommandSchema,
} from '../../features/live-assistant/contracts';
import { verifyLiveSession } from '../../features/live-assistant/token';
...
export class MeetLiveDurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (
      ...
      (claims.app ?? 'meet') !== (saved.claims.app ?? 'meet')
    )
      return new Response('Unauthorized', { status: 401 });
    if (
      request.headers.get('Origin') !==
        (saved.claims.app === 'parley'
          ? this.env.PARLEY_APP_URL
  ...
  private async connect() {
    ...
    try {
      ...
      const provider = await this.providerFactory({
        ...
        scenarioInstructions: saved.scenarioInstructions,
