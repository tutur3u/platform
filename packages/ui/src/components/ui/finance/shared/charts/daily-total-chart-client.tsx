...
interface DailyTotalChartClientProps {
  wsId: string;
  currency?: string;
  className?: string;
...
export function DailyTotalChartClient({
  wsId,
  currency = 'USD',
  className,
  ...
  const dateRange = useMemo(() => {
    const endDate = dayjs().subtract(dateOffset * 14, 'days');
    const startDate = endDate.subtract(13, 'days');
    return {
