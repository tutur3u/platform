...
import { Calendar, CalendarDays, CalendarRange, Clock, Users } from "lucide-react"
  ...
  if (plans.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <h3 className="mb-3 text-xl font-semibold text-foreground">{t("no_plans_yet")}</h3>
        <p className="max-w-md text-center text-sm leading-relaxed text-foreground/70">{t("no_plans_desc")}</p>
      </div>
    )
  }
      ...
      {plans.map((plan: MeetTogetherPlanWithParticipants) => (
        ...
        >
          ...
          <div className="flex-1 space-y-3">
                ...
                {plan.start_time && (
                  <div className="rounded-full bg-foreground/10 px-3 py-1 text-xs font-medium whitespace-nowrap text-foreground/80">
                    ...
                    {Intl.NumberFormat("en-US", {
                      ...
                      Number.parseInt(plan.start_time?.split(/[+-]/)?.[1] ?? "0") *
            ...
            {/* Description */}
            {plan.description && (
              <p className="line-clamp-1 text-sm leading-relaxed text-foreground/70">{plan.description}</p>
            )}
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-foreground/60">
              ...
              {plan.start_time && plan.end_time && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    <UserTime time={plan.start_time} /> - <UserTime time={plan.end_time} />
                  </span>
                </div>
              )}
