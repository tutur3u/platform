...
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@tuturuuu/ui/form"
...
const convertToTimetz = (time: number | undefined, utcOffset: number | undefined) => {
...
export default function CreatePlanDialog({ plan }: Props) {
  ...
  const form = useForm({
    ...
    values: {
      ...
      dates: plan.dates?.sort((a, b) => a.getTime() - b.getTime())?.map((date) => dayjs(date).format("YYYY-MM-DD")),
      is_public: true,
      ws_id: plan.wsId,
  ...
  const missingFields = !plan.startTime || !plan.endTime || !plan.timezone || !plan.dates?.length
      ...
      <DialogTrigger asChild>
        <button
          ...
          className={cn(
            ...
            missingFields || creating ? "cursor-not-allowed opacity-30" : "cursor-pointer",
          )}
        ...
        >
          <div
            className={cn(
              ...
              missingFields || creating || "group-hover:-inset-1 group-hover:opacity-100 group-hover:duration-200",
            )}
          />
