...
import {
  createTimedEventFromAllDay,
  findCalendarElements,
} from './calendar-utils';
import { HOUR_HEIGHT, MIN_COLUMN_WIDTH } from './config';
...
export const AllDayEventBar = ({ dates }: { dates: Date[] }) => {
  ...
  const detectDropZone = useCallback((clientY: number) => {
    ...
    if (!calendarView) {
      console.log(
        'DetectDropZone: calendar-view not found, defaulting to all-day'
      );
      return 'all-day';
    }
    ...
    if (!allDayContainer) {
      console.log(
        'DetectDropZone: allDayContainer not found, defaulting to all-day'
      );
      return 'all-day';
    }
    ...
    const isInTimedArea =
      clientY > allDayRect.bottom + DETECTION_BUFFER &&
  ...
  const calculateVisibleHourOffset = useCallback(
    (clientY: number, cellRect: DOMRect, cellHour: number): number => {
  ...
  const calculateTargetDateIndex = useCallback(
    (
      clientX: number,
      timeTrailRect: DOMRect,
      calendarViewRect: DOMRect
    ): number => {
  ...
  const roundToNearestQuarterHour = useCallback(
    (hourFloat: number): { hour: number; minute: number } => {
      ...
      const clampedMinute =
        clampedHour === 23 && finalMinute > 45 ? 45 : finalMinute;
  ...
  const calculateTimeSlotTarget = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (
        ...
        console.warn('CalculateTimeSlotTarget: Invalid input coordinates', {
          clientX,
          clientY,
        });
        return null;
      ...
      const { timeTrail, calendarView: calendarViewDiv } =
        findCalendarElements();

      if (!timeTrail || !calendarViewDiv) {
        console.log(
          'CalculateTimeSlotTarget: timeTrail or calendarViewDiv not found'
        );
        return null;
      }
      ...
      if (!hourAttr || isNaN(parseInt(hourAttr))) {
        console.log(
          'CalculateTimeSlotTarget: invalid or missing data-hour attribute'
        );
        return null;
      }
      ...
      if (cellHour < 0 || cellHour > 23) {
        console.log(
          'CalculateTimeSlotTarget: hour out of valid range',
          cellHour
        );
        return null;
      }
      ...
      if (cellRect.width === 0 || cellRect.height === 0) {
        console.log(
          'CalculateTimeSlotTarget: calendar cell has zero dimensions'
        );
        return null;
      }
      ...
      const actualHour = calculateVisibleHourOffset(
        clientY,
        cellRect,
        cellHour
      );
      ...
      if (relativeY < 0) {
        console.log(
          'CalculateTimeSlotTarget: relativeY < 0, above timed calendar'
        );
      ...
      const clampedDateIndex = calculateTargetDateIndex(
        clientX,
        timeTrailRect,
        calendarViewRect
      );
      ...
      if (!targetDate) {
        console.log(
          'CalculateTimeSlotTarget: no target date found for index',
          clampedDateIndex
        );
        return null;
      }
      ...
      const { hour: clampedHour, minute: clampedMinute } =
        roundToNearestQuarterHour(hourFloat);
    ...
    (e: MouseEvent) => {
      ...
      if (targetZone === 'timed') {
        ...
        if (currentDragState.draggedEvent && timeSlotTarget) {
          ...
          console.log(
            'Drag Move - Timed Zone but no valid timeSlotTarget, clearing cross-zone state'
          );
          setCrossZoneDragState({
            isActive: false,
            draggedEvent: null,
      ...
      setDragState((prev) => ({
        ...
        targetDateIndex:
          targetZone === 'all-day'
            ? Math.floor(relativeX / (rect.width / visibleDates.length))
            : null,
      }));
    },
    [
  ...
  const handleDragEnd = useCallback(async () => {
    ...
    try {
      if (targetZone === 'timed' && currentDragState.timeSlotTarget) {
        ...
        console.error(
          'Conversion failed: targetZone is timed but timeSlotTarget is null',
          {
            targetZone,
            timeSlotTarget: currentDragState.timeSlotTarget,
        ...
        toast({
          ...
          description:
            'Could not determine target time slot. Please try again.',
          variant: 'destructive',
        });
      } else if (
        targetZone === 'all-day' &&
        currentDragState.targetDateIndex !== null
      ) {
        ...
        if (targetDateIndex !== originalDateIndex) {
          ...
          const currentStart = getDayjsDate(
            currentDragState.draggedEvent.start_at
          );
      ...
      toast({
        title:
          targetZone === 'timed'
            ? 'Conversion to Timed Event Failed'
            : 'Event Move Failed',
        description:
  ...
  const barHeight = Math.max(
    MIN_BAR_HEIGHT_REM,
    eventLayout.maxVisibleEventsPerDay * EVENT_HEIGHT_MULTIPLIER
  );
    ...
    <div className="flex">
      ...
      <div
        ...
        className={cn(
          ...
          crossZoneDragState.isActive &&
            crossZoneDragState.sourceZone === 'timed' &&
            crossZoneDragState.targetZone === 'all-day' &&
            'border-green-300/50 bg-green-50/20 shadow-inner dark:border-green-700/50 dark:bg-green-900/10',
      ...
      >
        ...
        >
          {visibleDates.map((date, dateIndex) => {
            ...
            const isTimedToAllDayConversion =
              crossZoneDragState.isActive &&
              crossZoneDragState.sourceZone === 'timed' &&
              crossZoneDragState.targetZone === 'all-day';
              ...
              <div
                ...
                className={cn(
                  ...
                  isCrossZoneDropTarget &&
                    'border-green-300/70 bg-green-50/60 dark:border-green-700/70 dark:bg-green-900/40',
                  // General timed-to-all-day conversion visual feedback (entire bar)
                  isTimedToAllDayConversion &&
                    !isCrossZoneDropTarget &&
                    'border-green-200/40 bg-green-50/30 dark:border-green-800/40 dark:bg-green-900/20',
        ...
        {crossZoneDragState.isActive &&
          ...
          crossZoneDragState.targetDate && (
            <div
              className={cn(
                'absolute z-20 rounded-sm border-2 border-dashed transition-all duration-150',
                'border-green-500 bg-green-100/80 dark:border-green-400 dark:bg-green-900/60',
            ...
            >
              <div className="flex items-center gap-1 truncate font-semibold text-green-800 text-xs dark:text-green-200">
                <svg
                ...
                <span className="truncate">
                  {crossZoneDragState.draggedEvent?.title || 'Event'}
                </span>
              </div>
            </div>
          )}
      ...
      {scrollIndicator.show && (
        ...
        >
          <div className="flex animate-pulse items-center gap-2 rounded-lg bg-blue-500/90 px-3 py-2 font-medium text-sm text-white shadow-lg backdrop-blur-sm">
